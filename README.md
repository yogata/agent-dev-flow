...
| コマンド | 役割 | 対象フェーズ |
|----------|------|-------------|
| `issue-req` | 要件の壁打ち・整理 | ①バイブス壁打ち |
| `issue-save-req` | 壁打ち成果物をREQ/ADRファイルとして保存 | ①バイブス壁打ち |
| `issue-create` | REQファイルからGitHub Issue作成 | ①バイブス壁打ち |
| `issue-work` | 計画立案から実装・コミット・PR作成まで一括実行 | ②構造的実行 |
| `issue-update` | Issue本文の更新やコメント追加 | ②③ |
| `issue-close` | PRマージ・記録追記・Issueクローズ・ブランチ削除 | ③レビュー完了 |
| `issue-backlog` | クローズ済みissue/PRから残課題を抽出・分類しdraftとして保存 | ①バイブス壁打ち |
| `issue-backlog-create` | 承認済みバックログdraftからEpic+子Issueを作成 | ①バイブス壁打ち |
...
| 用語 | 定義 |
|------|------|
| ①バイブス壁打ち | 壁打ちフェーズ。AIと対話し要件・設計を固める |
| ②構造的実行 | 構造的実行フェーズ。Planに沿って実装しコミットする |
| ③レビュー完了 | レビュー完了フェーズ。PRマージ・Issueクローズ・事後処理 |
...
| SSoT | Single Source of Truth。情報の唯一の正しい源。フェーズごとに遷移: ①=セッション会話+draft、②=Issue本文+Work Plan、③=PR+レビュー結果 |
