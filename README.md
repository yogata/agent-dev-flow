...
| `intake-promote` | 昇華候補を learning パイプラインの入力に変換 | 昇華 |
...
| `agentdev-template-manager` | Issue/PR/REQ/ADRテンプレートを管理 |
| `agentdev-case-orchestration` | case-run の状態機械・Wave scheduling・サブエージェント実行プロトコルを提供 |
| `agentdev-learning-capture` | エージェント主体で学びを検知・抽出・自律蓄積 |
| `agentdev-learning-orchestration` | learning pipeline の schema・評価・処分区分・prune 方針を提供 |
