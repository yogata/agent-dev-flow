...
| 分岐 | 条件 | アクション |
|------|------|------------|
| Pattern B | 機能追加（`enhancement`, `feature`） | `.sisyphus/drafts/req-draft-{topic-slug}.md` にドラフト保存 → `issue-save-req` へ |
| Pattern A | バグ修正・軽微変更（`bug`, `critical`） | セッション内で要件doc完結 → `issue-create` へ |
| 承認ゲート | ユーザー承認 | 次ステップへ |
...
| スキル | 用途 | 活用内容 |
|--------|------|----------|
| `req-analysis` | 壁打ちメソドロジー・深掘り | 機能/非機能要件展開、Given-When-Then形式の完了条件、チェックボックス品質基準 |
| `adr-guidelines` | ADR閾値判定 | 技術判断がADR閾値以上かの判定。判断の記録のみ（ファイル作成は不可） |
| `issue-lifecycle` | Pattern判定 | Pattern Registry（Pattern A/Bの判定基準、固有ルール） |
...
- Pattern B: `.sisyphus/drafts/req-draft-{topic-slug}.md`（draft-metaセクション付き）
- Pattern A: セッション内要件doc（ファイル出力なし）
...
| 分岐 | 条件 | アクション |
|------|------|------------|
| Pattern A検出 | `draft-meta.pattern == A` | 即座にエラー中止 |
...
| `issue-lifecycle` | Pattern判定 | Pattern B以外の実行禁止ガード |
...
| Pattern A | バグ修正 | `issue_desc_bug.md` テンプレート + `issue_comment_bug_analysis.md` コメント |
| Pattern B | 機能追加 | `issue_desc_feature.md` テンプレート + `issue_comment_feature_technical.md` コメント |
...
| テンプレート | 用途 | Pattern |
|-------------|------|----------|
| `issue_desc_feature.md` | Issue本文（機能追加） | Pattern B |
| `issue_desc_bug.md` | Issue本文（バグ修正） | Pattern A |
| `issue_comment_feature_technical.md` | 技術検討コメント | Pattern B |
| `issue_comment_bug_analysis.md` | 分析結果コメント | Pattern A |
...
| Pattern B | 機能追加 | specs更新（system.md / patterns.md） |
| Pattern A | バグ修正 | specs更新スキップ |
...
| スキル | 用途 | 活用内容 |
|--------|------|----------|
| `req-analysis` | 要件検証 | チェックボックス品質基準による完了条件検証 |
| `spec-compliance` | 乖離検出 | 要件と実装の乖離検出・報告 |
| `issue-lifecycle` | Pattern判定・フェーズ管理 | Pattern A/Bの判定、specs更新要否 |
...
| Pattern B | 機能追加 | docs/検証実行（REQ、specs、ADR、docs/README.md） |
| Pattern A | バグ修正 | docs/検証スキップ |
...
| Pattern Aクローズ | バグ修正 | `issue_comment_bug_record.md` テンプレート |
| Pattern Bクローズ | 機能追加 | `issue_comment_feature_implementation.md` テンプレート |

#### 使用スキル一覧

| スキル | 用途 | 活用内容 |
|--------|------|----------|
| `issue-lifecycle` | Pattern判定 | Pattern A/Bでdocs/検証要否を判定 |
...
| テンプレート | 用途 | Pattern |
|-------------|------|----------|
| `issue_comment_feature_implementation.md` | 実装記録コメント | Pattern B |
| `issue_comment_bug_record.md` | 対応記録コメント | Pattern A |
...
|------|----------|
| 壁打ち | `/issue/issue-req` |
| 壁打ち→構造的実行準備（Pattern B） | `/issue/issue-save-req` |
| 壁打ち→構造的実行の境界（Pattern B） | `/issue/issue-save-req` |
| 壁打ち→構造的実行の境界（Pattern A） | `/issue/issue-create` |
| 構造的実行 | `/issue/issue-work` |
| 構造的実行→レビュー完了の境界 | レビュー待ち |
| レビュー完了 | `/issue/issue-close` |
| 乖離あり（重大2件以上） | 壁打ちへループバック |
...
| ドラフト保存（Step 5B） | issue-req | ✅ 使用中 | Pattern Bの標準フロー |
...
| REQ CREATE | issue-save-req | ✅ 使用中 | 全Pattern Bで基本操作 |
...
| specs更新 | issue-work | ⚠️ 条件付き使用 | Pattern Bのみ |
...
| docs/検証（REQ/specs/ADR/README） | issue-close | ⚠️ 条件付き使用 | Pattern Bのみ |
