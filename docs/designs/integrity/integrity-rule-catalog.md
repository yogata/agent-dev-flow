...
整合性検査の全ルールのスキーマ定義とルールインデックスを維持するカタログ（REQ-010-011, REQ-001-008）。
...
- [IR-005: Decision ↔ REQ 相互参照存在](rules/IR-005-adr-req-bidirectional-reference.md)
...
- [IR-025: 廃止 Decision path 規則](rules/IR-025-retired-adr-path-rule.md)
...
- [IR-058: distribution-untracked-skill-reference](rules/IR-058-distribution-untracked-skill-reference.md)
...
- [IR-070: design-frontmatter-required-keys](rules/IR-070-design-frontmatter-required-keys.md)
- [IR-071: integrity-rule-related-req-existence](rules/IR-071-integrity-rule-related-req-existence.md)
<!-- AUTOGEN:END -->
...
プレースホルダセルに em-dash（`—`）を使用しない置換表記を正規のものとして確定する（判別基準は PR 2271 で確定済み。規則確定と是正の分離により、是正実行は AG-004 コーパス機械是正が担う）。
...
**regression test fixture mirroring 方式**:
