...
|-------|-------|
| rule_id | IR-052 |
| description | 完了条件 grep パターン（機械的 checkbox 検出、`- [ ]` / `- [x]` カウント）は、否定文脈、anti-pattern 例示を「未達」として捕捉しないこと（REQ-010-011）。本ルールは grep ベース検出を実装する際の設計基準であり、現在の QG-4 完了条件評価は推論ベース（case-close STEP-2）であるため、grep 実装追加時に適用する |
...
現在の QG-4 完了条件評価は推論ベース（case-close STEP-2、agent が各 checkbox の達成を意味判断）。
