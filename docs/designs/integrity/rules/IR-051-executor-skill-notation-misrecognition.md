...
|-------|-------|
| rule_id | IR-051 |
| description | docs/SPEC/command/skill で 既知 command 名、既知 harness 名、既知 subagent 名（語彙レジストリ `.opencode/skills/repo-agentdev-integrity/references/vocabulary-registry.md` 参照）が「スキル」「skill」と表記されていることを検出すること。実行主体の分類（command / skill / subagent / harness）は `docs/designs/responsibilities/document-type-responsibilities.md`「実行主体分類の査読基準」に定義される |
| severity | heuristic |
