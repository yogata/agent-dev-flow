...
- 既存の「Phase 1 case-open 順次、Phase 2 case-run 並列、Phase 3 case-close 順次」は「orchestration stage 1 case-open・stage 2 case-ready・stage 3 case-run・stage 4 case-close」へ置き換える
...
### REQ 再評価基準（ステークホルダー視点、四妥当性基準）

現行 REQ 全件について、REQ-001-048（ステークホルダー視点、四妥当性基準）に基づく再評価を実施する。要件行単位で次を判定する:
...
安定契約例外（REQ-001-068）に該当する要件行は再評価の対象外とする。
...
| 正規所有対象 | 対象 command、skill、workflow、品質ルール、整合性ルール等の関心キーが宣言されている（REQ-001-013、REQ-082-009） |
