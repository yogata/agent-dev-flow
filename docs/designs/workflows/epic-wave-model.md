...
- L2（ファイル衝突）は Wave 分離の理由としない。並列実行を許容し、コンフリクトは rebase で機械的解消する（コンフリクト解消 Level 1、REQ-035-009）
...
### execution_unit 構成の依存ヒントと Wave 構成判断基準

連結成分計算の必須依存エッジに加え、同一ファイル衝突の抑制のため次の依存ヒントを execution_unit 構成の依存判定へ反映する。依存ヒントはエッジ判定要素への追加に留め、連結成分アルゴリズムの構造自体は維持する。
...
依存ヒントを検知した場合の Wave 構成判断基準は次のとおり、重複の種別・近接度に応じて判断する（全ファイル重複の全面禁止とはしない）。

- 直列化: 同一ファイルの同一リージョンを変更する確度が高い場合（同一 Wave 内での衝突確実時）
- 依存付与: 前段 OU の完了を待って後段 OU を実行する場合（部分的重複・近接時）
- 単一 PR 集約: 変更が小規模で機械置換同士等、分割のコストが衝突解消コストを上回る場合
