...
| .agentdev/jev-observations/ | repo 内正規状態 | 未評価 Observation（Jev 先行評価の観測記録。1 semantic evaluation = 1 observation。生成元: Jev 先行評価を実行する6系統 Workflow（Jev Custom Tool 経由）。利用元: Jev 有効性評価（Issue B）。git 管理対象の永続 domain state（REQ-002-012 準拠。JSONL は正本としない）。評価・置換判断の進行に応じた整理方針は評価 Issue（B）で定める） |
