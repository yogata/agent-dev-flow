...
Issue #2362 の移行作業（棚卸し (a')）で全現行要件行を判定した結果である。判定基準は traceability-model.md「対応関係の完全性規則」の目安（恒続的な成果物として検証可能な対象を持つか）に従い、検証対応の宣言対象とならなかった行を実行時の品質ゲートやレビューで検証する性質の行として登録した。REQ-049 のエントリは Issue #2420 の対応関係補完で追加した。
...
- REQ-049-001..REQ-049-004: 発生から反映確認までの継続追跡の総合能力、対象粒度の非限定と解決結果の多様性、起票時の反映先決め打ち禁止、Intake / Learning、GitHub Issue、Decision、RU との系統分離の実行時振る舞いと責務境界規定
- REQ-049-005: 課題ファイルの正規配置先、git 管理対象性、`.agentdev/` 配下一時成果物との区別。運用規則
- REQ-049-014: 課題ファイルを履歴系文書とする検査区分。文意品質検査の適用区分の実行時規則
- REQ-049-015..REQ-049-019: 操作能力の共通能力としての提供、複数 workflow からの利用、Skill 内部構成の Design 確定、GitHub Issue 管理 skill との区別、`/agentdev/issue` 公開入口の提供範囲
- REQ-049-020..REQ-049-025: 自然言語での操作、明示実行の非必須化、未解決事項の課題管理利用、課題化判断、承認・判断境界の非迂回、保留課題の再評価条件確認（到達機構は検証対応必須行 REQ-049-026 として検証）の実行時振る舞い
- REQ-049-027..REQ-049-028: 再評価後の分岐選択、成果物更新責務の非所有と委譲の実行時振る舞いと責務境界規定
- REQ-049-030: 課題管理機構の初期課題登録の実行時作業（登録済み ISL-001 が実装対応を保持）
