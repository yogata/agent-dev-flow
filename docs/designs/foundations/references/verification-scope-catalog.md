...
Issue #2362 の移行作業（棚卸し (a')）で全現行要件行を判定した結果である。判定基準は traceability-model.md「対応関係の完全性規則」の目安（恒続的な成果物として検証可能な対象を持つか）に従い、検証対応の宣言対象とならなかった行を実行時の品質ゲートやレビューで検証する性質の行として登録した。REQ-049 のエントリは Issue #2420 の対応関係補完で追加し、Issue #2439 の REQ-049 全面再構成（要件行 001〜019 への置換）に合わせて再構成した。
...
- REQ-052-011: Plugin/Hook 実行時状態の決定的切替（DEC-023 種別契約の延長行）。実行時振る舞いとレビューで検証

### REQ-053（OpenCode 同一セッションモデル昇格と ADF 共通設定）

- REQ-053-001..REQ-053-017: モデル昇格機構は実動作検証（test_strategy）とレビューで検証。checker 新設は DEC-023 決定6により禁止
