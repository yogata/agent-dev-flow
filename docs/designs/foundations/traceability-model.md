...
- 検証スコープポリシーは、旧「検証対応要否カタログ」（references/verification-scope-catalog.md）を置き換える。当該カタログは本モデルから廃止され、カタログが保持していた ADF 自身の検証対応スコープデータは ADF リポジトリの `traceability/policy.yaml` へ移行する
