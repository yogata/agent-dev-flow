...
- 要件行の品質観点（要件性チェックリスト、1行1関心、肯定文主文、粒度、配置妥当性。REQ 分析における意味品質の正規所有）
...
- 要件行が必達要件として記述されているか
- 要件行の品質（主語、対象、状態、検証可能性、独立性、1行1関心、肯定文主文）を満たしているか（原本は document-type-responsibilities Design「要件行の書き方」「要件性」節）
