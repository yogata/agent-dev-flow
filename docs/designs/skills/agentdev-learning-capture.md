...
- git 永続化は呼出元 command（case-ready / case-revise / case-open / case-close）の責務（後述「呼出元 command 契約」参照）
- 実観測ベース（実際に検知、回避、修正した問題のみ）

## 呼出元 command 契約

本スキルは、主ワークフロー構成工程 command（case-ready / case-revise / case-open / case-close）から委譲を受ける。
