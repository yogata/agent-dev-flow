...
- gh 直接記述の委譲漏れ検出（REQ-011）。スキャン対象、除外対象は [IR-053: gh 直接記述検出](../integrity/rules/IR-053-gh-direct-invocation-detection.md) 参照
...
- 粒度の適切性（Skill 分割候補検出）
- 診断分類の正確性
- gh 直接記述の委譲漏れ（REQ-011）。command/skill 配下で Custom Tool `agentdev_gh` 操作契約へ委譲すべき gh 直接記述が残留していないか。Tool 実装・検出の除外対象が正しく機能しているか
