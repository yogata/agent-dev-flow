...
<!-- ADF-COVERS(implementation): REQ-012-030, REQ-012-043, REQ-012-044, REQ-012-045, REQ-012-046, REQ-012-047, REQ-012-048, REQ-012-049, REQ-012-050 -->
...
- 未知の成果物役割
- 存在しない要件への参照
- 検証対応要否カタログの無効なエントリ・参照（存在しない要件行への参照、エントリ形式違反、同一REQファイル外・逆順の範囲、カタログ読取不能）
- 実装対応の欠落（現行要件で0件。全現行要件行が計上対象）
