...
req-define が要件を確定する前にアーキテクチャ上の影響を確認するため、アーキテクチャ助言サブエージェント（harness 側で選定）が助言を提供し、最終判断は親エージェントが行う。

## 適用対象

- req-define Step 4（要件展開）後、Step 5（ADR 判断）前
- 既存 REQ/Decision/SPEC との衝突可能性、ADR 作成候補、責務境界変更候補がある場合
...
- 相談入力: 要件候補、既存 REQ/Decision/SPEC の矛盾候補、Decision 候補、SPEC 候補、責務境界変更、未解決分岐、具体質問
