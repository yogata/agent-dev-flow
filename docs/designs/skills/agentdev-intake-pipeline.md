...
各工程 command（`case-ready` / `case-revise` / `case-open` / `case-close`）からの自動 deviation capture 要求を受ける item 生成操作。
...
| 区分 | 入力形式 | 呼出元 | 役割 |
|---|---|---|---|
| 自動 capture 向け item 生成操作（本操作） | 各工程 command が実観測した deviation（構造化済み） | `case-ready` / `case-revise` / `case-open` / `case-close`（プログラム的委譲） | 自動 deviation capture |
