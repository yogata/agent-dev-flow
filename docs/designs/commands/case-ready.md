...
- 実行構造確定: 連結成分、3軸判断、単独根の Standard 化、構成検証（Epic サイズ上限、意味的依存の維持〔DAG 整合〕、全 operation_unit の Wave 割当）、Wave ファイル重複前置検出（詳細は本 Design「v3 epic-wave-model Design からの吸収」節）。Wave は意味的依存 DAG のみから構成され、Wave サイズに実行時並列数・同時実行上限を適用しない（DEC-041、REQ-061-010、REQ-061-038）
