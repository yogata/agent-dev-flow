...
> 全ての `/agentdev/*` 公開コマンドは、`docs/designs/commands/<command-name>.md` に専用 SPEC を持つ。
...
- 横断 SPEC（`docs/designs/workflows/`）は共通契約のみを記載する。個別 command SPEC の動作は当該 command SPEC に書く。
...
## command SPEC と command 定義の対応付け（v2:REQ-0143-005）

command SPEC は command 定義ファイル（`src/opencode/commands/agentdev/*.md`）の Step 番号を複製せず、以下の軸で command 定義と対応付ける（v2:REQ-0143-005）。
