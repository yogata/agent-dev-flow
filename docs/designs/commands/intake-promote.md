...
- GitHub Issue 作成
- intake item 元内容の改変
- backlog-review の自動起動
- learning pipeline 入力生成
- learning item 保存、分類、昇華
- ユーザー明示的承認なしの採用済み成果物生成（`POL-promoted-artifact-requires-approval`）
- 分類結果の非提示（必ず提示、確認修正機会提供）
- 分類未確定のままの自動確定、自動進行（REQ-003-003。確定後の自動進行は REQ-003-008 で許容）
- workflow 管理成果物の扱い
- 整形結果への frontmatter 含有
- 整形結果への重複排除キー、後続成果物参照含有
- 元 item 本文への整形結果書込
- `.agentdev/intake/accepted/` の参照使用
- 保存先 `.agentdev/intake/promoted/` 直下以外
- 破壊的変更（inbox 全体強制クリア、大量エントリ一括削除等）の分類承認とは別系統の明示承認の省略（`POL-destructive-change-explicit-approval`）

## 検証観点

- HITL 承認の確実性（採用済み成果物生成の承認、分類結果の提示、分類未確定時の自動進行禁止）
- 整形結果の frontmatter / 重複排除キー / 後続成果物参照を含まないこと
- 保存先が `.agentdev/intake/promoted/` 直下のみであること
- 採用 item 元ファイルの inbox 削除（`archive/promoted/` への移動を廃止）
...
既存の HITL（採用済み成果物生成の承認、分類結果の提示、分類未確定時の自動進行禁止）、自動実行ルール（REQ-003-008）、破壊的変更の明示承認（`POL-destructive-change-explicit-approval`）は変更しない。
