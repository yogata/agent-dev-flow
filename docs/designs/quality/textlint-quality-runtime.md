...
updated: 2026-09-11
---
<!-- ADF-COVERS(implementation): REQ-053-024, REQ-053-025, REQ-053-026, REQ-053-027, REQ-053-028, REQ-053-029, REQ-053-030, REQ-053-031, REQ-053-032, REQ-053-033, REQ-053-034, REQ-053-035, REQ-053-036, REQ-053-037, REQ-053-038, REQ-010-075, REQ-057-013 -->
...
- 機構固定除外: node_modules は依存パッケージ実装物であり、加算設定（追加対象）があっても再包含しない。この挙動は src/opencode/plugins/agentdev-textlint-guard/lib/targets.ts の固定除外実装と一致する。機構固定除外の解除は本 Design の改定を要する
- 再包含可能な既定除外: 歴史記録サブツリー（docs/reports 等）は過去記録の誤検出防止を目的とする既定除外であり、明示的な加算設定により再包含できる
...
- max 値・disableXxx の option 校正を実施し、校正後の検出件数と残存指摘を確認する。汎用 preset 既定値は専門用語・定義リスト・注意喚起太字を含む corpus で大量の誤検出を生む実績（1,856件・1,825件規模）があるため、実測なしの採用を行わない
...
規則構成の同一性を版記録で突合する場合、規則構成ハッシュは実行環境の配置位置に依存しない手順で計算する。
prh 規則の options.rulePaths は実行環境の plugin dir 絶対パスを含むため、hash 入力の構成前に正規化する。
