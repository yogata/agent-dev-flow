...
公開 `/agentdev/*` command の手順セクションの正規形は表形式（前得出出力検証表）である。
`### Step N` 見出しの逐次列挙に代え、各工程を前提条件・出力契約・検証基準の表で記述する。
...
ガードレール番号 Gxx の連番制度（G01 起点・欠番なし・重複なし・変換対照表）は廃止済みであり、現行は意味識別子による（REQ-051）。
...
- 従来の Step 0 検出・非連番検出・numbered list 主手順検出は前出出力検証表様式への移行に合わせて廃止済みである。現行の検査は、公開 command の `### Step N` 見出し残存の否定検出（`command-format-public-step-heading`）、`## workflow` セクションの表形式必須検出（`command-format-workflow-table`）、および IR-028/029 による旧様式の誤用検出（英字混在見出し・英字サブステップ）である
...
repo-local command（`/repo/*`）が従来形式の主手順（`### Step N`）に加えて、入力分岐等により切り替わる代替フロー（alternative flow）を持つ場合、主手順の `### Step N` 連番を維持したまま代替フローを記述するためのサブステップ表現として `**EN.**` lettered prefix 形式を許容する。
現行の使用実績は 0 件であり、様式定義として保持する（公開 command の工程表移行に伴い、かつての採用例であった case-close.md の Epic Wave クローズフローは解消済み）。
...
- `**EN.**` 形式は代替フロー専用であり、主手順の Step 表現として使用しない（repo-local command の主手順は `### Step N: タイトル` 見出しを使用する）
- 後述「機械検査対象」で `**EN.**` 形式は違反として検出しない（主手順の Step 番号連番とは独立した番号空間のため）
- 本形式の採用は `/repo/*`（repo-local コマンド）限定とする。公開 `/agentdev/*` コマンドの手順は前出出力検証表形式が正規形であるため、`**EN.**` lettered prefix 形式を使用しない
...
- 副番号（M、K）は 1 起点とする。ゼロ起点（例: `E4-0`）は使用しない。規約確定後の振り直し判断により、唯一の残存例であった `epic-wave-close.md` の `E4-0`/`E4-1` は `E4-1`/`E4-2` へ振り直し済みであり、ゼロ起点の例外は存在しない
...
- **安全境界**: command が越えてはならない責務範囲（ファイル編集スコープ等のファイル操作制約）
