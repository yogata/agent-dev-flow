...
- [commands/inspect-doc-inputs.md](specs/commands/inspect-doc-inputs.md) — `/agentdev/inspect-doc-inputs`（project doc-inputs 機構の整合性診断、読み取り専用）
...
| [project-doc-inputs.md](specs/foundations/project-doc-inputs.md) | project doc-inputs 機構（config.yaml、doc-input schema、普遍手順、例外範囲、ADR-0133・superseded by ADR-0135・歴史参照） |
...
| [workflow-contracts.md](specs/foundations/workflow-contracts.md) | ワークフロー契約（旧版・縮小済み・旧版互換索引）。正規の横断ワークフロー契約は `specs/workflows/` 配下を参照 |
...
> workflows/ と foundations/ の使い分け: 正規の横断ワークフロー契約は `workflows/` 配下、`foundations/workflow-contracts.md` は旧版互換索引（縮小済み）。
