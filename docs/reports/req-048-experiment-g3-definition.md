...
ランキング §5 が G3 の実行の前提として記録したとおり、Baseline V2 測定 §6 の harness テレメトリ読取手順の整備が測定の前提であり、整備前に実行数値は得られない。技術検証は実験実行前に、Baseline V2 測定 §6 の整備項目のうち次の2項を整備する。整備は REQ-048-003 のとおり harness 側の既存生実行履歴を読み取り専用で分析時に読む方向で行い、ADF 側の新規必須 field、永続 state、公開入口を追加しない。
...
測定対象は Baseline V2 測定 Report §2 の手順に従い、測定整備の前提節に記した整備完了後に取得する。測定の実施は、実験を技術検証として実行する際に行う。
...
- 測定整備（harness テレメトリ読取手順、session 識別子取得手段）の整備は、技術検証が実験実行の前置きとして実施する。この整備自体は本実験の主要構造変更に含めない
- 技術検証は、本 Report の Baseline 節の手順で baseline を固定記録し、測定整備を完了した上で Observation 節の指標を測定し、Guardrail 節の不変条件を確認した上で、Decision 記録形式で判断を記録する
...
| 実行・判定が後続の技術検証に分離されている | 合格 | 実験の実行・判定の分離節が、本実行単位の完了条件に含まれないことを明示。実行時の測定条件・手順確定と測定整備の前置き実施を記録 |
