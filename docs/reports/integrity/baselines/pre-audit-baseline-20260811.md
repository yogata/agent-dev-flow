...
> **履歴文脈注記（ファントム REQ-010 行参照、Issue #2248 / OU-0031）**: 本 baseline 記録中の「REQ-010-NNN」（NNN が 013〜052 帯の旧 REQ-010 行、DEC-013 で RETIRE した 053..057、または 063 を超える番号帯）は、取得時点で引用された参照である。このうち 063 を超える番号は現行 REQ-010 に存在したことのない番号（監査起案時の暫定番号割当）であり、現行要件の正規参照ではない。本ファイルは baseline 時点の記録（Report）として原文のまま保持し、参照の正規化は本注記にて代替する。
