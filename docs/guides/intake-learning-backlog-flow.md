...
13項目形式で記録する。
問題事象、発生局面、検知方法、根本原因、自律対応内容、ユーザー確認有無、Decision/REQ/spec 影響、横展開観点、再発条件、予防策候補、想定反映先、関連、タグ。
...
AgentDevFlow のパイプライン状態はディレクトリ配置とファイルの存在で表現する（v2:REQ-0112-023）。
...
`.agentdev/` は AgentDevFlow の正規のドメイン状態である（v2:REQ-0112-024）。
