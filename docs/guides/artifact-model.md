...
**優先順位**: REQ > ADR > SPEC。DOC-MAP と guides は基準を代替しない（REQ-0101, 016, 031）。
...
**参照ルール**:
- REQ → ADR、ADR → ADR、Issue → ADR の参照を許可（REQ-0101）
- ADR → Issue の逆参照は禁止（REQ-0101）
- 文書間矛盾時は REQ を優先（REQ-0101）
...
**分離原則**: Command は判定ロジックを直接記述せず Skill を `load_skills` で参照する。Skill は Command の Step 番号やファイルパスを記述しない（REQ-0103, 005）。Script は決定的で単体テスト可能な処理に限定する。

**Skill 品質基準**: SKILL.md は500行以下。Progressive Disclosure パターンで詳細は参照ファイル（1階層まで）に配置（REQ-0103, 013）。description に `USE FOR:` / `DO NOT USE FOR:` 形式でトリガー条件を記載（REQ-0103）。
