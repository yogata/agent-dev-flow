...
| フェーズ | 内容 |
|----------|------|
| 準備 | Issue 読取り、worktree 作成、前置確認 |
| 委譲 | 実行担当サブエージェントへの作業委譲 |
| クリーンアップ | 委譲結果の処理、worktree クリーンアップ |

> **委譲フェーズのサブエージェント作業**: Plan 策定、実装、テスト、docs/designs 整合性確認は委譲内の実行担当サブエージェントの作業である。
> 実装方針の形成は case-run 本体ではなく、`agentdev-case-run-execution-adapter` の委譲契約内（REQ-015-010）で行う。
