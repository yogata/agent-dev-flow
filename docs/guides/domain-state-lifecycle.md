...
| `promoted/` | 昇華判定済み artifact の配置先（req-backlog の入力） |
...
| 項目 | 内容 |
|------|------|
| 役割 | 昇華判定済み artifact の配置先（req-backlog の入力） |
| 生成トリガー | `/agentdev/learning-promote` の完了 |
| 対応コマンド | `/agentdev/learning-promote` |
| 次に進むコマンド | `/agentdev/req-backlog` |

フラット構造（`*.md`）。サブディレクトリは持たない。frontmatter を持たず、route / status はディレクトリ配置で表現する（REQ-0046-026）。req-backlog による RU 化成功後に削除される。
...
| `promoted/` | req-backlog 用 promoted artifact（フラット） |
...
| 項目 | 内容 |
|------|------|
| 役割 | req-backlog 入力用の promoted artifact |
...
stateDiagram-v2
    ...
    active --> consumed_by_req_define: req-define（読取り）
    consumed_by_req_define --> deleted: req-save（REQ永続化完了時）
    consumed_by_req_define --> deleted: case-open（Issue永続化完了時）
    active --> [*]: 矛盾検出時は残置（ユーザー確認）
```

### 基本フロー

`req-backlog` が intake/learning の `promoted/*.md` を読み込み → `.agentdev/backlog/req-units/RU-*.md` を生成 → `req-define` が RU を Requirement Source として消費 → `req-save` または `case-open` で永続化完了後に RU ファイルを削除
