...
フラット構造（`*.md`）。サブディレクトリは持たない。req-backlog による RU 化成功後に削除される。
...
気づき・課題の発生 → `/agentdev/intake-capture` → `/agentdev/intake-review` → `/agentdev/intake-promote` → `/agentdev/req-backlog` → `/agentdev/req-define`
