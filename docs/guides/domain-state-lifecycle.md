...
| `promoted/` | Requirement Source stub の staging 領域 |
| `promoted/archive/` | 取り込み済み staging stub の保管 |
...
|------|------|
| 役割 | 取り込み済み stub の記録 |
| 生成トリガー | `/agentdev/case-close` での取り込み判定 |
| 対応コマンド | `/agentdev/case-close` |
| 次に進むコマンド | なし（終端状態） |

取り込み済み staging stub の保管先です。
...
| `promoted/` | 後続コマンド用 artifact（フラット構造） |
...
| 項目 | 内容 |
|------|------|
| 役割 | 後続コマンド（req-define / intake-open）用の整形済み artifact |
| 生成トリガー | `/agentdev/intake-promote` の完了 |
| 対応コマンド | `/agentdev/intake-promote` |
| 次に進むコマンド | `/agentdev/req-define` または `/agentdev/intake-open` |
