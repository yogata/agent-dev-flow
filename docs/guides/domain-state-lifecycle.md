...
- `promoted/`: フラット構造（`*.md`）。サブディレクトリは持たない。frontmatter を持たず、route / status はディレクトリ配置で表現する（REQ-0105）。req-backlog による RU 化成功後に削除される。
...
`req-backlog` が intake/learning の `promoted/*.md` を読み込み、またはチャット内でユーザーと合意形成済みの内容を session-sourced RU として `.agentdev/backlog/req-units/RU-*.md` に保存 → `req-define` が RU を Requirement Source として読み取る → `case-open` の Issue作成 + VERIFY 成功後に RU ファイルを削除（REQ-0105-012, 015）
...
- `req-backlog` は intake/learning の raw item（inbox.md・archive/active.md・entries/）を更新しない（REQ-0105）
...
|-------|------|------|-------------|------|
| 1 | intake/learning promoted artifact | RU | `req-backlog` | N:1 統合 / 1:N 分割 / 1:1 |
