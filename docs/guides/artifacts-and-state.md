...
| ADR (retired) | `docs/adr/retired/ADR-00XX.md` | 再編前の履歴番号帯。現行根拠として使用しない |
...
- `.agentdev-plugin/`: 適用プロジェクトにおける agent-dev-flow の git clone 先（REQ-0103-072~077）。AgentDevFlow本体リポジトリでは使用しない。`.gitignore` で管理対象外とする。
