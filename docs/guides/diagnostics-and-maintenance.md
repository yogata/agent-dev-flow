...
AgentDevFlow の整合性検査と REQ 体系の健全性診断を説明する。
...
結果は `.agentdev/integrity/reports/` に JSON または Markdown 形式で出力する。
結果分類は NG / warning / info の3段階。
...
integrity 検査は検査範囲に応じて3層に分かれる。
各層の検査水準（strict / heuristic / observation）の定義と対応方針は `.opencode/skills/repo-agentdev-integrity/references/gate-levels.md` を参照する。
