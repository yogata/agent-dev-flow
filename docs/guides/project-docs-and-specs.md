...
- 現行基準は v2:ADR-0101 以降の番号帯を使用する（v2:REQ-0112-047）
- ADR-0001〜0099 は再編前の履歴番号帯であり、2026-07-20に物理削除された（v2:REQ-0112-048）。後継関係は Decision Map（`docs/adr/README.md`）の参照のこと
- 承認済み ADR の決定内容は安定して維持する（v2:REQ-0112-045）。変更が必要な場合は新規 ADR を作成する
- ADR 体系の全面改定時は例外として、ユーザー承認済み範囲で最小限を超える編集を許可する（v2:REQ-0112-044）
