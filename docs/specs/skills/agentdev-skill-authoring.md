...
- サブエージェント編集安全性
- スキル本文と references の project docs 参照は skill doc-input に集約する（ADR-0133、SPEC `../foundations/project-doc-inputs.md`）
  ...
  - 実行時に読むべき docs は `.agentdev/doc-inputs/skills/<skill>.yaml` の `conditional_read`, `allowed_discovery` へ移す
  - skill doc-input はスキル単位で1ファイルに集約し、reference ごとの doc-input は作らない
  - スキルは呼び出し元コマンドから渡された解決済み文脈を優先し、skill doc-input は不足分の追加文脈として扱う

## project docs 参照

スキル本文と references の project docs 参照は skill doc-input（`.agentdev/doc-inputs/skills/<skill>.yaml`）に集約する（ADR-0133、REQ-0157、SPEC `../foundations/project-doc-inputs.md`）。各 SKILL.md には doc-input 参照方針（4項目）を配置する:
...
2. **doc-input の読込契約**: 呼び出し元コマンドから渡された解決済み文脈を優先し、不足分のみ skill doc-input（`.agentdev/doc-inputs/skills/<skill>.yaml`）を読む。skill doc-input はスキル単位で1ファイルに集約し、reference ごとの doc-input は作らない
