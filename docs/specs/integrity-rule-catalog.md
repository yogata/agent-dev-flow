...
IR-044 は isNegationContext と同列に isDelegationContext、isMetaScopeRuleContext、isBehaviorPredicateContext を exemption（免除）条件として扱う。
委譲、集約、切り出し、存在確認の文脈、REQ/SPEC 責務範囲を規定する META 規則行、振る舞い（存在・状態）を述べる要件行で SPEC キーワード（テストデータ、チェッカー、enum 等）が現れる場合、検出を warning から免除する（REQ-0108-259, REQ-0145-012, AG-001〜004 / ACT-SPEC-001）。
...
| `isMetaScopeRuleContext` | 「REQ/SPEC 境界」「責務範囲」「〜を SPEC（領域）とする」「SPEC 対象を列挙」「SPEC 領域」「責務区分」 + enum/format/schema 等の SPEC 種別の列挙 | REQ-0101-067「enum 値、フォーマット（format）等は SPEC 領域に列挙する責務範囲規定行」（enum/format を SPEC 対象として列挙し責務範囲を規定する行） |
| `isBehaviorPredicateContext` | 存在・状態述語「〜が存在する」「〜が存在しない」「〜を実装する（状態）」 + 種別修飾語（件数・内容を規定しない修飾: 「fixture」「variant」等の drift 対象種別名） | REQ-0144-009「drift 仕組みが存在する」「fixture」は drift 対象種別を示す修飾語であり件数・内容を規定しない |
...
| SPEC キーワード + `isMetaScopeRuleContext` 成立（REQ/SPEC 責務範囲規定行） | 免除（当該行は SPEC 詳細の記述ではなく責務範囲の規定） |
| SPEC キーワード + `isBehaviorPredicateContext` 成立（存在・状態述語 + 種別修飾語のみ） | 免除（REQ-0108-259 存在確認文脈の実装明示化） |
...
- `isMetaScopeRuleContext` 適用外: SPEC 詳細そのものを列挙する行（例: enum 値 `A/B/C` の一覧、fixture の具体件数と内容を羅列する行）は免除しない。責務範囲を規定する META 行（enum/format 等 SPEC 種別を名指しして「SPEC 領域」と宣言する行）のみ免除する。両者の区別は「行が SPEC 種別を名指しして責務境界を宣言しているか」「件数・内容を規定しているか」で判定する。
- `isBehaviorPredicateContext` 適用外: 種別修飾語（「fixture」「variant」等）に加えて件数・内容を規定する記述が含まれる場合は免除しない。存在・状態述語（「仕組みが存在する」等）のみで件数・内容を規定しない行のみ免除する。
...
- 回帰テストで新 exoneration 条件が真陽性を誤って免除しないことを検証する（REQ-0108-259, REQ-0108-055 準拠）。保護対象の真陽性: REQ-0114-082、REQ-0144-008。これらは SPEC 詳細（件数・内容を含む）が REQ に残留している実例であり、`isMetaScopeRuleContext` / `isBehaviorPredicateContext` のいずれにも該当しないことをテストで固定する。
