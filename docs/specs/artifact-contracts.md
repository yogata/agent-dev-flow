...
| Repo-local Command | `.opencode/commands/repo/`（source なし） | Self-hosting repo 専用入口（ADR-0020） | ユーザー起動 | レポート、成果物 |
| Repo-local Skill | `.opencode/skills/repo-*/`（source なし） | Self-hosting repo 専用判断基準（ADR-0020） | Command からの参照 | 判断結果の参照提供 |
