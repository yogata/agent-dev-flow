...
## allowed_discovery 運用契約の充実（REQ-0157 APPEND）

`allowed_discovery` の契約を充実させる。

- **空配列の意味**: `allowed_discovery` が空配列の場合は「探索不要」を意味する。DOC-MAP または docs 配下 README から探索可能な paths を持たない command または skill（must_read, conditional_read が空、または固定パスのみ）の場合のみ空配列を許容する。
- **探索が必要な command**: req-define, req-save, spec-save, case-close, inspect-docs, inspect-doc-inputs は、必要最小限の探索許可（DOC-MAP および docs 配下 README 経由）を `allowed_discovery` に記述する。
- **探索許可の限定**: 探索許可は DOC-MAP および docs 配下 README 経由に限定する。`.agentdev/doc-inputs/**` 外のパスを直接探索許可として記述しない。
- **spec-save の特例**: spec-save の doc-input は `document-model.md`（SPEC 配置規則確認）を明示的に許可する。

## doc-inputs 更新契約（REQ-0157 APPEND）

以下の変更発生時に doc-inputs の更新が必要である。

| 変更種別 | 更新内容 |
|---|---|
| (a) command または skill の追加・削除・責務変更 | 対応する doc-input ファイルの作成・削除・`must_read`/`conditional_read`/`allowed_discovery` の見直し |
| (b) SPEC の移動・分割・改名 | 参照先 SPEC パスの更新（移動時は要エラー確認、AG-006 例外） |
| (c) `must_read` の増減 | path, purpose の追加・削除 |
| (d) `conditional_read` の条件変更 | when, paths, purpose の追加・削除・条件見直し |
| (e) 探索範囲（`allowed_discovery`）の変更 | 探索許可の追加・削除・経路見直し |
| (f) DOC-MAP または docs 配下 README の入口変更 | 探索経路の整合性確認、doc-input の paths が引き続き DOC-MAP/README 経由で到達可能であることの確認 |

case-close は上記変更の有無を完了判定に組み込む。doc-inputs 更新要否の確認と `check_doc_inputs.ts` 実行要否を case-close profile で判定する。
