...
| foundations/patterns.md | accepted | 文書フォーマット規約 | frontmatter 規約、REQ/SPEC/guides の記述形式 |
| foundations/design-principles.md | accepted | 設計原則 | アーキテクチャ設計原則 |
| foundations/workflow-contracts.md | accepted | ワークフロー契約（旧版、縮小済み） | 個別動作は各 command/skill SPEC および workflows/ へ移管済み |
...
| responsibilities/req-impact-map.md | accepted | REQ 影響マップ | 各現行 REQ が影響する整合性ルールとアーティファクト（分類先: responsibilities/、親エージェント確認済み） |
...
| integrity/rule-ownership.md | accepted | ルール所有権マトリックス | ルールドメインと責任 REQ/SPEC の対応 |
