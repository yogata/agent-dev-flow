...
| integrity/targeted-docs-guard-implementation.md | accepted | Targeted Docs Guard 実装詳細 | check_changed_docs.ts 変更文書限定検査契約の実装計画（Phase 1-6）、report フィールド一覧（REQ-0158 から HOW を移送） |
...
| local/audit-ledger-lifecycle.md | superseded | 監査台帳ライフサイクル SPEC | one-time 監査成果物のライフサイクル（生成、参照、参照専用化、次フェーズ転記、廃棄）を一般化。現行契約は `workflows/backlog-artifact-lifecycle.md`「one-time 成果物ライフサイクル」セクションへ統合済み（REQ-0101-076、REQ-0101-077） |
