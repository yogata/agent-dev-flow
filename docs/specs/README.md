...
| [workflows/epic-wave-model.md](workflows/epic-wave-model.md) | Epic / Wave / Issue 実行モデル | OU 階層、子Issue 状態 enum、Wave スケジューリング、execution_unit 定義、連結成分アルゴリズム、3軸判断モデル、per-Epic 単一書き手 |
...
| [document-type-responsibilities.md](document-type-responsibilities.md) | 文書種別責務、配置基準 | 文書品質ゲート原本仕様（配置基準、用語政策）、REQ-0140 参照先、文書種別責務、要件行の書き方、要件性、粒度、移送判断、実行主体分類の査読基準 |
...
| [local-generation.md](local-generation.md) | ローカル版 OpenCode 生成 | link mode 接続フロー、link target 確認、unlink / relink による更新運用、ガードレール（REQ-0141, REQ-0134, ADR-0131） |
| [local-transform.md](local-transform.md) | ローカル版 OpenCode 変換プロンプト | 変換プロンプトの廃止根拠、変換品質検証の agentdev-gh-cli 差し替え版品質検証への読み替え（REQ-0141, ADR-0131） |
...
| [backticks-identifier-threshold.md](backticks-identifier-threshold.md) | backticks 識別子/一般名詞 判定閾値 | docs/** 配下の backticks 必須（識別子）と backticks 任意（一般名詞）の機械判定閾値。document-type-responsibilities.md 用語政策の補完 SPEC、mechanical-replacement-rules.md 相互参照先 |
