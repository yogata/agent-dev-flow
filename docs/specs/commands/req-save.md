...
- Step 4: REQ ファイル操作（`agentdev-req-file-manager`）（CREATE/APPEND/UPDATE、SPLIT候補、REQ再構成候補を処理）。決定的処理（REQ番号採番、要件行 ID 採番、frontmatter id↔ファイル名整合性確認）は `agentdev-req-file-manager/scripts/` の決定的スクリプト呼び出しで実行（REQ-0136-029、design-principles.md 第5節「決定的処理の Script 委譲原則」）
  - Step 4-0: QG-1（適用結果の整合性検証）: 採番結果の整合性（`new:{slug}` → 確定番号の置換漏れなし）、マージ結果の整合性（要件テーブル構造、番号重複なし）、インデックスの整合性（README/DOC-MAP/mapping-table エントリと採番結果の一致）、変更範囲の妥当性を検証。内容の品質（検証可能性、REQ/SPEC 分類適合性等）は req-define の QG-1 の責務であり、req-save は再検証しない（REQ-0102-081/082）
...
- QG-1（Definition Integrity Gate）: Step 4-0 で「適用結果の整合性検証」として実行（採番結果の整合性、マージ結果の整合性、インデックスの整合性、変更範囲の妥当性）。内容の品質は req-define の QG-1 の責務（REQ-0102-081/082）
