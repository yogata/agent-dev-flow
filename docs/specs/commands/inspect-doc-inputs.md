...
## 実行環境（ランナー）規約（REQ-0108-054）

本 command が呼び出す検査スクリプト `check_doc_inputs.ts`（`.opencode/skills/repo-agentdev-integrity/scripts/check_doc_inputs.ts`）の実行ランナーは **Bun** とする。

ランナーを Bun に固定する根拠:

- スクリプト本体が TypeScript 直接実行と `require()` 呼び出しを併用する構文（`require("path") as typeof import("path")`）を使用する
- 併設テスト（`check_doc_inputs.test.ts`）が `bun:test` フレームワークに依存する
- `node` 等の Bun 以外のランナーで直接起動すると `.ts` が ES module として解釈され `ReferenceError: require is not defined` が発生する

起動形式: `bun run .opencode/skills/repo-agentdev-integrity/scripts/check_doc_inputs.ts`。本規約は docs-check command 定義（`.opencode/commands/repo/docs-check.md`）と対で運用し、ランナー選択を利用者が推測しない状態を維持する。
