...
**昇華可能性評価、無条件自動REQ化禁止（REQ-0155-005）**: 各問題クラスについて恒久契約（REQ/ADR/SPEC）への昇華可能性を評価し、昇華可能な知見のみ `promoted/` へ出力する。無条件の自動REQ化は禁止し、学びは backlog-review → req-define → req-save の昇華経路を経て初めて REQ 化される。昇華不能な知見は `archive/active.md` の living pool で維持する。
