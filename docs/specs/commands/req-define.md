...
- Step 7: 要件doc生成（テンプレート: `templates/req-define/req-draft.md`）
 ...
 - Step 7-5: draft-data review_dispositions 生成（採否判断の記録、AG-007）
...
req-define は backlog-review の暫定分類（`tentative_classification`）を暫定入力とし、最終分類を自身で確定する（REQ-0102-087、AG-005、RU-20260722-02 合意、ADR-0139）。
...
REQ 影響なしと確定した変更からは `artifact_actions` の `artifact: req` エントリを生成しない（REQ-0102-088、AG-005）。代わりに `artifact: spec` エントリのみを生成し、SPEC への配置のみを行う。
