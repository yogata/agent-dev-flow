...
- **対象外**: 各 SPEC、REQ、ADR の本文（自動生成対象は索引類のみ）。`integrity-rule-catalog.md` の IR エントリ（候補1として別途検討）
...
1. **Phase A（F-001/002/005 即時修正）**: 既知の不整合を人手修正で解消する。本フェーズで実施済み（PR #1599）
2. **Phase B（要件 SPEC 化）**: 本 SPEC を新設し、機構の契約を確定する。本 SPEC 作成が相当
3. **Phase C（生成スクリプト実装）**: 自動生成スクリプトを実装し、主要な索引類から適用する
4. **Phase D（CI 組込）**: docs-check または CI に生成検証を組込む
