...
| baseline 形式 | `version`, `rule_id: "NG-BASELINE"`, `generated_at`, `entries[]`（各 entry は `category` / `check` / `file`（null 許容）/ `evidence`（null 許容）/ `count` を持つ）|
...
| 更新実行手順 | `bun run .opencode/skills/repo-agentdev-integrity/scripts/check_integrity.ts --update-ng-baseline`（`check_extensions.ts` も同様）を実行し、生成された baseline ファイルを commit する。更新後は `--json` 実行で新規 NG が 0 件になることを確認する |
| 更新非対象 | 当該変更に直接起因する新規 NG。これらは baseline 更新で隠蔽せず、必ず実装修復を行う |
