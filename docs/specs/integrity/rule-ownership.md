...
| 36 | obsolete-spec-path（旧SPEC直下パス参照検出） | REQ-0158 (002) | integrity-rule-catalog.md (IR-057) | docs/specs/ 基盤SPEC ドメイン別体系化（REQ-0156）以前の直下パス参照を検出。`obsolete-path-map.yaml` を対照表として IR-057 が検証。link mode 統一（ADR-0131）に伴う廃止語彙（`generation-flow.md`、`transform/`、`local-opencode-transform`、直接生成方式、生成フロー、再生成、上書き保護）の残留検出を含む |
