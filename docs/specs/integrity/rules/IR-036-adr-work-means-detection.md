...
**実装状態**: detection_method (1)（frontmatter `status` ベースの ADR フィルタリング）は `check_integrity.ts` の `checkAcceptedAdrOnlyCitation` 関数が実装する（REQ-0108-125）。detection_method (2)（作業手段キーワード検出）は未実装であり、regression_test 欄に示す検証項目とあわせて後続課題とする。
