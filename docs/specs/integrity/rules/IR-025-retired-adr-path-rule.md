...
|-------|-------|
| rule_id | IR-025 |
| description | 旧番号帯 ADR（v2:ADR-0001〜0099）が `docs/adr/retired/` に配置されていること。`docs/adr/` 直下に旧番号帯 ADR が残存していないこと（REQ-001-047, 048） |
| severity | strict |
| category | obsolete-structure |
| detection_method | `docs/adr/ADR-0*.md`（v2:ADR-0000〜v2:ADR-0099）の存在確認 |
...
`docs/adr/ADR-0*.md`（v2:ADR-0000〜v2:ADR-0099、旧番号帯）の存在確認。

**適用範囲**: ADR-00XX（旧番号帯、REQ-001-048 に基づく retired/ 移動対象）のみ。
