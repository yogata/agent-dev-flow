...
regression_test フィールドの表記は以下の方針に従う（SPEC 責務境界: 将来予定の混入を避けるため、「(追加予定)」は使用しない）。

| 表記 | 使用場面 |
|------|----------|
| `(未実装)` | 回帰テストが未実装の場合。検証内容の概要を後ろに併記する（例: `(未実装)。X の条件で検出されることを検証する`） |
| テストID または概要 | 回帰テストが実装済みの場合。テストファイル名、テストケースID、または検証内容の概要を記述する |

「(追加予定)」は SPEC 責務境界（REQ-0101-060: docs/specs/* に将来予定を混入させない）に抵触するため使用しない。未実装の回帰テストは「(未実装)」で表現する。

## ルールインデックス

各ルールの15フィールド詳細は [rules/](rules/) サブディレクトリの個別ファイル `IR-NNN-{slug}.md` を参照（REQ-0155-007 局所物理分離、REQ-0156-008）。catalog はスキーマ定義とインデックスを維持する。
