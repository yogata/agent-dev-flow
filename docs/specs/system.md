...
tips-*コマンドは3つのコマンド + 2つの補助スキルで構成され、学びの3層パイプライン（キャプチャ→分析→昇華）を提供する。基本方針は「入口を緩く、出口を厳しく」。
...
| `/tips/tips-refactor` | 問題クラス分類→8軸評価→archive移動。Input/Output/Steps中心の薄いコマンド | 分析層 | evaluation-report.md生成、refactor時prune（任意）、`tips-pipeline-orchestration` skill参照 |
| `/tips/tips-elevate` | 昇華判定→Requirement Source staging stub生成。直接反映禁止 | 昇華層 | 11処分区分+duplicate、既存対策照合、elevate時prune（必須）、`tips-pipeline-orchestration` skill参照、issue-reqへの明示入力ファイル案内付き |
| `tips-pipeline-orchestration`（スキル） | schema・評価・stagingの共通知識ベース | 共通 | inbox entry schema、問題クラス分類、8軸評価、処分区分、Requirement Source staging stub schema、prune方針、反映先振り分け |

**データフロー**: `inbox.md` →（tips-refactor）→ `archive.md` + `evaluation-report.md` →（tips-elevate）→ `elevation-staging/`（Requirement Source形式）
