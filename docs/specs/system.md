...
docs-check は `/repo/*` namespace の repo-local command である（ADR-0106、REQ-0115-001）。AgentDevFlow の public command ではなく、self-hosting repo の自己監査コマンドである。

| コマンド | 役割 | エージェント |
|---|---|---|
| `/repo/docs-check` | agent-dev-flow repo の自己監査（repo-local、consumer 配布対象外） | sisyphus |

#### docs-review ワークフロー

| コマンド | 役割 | エージェント |
|---|---|---|
| `/agentdev/docs-review` | docs 全体の意味整合レビューと REQ 再構成診断（REQ-0115） | prometheus |
...
`agentdev-epic-tracker`: 親Epic Issueのステータス追跡テーブル（`| # | Issue | ステータス | 内容 |` 4列形式）を更新する知識ベース。ステータス値は `☐ 未着手` / `🔄 進行中` / `✅ 完了 (PR#N: URL)` / `❌ 対処不要` / `⏭ スキップ` の5値（REQ-0106-030）。`case-run` Phase A で `🔄 進行中` に更新し、`case-close` Step 8 で `✅ 完了 (PR#N: URL)` に更新する。`❌ 対処不要` は手動設定のみの終了状態。`⏭ スキップ` は前提条件未達等で Orchestrator が設定する終了状態（手動設定不可）。Epic自動クローズ判定では `❌ 対処不要` と `⏭ スキップ` を `✅ 完了` と同等の終了状態として扱う。多重Issueモードでは親エージェントがWave開始前に一括更新する。
