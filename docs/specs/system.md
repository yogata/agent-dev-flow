...
| `/agentdev/learning-refine` | 問題クラス分類→8軸評価→archive移動。Input/Output/Steps中心の薄いコマンド | 分析層 | evaluation-report.md生成、refactor時prune（任意）、`agentdev-learning-pipeline` skill参照 |
| `/agentdev/learning-promote` | 昇華判定→Requirement Source staging stub生成。直接反映禁止 | 昇華層 | 11処分区分+duplicate、既存対策照合、elevate時prune（必須）、`agentdev-learning-pipeline` skill参照、req-defineへの明示入力ファイル案内付き |
