...
integrity-check は `/repo/*` namespace に移行した（ADR-0106）。AgentDevFlow の public command ではなく、self-hosting repo の自己監査コマンドである。
