...
`case-close` Step 2 でIssue本文の未チェック項目に対して達成判定を行う。即時停止ではなく、5条件プロトコル（完了状態特定・証拠存在・直接対応・反証なし・理由記録可能）に基づき根拠付きで判定する。達成→`[x]`更新、未達→`[ ]`維持。Issue本文更新後に再取得・再検証し、未達項目残存時は構造化エラーで停止する。全項目達成時のみPR merge/Issue closeに進む。
