...
**データフロー**: `inbox.md` + `archive/active.md` →（learning-promote: internal normalize → classify → eval → judge → HITL → execute）→ `promoted/`（フラット構造）→（backlog-review → backlog-save）→ `.agentdev/backlog/req-units/RU-*.md`
