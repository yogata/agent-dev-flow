...
**データフロー**: `inbox.md` →（learning-refine）→ `archive.md` + `evaluation-report.md` →（learning-promote）→ `promoted/`（フラット構造）→（req-backlog）→ `.agentdev/backlog/req-units/RU-*.md`
...
- `archive.md`: living pool（終端保管ではない。`archive` は終端保管を意味しない）。未処分・保留中・再評価対象の entry を保持し、promote の入力として参照される。prune・promote時pruneで動的に変化
- `evaluation-report.md`: refine/promote 間の境界 artifact（毎回上書き、長期履歴ではない）
- `promoted/`: 昇華判定済み artifact の配置先（フラット構造）。req-backlog が読み込み、RU 生成成功後に削除。`.opencode/` や実装コードへの直接反映禁止
...
**自律解決判定**: 5条件プロトコルで達成不可の項目について、PR diffの変更対象分類×チェックボックス要求検証種別分類に基づき自律解決可否を判定する。検証適用不能（変更対象に対して要求検証が直接証拠にならない）または代替検証実施（代替手段で直接証拠を得られる）の場合→`[x]`更新（適用不能理由を記録）。自律解決不能→`[ ]`維持。Issue本文更新後に再取得・再検証し、未達項目残存時は構造化エラーで停止する。全項目達成時のみPR merge/Issue closeに進む（REQ-0032-015〜022）。

**証拠ソース**: PR本文、PR差分、コミット内容、CI結果、Issueコメント、PRコメント、実装済みファイル内容。

**責務境界**: `case-run` のチェックボックス更新責務は維持される。`case-close` は最終セーフティネットとして動作する（REQ-0032-013, 014）。
...
旧REQ群（REQ-0001〜0040）を現行仕様基準として再構成した新基準REQ群（REQ-0042〜0049）を主参照とする。旧REQは履歴として保持し、分類（retained / partially superseded / superseded）に基づき扱う。

**新基準REQ群構成**（REQ-0041 に基づく）:

| REQ | 領域 |
|-----|------|
| REQ-0042 | REQ/ADR/SPEC/DOC-MAP基準構造 |
| REQ-0043 | req-define / req-save / REQ分類ゲート |
| REQ-0044 | Command/Skill/Template/Script責任分界 |
| REQ-0045 | AgentDevFlow command protocol |
| REQ-0046 | intake/learning/req-backlog/RU lifecycle |
| REQ-0047 | case-run/case-close/post-run capture |
| REQ-0048 | reporting/GitHub body/link/writing quality |
...
`req-define` / `req-save` に反映作業混入防止のゲートを設ける（REQ-0041-013〜016）。
