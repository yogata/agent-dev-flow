...
- `elevation-staging/`: Requirement Source stub の staging 領域（背景/問題/望ましい変更/対象範囲/反映先候補テーブル/既存対策確認/制約/完了条件/元learning item/推奨Issue分類）。`.opencode/` や実装コードへの直接反映禁止。req-define 明示入力ファイル経由で実装に移行
...
**Staging Stub Archive**: case-close で取り込み済み staging stub を `elevation-staging/archive/` に移動する。imported 判定は機械的4条件（Issueにstubパス記録 + PRマージ済み + Issue completed close + case-close正常完了）で行い、意味的一致検証はしない。移動前に stub 本文に `## 取り込み記録` セクション（Issue #, PR #, 取り込み日, 処理）を追記する。同名ファイル既存時は上書きしない。不採用 stub は対象外。`agentdev-learning-pipeline` skill に archive ルールを定義し、case-close が同 skill を load して実行する
...
| `/agentdev/intake-promote` | accepted item を req-define / intake-open 用 artifact に整形し、promoted/{route}/ に保存。accepted item は archive/promoted に移動。frontmatter は持たない | sisyphus |
| `/agentdev/intake-open` | promoted/intake-open/ の artifact から GitHub Issue を作成。成功時 artifact を削除 | sisyphus |
