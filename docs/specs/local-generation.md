...
GitHub Issue / PR を使わない個人利用環境（ローカル版 OpenCode）向けに、AgentDevFlow のコマンド / スキル / ひな形を生成先リポジトリの `.opencode/` に直接生成する方式を定義する（REQ-0141-001〜033）。
原本である `src/opencode/` を改変せず、識別子と安全ゲートで原本保護を担保する。
...
`src/opencode-local/` はローカル版生成のためのスキーマ、変換プロンプト、変換仕様のみを保持する生成時ソース領域である（REQ-0141-004, 005）。
...
AI エージェントが変換プロンプトに従って生成する（REQ-0141-032）。
変換スクリプトはリポジトリ管理対象外とする（REQ-0141-009）。
...
ローカル版生成の実行手順は `src/opencode-local/README.md` に記載する（REQ-0141-031）。
手順は OpenCode 等 AI エージェントで `transform/generate.md`（変換用プロンプト）を入力またはファイル参照して実行する内容とする。
