...
> **Scope**: 本 SPEC は agent-dev-flow リポジトリのリポジトリ内部設計文書である（ADR-0103）。ローカル版 OpenCode の生成フロー・安全ゲート・`generated_by` 識別子・ひな形変換方針・更新運用を定義する。実行時配布対象ではなく、実行時コマンドは本ファイルに依存しない（ADR-0104）。REQ-0141 と ADR-0126 の詳細仕様を正とする。
