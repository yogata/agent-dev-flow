...
### SKILL.md 原本節フォーマット（DERIVE 機構）

SKILL.md は frontmatter 直後に「原本（SSoT）」節を配置する。原本節は以下の3要素を含む単一段落とする。

1. **SPEC 参照リンク**: 対応する SPEC（`docs/specs/skills/agentdev-*.md`）への相対リンク
2. **DERIVE 宣言**: 本 SKILL.md は実行入口であり、SPEC を SSoT として DERIVE する宣言。機能節の記述は SPEC と整合し、SKILL.md 固有の運用ビュー、参照資料、トリガーを補完する旨。SPEC と重複する場合 SPEC を正とする旨
3. **U-012 解消宣言**: extension（`.agentdev/extensions/skills/`）は標準 SKILL.md を前提とし、SKILL.md と重複しない補完情報のみを提供する旨

原本節に内部 ID（`REQ-XXXX`、`ADR-XXXX`、`IR-XX` 等）を含めない。配布物である SKILL.md への内部 ID 記述は IR-055 strict 違反（runtime-unresolved-reference）の原因となる。原本節と機能節双方で SPEC の内容を再記述しない（前節「禁止パターン」と同一原則）。

### U-012 解消パターン（extension と SKILL.md の関係）

U-012（extension と SKILL.md の記載重複）は以下の関係性を定めることで解消する。

- **原本**: 標準 SKILL.md（`src/opencode/skills/agentdev-*/SKILL.md`）。原本仕様は対応する SPEC（`docs/specs/skills/agentdev-*.md`）
- **補完**: extension（`.agentdev/extensions/skills/agentdev-*/`）。標準 SKILL.md を前提とし、SKILL.md と重複しない補完情報（プロジェクト固有パス、プロジェクト固有運用ビュー等）のみを提供する

同じ内容を SKILL.md と extension 双方へ記載しない。extension 側には「SKILL.md に既出の内容」をコピーせず、SKILL.md が前提とする固定知識の範囲外に限定した補完情報のみを記載する。本関係は前項「SKILL.md 原本節フォーマット」の U-012 解消宣言によって SKILL.md 側からも宣言する。
...
- 優先度高位（重複度合い高位 かつ 影響度高位）
- 優先度中位（重複度合い中位 または 影響度中位）
- 優先度低位（上記以外）
...
#### 優先度高位（agentdev-doc-writing は查読済）
