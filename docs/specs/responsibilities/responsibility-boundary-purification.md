...
> **用語注記**: 「実行担当サブエージェント内部の実行制御」は、推論、context 管理、retry、エラー解析等を含む上位概念として扱う。line 38 限定注記内の表記と一致させる（語彙揺れ是正、intake-2026-07-19-responsibility-boundary-purification-vocab-drift-inline-control.md）。
