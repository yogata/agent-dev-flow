...
> **注意**: 本プロトコルは case-run のハーネス起動仕様である（ADR-0114 延長・ADR-0128 による task() 委譲モデル）。case-run は AGENTS.md で指定されたハーネス（推奨: oh-my-openagent）を task() で起動する。軽量な一般委譲は前述の「汎用サブエージェント委譲契約」を参照。subagent-protocol.md（Skill reference）に編集安全手順・大規模ファイル分割委譲・AST-grep 運用を定める。
...
case-run は AGENTS.md で指定されたハーネス（推奨: oh-my-openagent）を task() 委譲で起動する。CLI subprocess（bunx oh-my-openagent run）は ADR-0128 により廃止済み。起動方式の詳細は `agentdev-case-run-execution-adapter` スキルの `references/oh-my-openagent.md` 参照。ハーネス不在時は case-run がエラー停止する（フォールバック経路なし）。

### ハーネス起動プロンプト構成

task() 委譲時に渡すプロンプト/入力に含める項目:
...
- 全 case-run 完了待機（各 case-run はハーネスを task() で起動）
- specs直列更新
