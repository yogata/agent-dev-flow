...
> **注意**: 本プロトコルは case-run のサブエージェント起動仕様である（ADR-0128）。case-run は `task(subagent_type="Sisyphus-Junior", load_skills=["ulw-loop"], ...)` で実行を委譲し、CLI subprocess は使用しない。軽量な一般委譲は前述の「汎用サブエージェント委譲契約」を参照。subagent-protocol.md（Skill reference）に編集安全手順・大規模ファイル分割委譲・AST-grep 運用を定める。

### 単一Issue実行モデル

case-run は単一 Issue の実装を Sisyphus-Junior(ulw-loop) に委譲する。

```
task(subagent_type="Sisyphus-Junior", load_skills=["ulw-loop"], prompt="/ulw-loop Implement Issue #N: <Issue本文>")
```

各ツール呼び出しには 120 秒 timeout が適用される。ハーネス不在時のフォールバック経路はない（ADR-0128）。

### Epic Wave 実行モデル

`case-run #epic` 受領時の Wave 実行:

1. Epic Issue 本文から現在 ready Wave の子Issue を特定
2. 各子Issue を `task()` で Sisyphus-Junior(ulw-loop) に並列委譲（最大5件）
3. 全 task() 完了待機
4. 結果収集 → return

制約: 1 Wave のみ実行する。マージはしない。次 Wave には進まない。

### Epic Wave クローズモデル

`case-close #epic` 受領時の Wave クローズ:

1. Epic Issue 本文から現在 Wave の PR作成済み子Issue を特定
2. PR マージ・子Issue クローズ
3. 最終 Wave で Epic クローズ
4. それ以外は残 Wave 通知

### サブエージェント起動プロンプト構成

`task()` で渡すプロンプト/入力に含める項目:
...
- 全 case-run 完了待機（各 case-run は `task()` で並列委譲）
- specs直列更新
