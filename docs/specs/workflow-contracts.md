...
| プロトコル | case-run 専用サブエージェントプロトコル（後述） | 本汎用サブエージェント委譲契約 |
...
> **注記**: 本セクションの実行メカニズム（親エージェント subagent 起動・specs 直列更新・Wave 間 rebase）は、case-run 委譲モデル（ADR-0114）および子Issue単位オーケストレーションモデル（SC-007）に置き換えられた。Wave 内並列実行は SC-007（改訂版）および ADR-0125 に基づき case-auto が case-run を並行委譲する形で維持される。本セクションは Wave 失敗時後続制御の概念参照として残置する。

scale: large の場合、Epic 統率者（Orchestrator）モードで実行する:

- Epic Issue 本文の `## 実行順序` セクションを SSoT とする
- Wave 単位で子 Issue を直列または並列実行
- 親エージェントは統率（orchestration）のみ担当し、実装詳細を抱え込まない
...
| 条件 | 結果 |
|---|---|
| Issue に `epic` ラベルが付与されている | Epic 統率者モード |
| Issue 本文に `## 実行順序` セクションがあり、Wave 列を持つ Markdown テーブルが存在する | Epic 統率者モード |
...
> **注意**: 本プロトコルは case-run のハーネス起動仕様である（ADR-0114 延長・CLI subprocess モデル）。case-run は AGENTS.md で指定されたハーネス（推奨: oh-my-openagent）を CLI subprocess で起動し、`task(subagent_type=...)` は使用しない。軽量な一般委譲は前述の「汎用サブエージェント委譲契約」を参照。subagent-protocol.md（Skill reference）に編集安全手順・大規模ファイル分割委譲・AST-grep 運用を定める。
...
→ **CLI subprocess モデルに移行**（ADR-0114 延長・oh-my-openagent 明示前提）:
...
case-run は AGENTS.md で指定されたハーネス（推奨: oh-my-openagent）を CLI subprocess として起動する。`task(subagent_type=...)` は重複オーケストレーション問題（oh-my-openagent issue #4027）により使用しない。起動方式の詳細・タイムアウト（1時間）・PR URL フォールバック（`gh pr list --head <branch>`）は `agentdev-case-run-execution-adapter` スキルの `references/oh-my-openagent.md` 参照。ハーネス不在時は case-run がエラー停止する（フォールバック経路なし）。

### ハーネス起動プロンプト構成

ハーネス起動（CLI subprocess）時に渡すプロンプト/入力に含める項目:
...
### 親エージェント責務（case-auto / case-run）

- Wave開始前のEpicステータス一括更新
- 全 case-run 完了待機（各 case-run はハーネスを CLI subprocess で起動）
- specs直列更新
- Findings / Capture候補件数の集約

### フォールバック

ハーネス不在時は case-run がエラー停止し、Sequential Wave 等のフォールバック経路は持たない（AGENTS.md で指定されたハーネスが前提）。Wave 内の並列/直列は技術的依存関係に基づき case-open が決定する。

## 子Issue単位オーケストレーションモデル

> 本セクションは case-auto の薄いオーケストレーター化（REQ-0114-045〜048, 073〜075）および case-open の自律構成生成（REQ-0132-007〜010）に基づく。従来の Epic 統率者契約（親エージェント subagent 一括起動）は case-run 委譲モデルに置き換えられた。Wave 内並列実行は case-auto が case-run を並行委譲する形で維持される（ADR-0125）。
