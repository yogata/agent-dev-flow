...
> **注意**: 本プロトコルは case-run の manager-orchestrator パターンにおけるサブエージェント起動仕様である。軽量な一般委譲は前述の「汎用サブエージェント委譲契約」を参照。subagent-protocol.md（Skill reference）に編集安全手順・大規模ファイル分割委譲・AST-grep 運用を定める。
