...
`src/opencode-local/` はローカル版固有の原本のみを保持する（REQ-0141-004）。
...
`src/opencode-local/transform/` と `src/opencode-local/generation-flow.md` は link mode では不要のため確定廃止とする（ADR-0131 decision #4、PR#1195 で完全削除済み、REQ-0141-004/009/028 確定廃止昇格）。
...
link 設定前に `.opencode/` 配下の各 path が意図した link target へ解決されることを確認する（REQ-0141-010, ADR-0131 decision #6）。
...
link mode 全体を一律停止するのではなく、link target が意図したものであれば許可する。
...
本節が残存 GitHub 固有参照の違反判定基準の正本である（REQ-0141-029）。`local-transform.md` から一元化し、同ファイルの削除後も本節が唯一の正本となる。
...
## 変換プロンプト廃止経緯（local-transform.md から一元化）

link mode 導入に伴い、`src/opencode-local/transform/` 配下の変換プロンプト要件を廃止した（ADR-0131 decision #4, REQ-0141-028）。本節は旧 `local-transform.md` が保持していた廃止経緯と変換品質検証の読み替えを集約し、同ファイル削除後も本 SPEC が正本となる。

### 変換プロンプトの廃止

以下のファイルは link mode では不要となるため廃止済みである（ADR-0131 decision #4、PR#1195 で完全削除済み、REQ-0141-028 確定廃止昇格）。
...
link mode では、`agentdev-gh-cli` の link 先だけを差し替えることでローカル版が構成できる（ADR-0131 decision #3）。そのため、command / skill 全体を変換するプロンプトは存在意義を失う。

### 変換品質検証の読み替え

変換プロンプトが担っていた品質検証要件は、`agentdev-gh-cli` 差し替え版の品質検証へ読み替える（REQ-0141-029）。
...
ローカル版 `agentdev-gh-cli` の品質検証の契約一致の検証観点は [agentdev-gh-cli SPEC](../skills/agentdev-gh-cli.md) の差し替え可能性セクション参照。

## ガードレール一覧

ローカル版 link mode 導入が遵守するガードレールを定義する（REQ-0141-014, ADR-0131）。
...
- link target が意図した target 以外へ解決される場合は link 設定を停止すること（REQ-0141-010）
...
- [ワークフロー契約](../workflows/workflow-contracts.md)（Local backend 差分契約）
...
> **local-transform.md 統合**: 変換プロンプト廃止経緯、変換品質検証の読み替え、残存 GitHub 固有参照の違反判定基準を旧 `local-transform.md` から本ファイルへ一元化した（REQ-0141-028/029、ADR-0131）。`local-transform.md` は case-run のファイル操作により削除済み。
