...
link mode 導入に伴い、`src/opencode-local/transform/` 配下の変換プロンプト要件を廃止した（AG-011, ADR-0131 decision #4, REQ-0141-028）。本節は旧 `local-transform.md` が保持していた廃止経緯と変換品質検証の読み替えを集約し、同ファイル削除後も本 SPEC が正本となる。

### 変換プロンプトの廃止

以下のファイルは link mode では不要となるため廃止済みである（AG-011, ADR-0131 decision #4、PR#1195 で完全削除済み、REQ-0141-028 確定廃止昇格）。
...
変換プロンプトが担っていた品質検証要件は、`agentdev-gh-cli` 差し替え版の品質検証へ読み替える（AG-011, REQ-0141-029）。
