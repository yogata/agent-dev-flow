...
| Guides | 人間向けナビゲーション層。規範的権限を持たない（ADR-0103） | MUST/SHALL 規範表現、REQ/ADR/SPEC 内容の重複 |
...
- **規範表現**: Report は SHALL/MUST 規範表現を使用しない。事実の記述と分析結果の提示に留める
...
### SPEC SHALL/MUST Limitation <!-- REQ-0116-011 -->

SPEC 内の SHALL/MUST 使用を制限する。

- **SPEC の SHALL/MUST は「現在有効な契約記述」に限定する**
- 将来要件は REQ に記述する
- 判断根拠は ADR に記述する
- SPEC は現在システムがどう動作しているか（HOW it works now）を記述し、どう動作すべきか（WHAT it SHOULD do）を記述しない
...
| Guide に規範的表現（SHALL/MUST）が含まれている | REQ/ADR 参照 | 規範内容を REQ/ADR への参照に置き換える |
...
|---|---|---|---|
| 責務境界（SPEC に要件混入） | SPEC-REQ-mix 検査 | 意味整合性レビュー | SPEC 内の SHALL/MUST が現在仕様の記述か判定 |
| 責務境界（Guide の規範表現） | Guide-normative 検査 | 規範表現検出 | Guide 内の SHALL/MUST を検出 |
...
| Report 規範表現 | Report-normative 検査 | SHALL/MUST 混入検出 | Report 内の規範表現を検出 |
