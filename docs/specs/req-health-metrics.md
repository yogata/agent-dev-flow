...
安定契約例外（REQ-0101-069、`document-model.md`「安定契約の例外」）に該当する要件行は、SPEC分離基準違反の検出対象外とする。
...
- **`document-model.md` SPEC 分離基準（REQ-0101-068）**: SPEC分離基準違反シグナルの判定基準。本 SPEC は閾値とシグナル加算のみを定義し、SPEC分離の判定本体は `document-model.md` に従う
- **`document-model.md` 安定契約の例外（REQ-0101-069）**: 安定契約例外の定義。本 SPEC の SPEC分離基準違反検出はこの例外を尊重する
