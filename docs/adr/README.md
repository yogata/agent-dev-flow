...
ADR-01XX（accepted/proposed）の12件が、現在のアーキテクチャ判断の基盤である。各 ADR は baseline 再編により旧 ADR の内容を統合・再定義している。
...
| ADR-0112 | サブエージェント委譲の一般化と manager-orchestrator の位置づけ | proposed | 2026-06-10 |
...
- [ADR-0112](ADR-0112.md) — サブエージェント委譲の一般化と manager-orchestrator の位置づけ
...
| ADR-0112 | relates-to | ADR-0111 | 本ADRの特殊型として維持 |
...
| ADR-0112 | — | 対応REQなし（architecture principleとして定義） |
