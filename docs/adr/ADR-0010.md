...
全 agentdev command に HITL boundary 基準を適用する。対象は workflow command（case-open, case-run, case-update, case-close）に限らず、intake（intake-capture, intake-from-github, intake-promote）、learning（learning-promote）、backlog（backlog-review, backlog-save）、integrity（integrity-check）、requirement（req-define, req-save, req-restructure-review）を含む。
