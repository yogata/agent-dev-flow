...
| REQ-0106-017 | `case-close` は pull 前ローカル変更の自動リセット対象を「PRで削除されたファイル」に限定する旧規則を廃止すること |
...
| REQ-0106-027 | `agentdev-workflow-orchestration` スキルの Wave scheduling 手順にファイル変更範囲の非重複チェックを追加すること (FPR-030) |
| REQ-0106-028 | `agentdev-workflow-orchestration/references/self-healing-and-errors.md` にマージコンフリクト予防のガイダンスを追加すること (FPR-031) |
| REQ-0106-029 | SKILL.md は500行以下を維持すること。詳細は references/ に配置 (FPR-032) |
