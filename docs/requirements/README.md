...
現在の要件判断では、以下21件を第一参照先とする。旧REQ 50件はすべて retired であり、履歴参照に限定する。
...
| [REQ-0103](REQ-0103.md) | Artifact責任分界 | command / skill / template / script / namespace / frontmatter 規約 / runtime-only 配布制約 / source-projection分離 / sync・migration / namespace予約 / SSOT化 / registry化 / consumer導入モデル / repo-local namespace・配布対象外制約 / .agentdev domain state / sync除外 / consumer plugin checkout・install script分離 |
...
| [REQ-0111](REQ-0111.md) | Command authoring 後方互換性維持原則 | command-authoring、後方互換性、設計原則。REQ-0119-025 により retire 予定 |
...
| [REQ-0121](REQ-0121.md) | Runtime Command RFC 2119 見直し + Integrity 検査再定義 | command / token 最適化 / RFC 2119 規範語 / 自然文置換 / integrity 検査再定義 / 語彙ポリシー整合 |
| [REQ-0120](REQ-0120.md) | Runtime Command 非必須参照除去 | command / token最適化 / 非必須参照節 / 実行骨格 / command-authoring |
| [REQ-0121](REQ-0121.md) | Runtime Command RFC 2119 見直し + Integrity 検査再定義 | command / token最適化 / RFC2119 / integrity / 語彙ポリシー |
