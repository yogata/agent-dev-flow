...
| REQ-050-009 | 単体実行を前提としない内部処理を scripts/ 直下に配置しないこと。consumer 専用の内部処理は scripts/consumer/ 配下に、self-hosting 固有の配布・検証・保守処理および self-hosting 環境向けの実行環境ブリッジ道具は scripts/self/ 配下に配置すること |
