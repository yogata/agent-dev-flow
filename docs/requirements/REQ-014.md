...
各 command の呼出統合は REQ-015、横断整合の恒常契約も REQ-015 が所有する（横断整合の完了時点検証は廃止済み REQ-016 が記録、2026-09-20 RETIRE）。
adversarial-review 自身の振る舞い契約（入力スキーマ、Reviewer/Reviewee protocol、収束判定）は
REQ-082-006〜040 および adversarial-review Design が既に所有する。
...
| REQ-014-003 | 共通 caller integration 契約（入力コンテキスト、返却契約）の正規所有者は adversarial-review Design であること。caller integration 用の新規永続 schema を作成しないこと |
...
| REQ-014-011 | 正規所有者マトリックス（adversarial-review Design / delegation-contracts / 各 command Design / domain skill Design / v4-lifecycle-state-machine + case-auto Design）を採用し、一つの意味を一つの Design だけが規範定義し、重複規範・矛盾を生じないこと |
...
| REQ-014-015 | 成立条件の導出、指摘の重要性判定、本質的争点への整理、収束判定のロジックは adversarial-review 側（REQ-003、adversarial-review Design）が単一所有し、呼出元（req-define、inspect-promote、intake-promote、learning-promote、backlog-review、case-open、case-run、case-auto）へ重複定義しないこと。呼出元は対論型レビューの結果を受領し、既存契約に従って採用済み指摘の反映、未解決事項の伝播、後続処理判断を行うこと |
...
- **対象外**:
  ...
  - 横断整合の恒常契約（REQ-015。完了時点検証は廃止済み REQ-016 として記録）
  - adversarial-review 自身の振る舞い契約（入力スキーマ、Reviewer/Reviewee protocol、収束判定）（REQ-082-006〜040、adversarial-review Design）
  - 共通契約の詳細パラメータ、入力フィールド構成、enum 値（adversarial-review Design、v4-lifecycle-state-machine Design、delegation-contracts Design）
