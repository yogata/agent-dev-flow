...
分割記録: REQ-006-001〜021 は case-open 実行契約 REQ（REQ-030）へ、REQ-006-022〜038 は case-run 実行契約 REQ（REQ-031）へ、REQ-006-039〜059 は case-close 実行契約 REQ（REQ-032）へ、REQ-006-060〜064 は case-update 実行契約 REQ（REQ-033）へ、REQ-006-065〜094・110・112〜114 は case-auto 実行契約 REQ（REQ-034）へ、REQ-006-095〜104 は Epic と Wave 実行モデル REQ（REQ-035）へ移動した（2026-08-14、要件行 ID は移動先で再採番）。
...
| REQ-006-111 | req-save command のガードレール G12 は deviation capture が Skill（agentdev-learning-capture または agentdev-intake-pipeline）への委譲である旨を明示すること。責務境界は変更せず、SPEC 副作用セクション（capture-boundaries.md）と整合すること |
