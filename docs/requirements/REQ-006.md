...
case-open は case-open 実行契約 REQ、case-run は case-run 実行契約 REQ、case-close は case-close 実行契約 REQ、case-ready は case-ready 実行契約 REQ、case-revise は case-revise 実行契約 REQ、case-auto は case-auto 実行契約 REQ が所有し、Epic Issue を実行順序の SSoT とする実行モデルは Epic と Wave 実行モデル REQ が所有する。これらの実行契約 REQ が定義する各工程は、いずれも内部 lifecycle 段階である（DEC-033）。
...
履歴文脈: 本 REQ がかつて所有していた各 command 実行契約に由来する要件行群は case-open、case-run、case-close、case-auto の各実行契約 REQ と Epic と Wave 実行モデル REQ へ移管済みであり、旧要件行番号への参照は現行文書では使用しない（移管先の置換連鎖は REQ-034-031〜034 へ付け替え済み）。
...
| REQ-006-109 | 各工程 command の deviation capture は、agentdev-learning-capture skill または agentdev-intake-pipeline への委譲で行い、別 command を直接呼ばないこと。完了報告には、保存した capture 成果物のパス・分類・保存結果のみを含めること |
...
| REQ-006-112 | role: case の Root Case は open、ready、running、blocked、review、closed、cancelled の状態を持つこと。open は Root Case 確立済み・Definition / execution contract 未確定・実行不可、ready は canonical Definition と execution contract が確定し実行可能、running は実行中、blocked は継続条件不足、review は実装完了・最終受入対象、closed は完了、cancelled は中止を意味すること。closed と cancelled は終端状態とすること |
| REQ-006-113 | case-run は ready からのみ実装を開始できること。blocked の発生元ごとに状態値を増やさないこと |
| REQ-006-114 | blocked の場合のみ Root Case に resume_command を保持し、停止解除後の正規再開入口を永続化すること。resume_command は内部 lifecycle の再入口分類値（req-define、case-revise、case-ready、case-run、case-close）とし、ユーザーが実行する再開手段は req-define または case-auto への Root Case 指定に集約すること。resume_command は通常状態への遷移時にクリアすること。新しい意味判断が必要な場合は req-define を指し、再合意済みで case-revise 段階の再開が必要な場合は case-revise を指せること |
