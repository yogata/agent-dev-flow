...
v4 において外部連携境界は Harness/Backend adapter 境界（ADF v4 実装責務境界 Design・DEC-036）に接続する。
...
| REQ-011-009 | 抽象 backend を先回りして新設せず、GitHub 前提の標準実装と、必要になった時点での adapter 別追加により差し替え可能性を担保する。 |
...
| REQ-011-018 | harness execution mechanism（agent 起動、background task、並列実行、context 管理）は ADF の規範所有対象外とし、Harness/Backend adapter 境界の責務とすること（OpenCode を first-class reference harness とする）。 |
...
| REQ-011-033 | agentdev_gh の issue_list は search 指定時に GitHub サーバ側絞り込みクエリへ search を推送して実効させること（search/issues エンドポイント・in:title 限定）。search 未指定時は従来どおり完全走査とし、完全一覧・安全上限の意味は変更しないこと。Tool 公開 description の search 意味契約はサーバ側絞り込みに合わせて更新し、tokenized 照合と substring 照合の差は Design が物理写像差異として宣言すること。 |
