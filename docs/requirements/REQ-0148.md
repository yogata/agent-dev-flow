...
| REQ-0148-018 | case-auto レベルでのグローバル並列上限は設定せず、case-run 単位の並列上限（REQ-0130-026、SPEC `epic-wave-model.md` 参照）のみを制御対象とすること（REQ-0101-069 安定契約例外: 安全境界。並列実行数上限は実行安全境界）。Phase 分離モデル（REQ-0114-102）では Phase 2 の同時起動数に固定値5を適用すること。本要件が言及する「5件」は case-run Wave 内子 Issue 並列上限（REQ-0130-026）と Phase 2 同時起動数（REQ-0114-106）の2文脈であり、execution_unit 全体並列上限は設定しない |
...
| REQ-0148-022 | ADR作成者は新規ADR「複数 execution_unit 並列実行モデル」を作成し、ADR-0109、ADR-0125 に relates-to で補完すること |
| REQ-0148-023 | 必須依存があっても事前契約、依存先Epic完了、Epicサイズ推奨範囲調整のいずれかを満たす場合は別 Epic 分割を許容すること |
| REQ-0148-024 | case-close は並列実行時に PR マージコンフリクトが発生した場合、後続 PR を rebase により解決すること |
...
- **対象**:
 ...
 - case-close（rebase によるコンフリクト解消責務、REQ-0151-001/002、REQ-0131-024/025 参照）
- **対象外**:
 - case-run（変更最小、現行踏襲）
 - 優先順位制御
 - クリティカルパス最適化
...
> **対象外「case-close（ほぼ変更不要）」撤回ノート（REQ-0151-007）**
>
> REQ-0148 初版は case-close を「ほぼ変更不要」として対象外としたが、本判断を撤回する。
> case-close に rebase によるコンフリクト解消責務を追加する（REQ-0151-001/002、REQ-0131-024/025 参照）。
> ADR-0129 が「マージコンフリクト解決コストを受容」と決定した内容を実装する解決メカニズム（ADR-0132 Level 1）として、case-close の変更は必須である。
> 機械的競合（rebase で自動解決可能）は case-close が解消し、解消不能時は case-auto へエスカレーションする。
