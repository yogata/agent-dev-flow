...
>
> REQ-0148 初版は case-close を「ほぼ変更不要」として対象外としたが、本判断を撤回する。case-close に rebase によるコンフリクト解消責務を追加する（REQ-0151-001/002、REQ-0131-024/025 参照）。ADR-0129 が「マージコンフリクト解決コストを受容」と決定した内容を実装する解決メカニズム（ADR-0132 Level 1）として、case-close の変更は必須である。機械的競合（rebase で自動解決可能）は case-close が解消し、解消不能時は case-auto へエスカレーションする。
