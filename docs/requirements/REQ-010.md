...
一時成果物の配置、ライフサイクル、構造化契約は REQ-008 が所有する。REQ と SPEC の健全性指標と分割予兆の定量定義は REQ-001 が所有し、IR 体系の存在条件と登録モデルは DEC-013（IR 登録モデルの簡素化）が所有し、本 REQ は指標と基準を用いた検査の実行を所有する。
