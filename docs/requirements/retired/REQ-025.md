...
>
> - 廃止理由: 全要件行が既存成果物への反映作業そのものであり（REQ-004-009 違反）、修復内容は完了済みの移行完了状態であること
> - 達成根拠: IR-005/036/055 の Decision 語彙対応と check_integrity.ts の検出ロジック更新は実施済み。適用範囲の IR-036 は DEC-013 により削除済み
> - 移行先: なし（後継要件なし。ADR 参照残存の検出は integrity-rule-catalog と check_integrity.ts が正規所有）
