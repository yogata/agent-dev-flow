...
case-auto の停止条件を段階化する。
あわせて改善効果を検証するため工程別タイムスタンプ計測を追加する。
...
| REQ-0151-007 | case-close と case-merge を分離しないこと。case-close のマージ手順に rebase を追加し、責務境界を「完了処理 + マージ時コンフリクトの機械的解消（rebase のみ、解消不能時は即エスカレーション、実装変更は行わない）」と再定義すること |
