...
| REQ-036-032 | 並列化の変更前後に、同一 Git revision の診断対象 corpus と既知 finding を比較基準として、壁時計時間、意味診断工程の時間、各診断担当の開始・完了・待機、委譲の失敗と再実行、モデル呼出とツール呼出を測定し、検出漏れと過剰検出の有無を確認すること。手動検証した対照例を含めて確認し、finding 差分には全件に正当理由を付すこと |
| REQ-036-033 | 並列化により短縮効果が得られない場合または診断品質が劣化する場合は成功扱いにせず、原因を評価して採否を判断すること。単一実行の測定値から改善率を断定しないこと。計測条件と結果を Case の検証証拠として残すこと |
