...
| REQ-0137-004 | `agentdev-git-worktree` スキルの `git-common-procedures.md` に、REQ-0137-001〜003 を満たす並列実行安全ステージング手順が定義されること。共有作業ツリーで git 操作を行う全コマンドは当該手順に従うこと |
...
- **対象外**:
  ...
  - draft/RU 以外のアーティファクト削除（intake archive・inspect archive 等。これらは別コマンドのライフサイクル）
