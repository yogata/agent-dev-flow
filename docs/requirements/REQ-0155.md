...
| REQ-0155-009 | SPEC 内部論理区分は5区分（挙動SPEC、カタログSPEC、横断契約SPEC、パラメータSPEC、実装詳細SPEC）とすること（AG-002、RU-20260722-02 合意）。パラメータ責務（retry 回数、timeout、閾値、重み、優先順位、上限、下限、fallback、許容範囲等）を実装詳細から区別すること。REQ-0155-001 が定義する従来4区分を拡張する形式とし、区分数を SPEC（document-model.md）で固定しない |
