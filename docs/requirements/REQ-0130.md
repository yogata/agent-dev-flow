...
- **対象**: case-run、作業用 worktree 実装、PR 作成、QG-3（PR作成直前ゲート）、本筋外 Finding の PR 本文記録、capture 責務境界（inbox 非変更・PR本文経由引き継ぎ）、チェックボックス外部書き込み確認、変更影響候補の実装計画組み込み、実装diff SPEC矛盾確認、1 Issueスコープ実行・サブエージェント委譲・永続状態ベース結果記録
