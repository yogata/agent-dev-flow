...
- **対象**: 配布成果物（command、skill、template、script のテキスト成果物）と producer 内部成果物の間の意味依存境界、producer 内部依存と consumer 解決可能依存の区別、意味境界不変条件の宣言、generic/template 許容、runtime 依存解決、harness 非依存、workflow 契約保持
