...
検出モデルと分類値は docs/specs/integrity/distribution-boundary.md が正規所有し、検出器、adapter、release archive 検査の実装詳細は Epic が所有する。
...
- **対象外**: 配布成果物の種別責務、原本と配置先、名前空間、Project Extensions 機構、skill 構造、subagent 委譲プロトコル（REQ-002）、配布基盤と導入モデル（REQ-009）、検出器・adapter・release archive 検査の実装詳細（Epic）、検出シグナル、分類値、誤検出抑制、projection 判定、gate 実行経路の詳細（docs/specs/integrity/distribution-boundary.md）
