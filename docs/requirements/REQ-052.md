...
Tool / Plugin 種別の harness 接続は v4 において Harness/Backend adapter 境界（ADF v4 実装責務境界 Design・DEC-036）が所有する。
...
| REQ-052-007 | ADF の実行時配布モデルは必要な Tools / Plugins を正規配布物として扱え、Harness/Backend adapter 境界を経由して harness へ接続できること |
...
| REQ-052-011 | Custom Tool が提供する構造化された副作用操作には、外部ソース（URL、Git リポジトリ等）からの取得操作を含められること。取得操作固有の詳細の Design 委譲は REQ-002-037 に従うこと |
| REQ-052-012 | GitHub Issue/PR操作の正規経路はCustom Tool agentdev_ghであり、Plugin設定契約、loader shim、runner-local細目、.gitignore推奨、v4-delegation-contracts経路をDesignに保持すること。 |
| REQ-052-013 | 副作用操作の対象解決に失敗した Tool は、失敗の failure detail に解決試行の診断情報（試行した解決手段、外部コマンドの終了コードと stderr の要因）と解決手続き（環境変数設定）への導線を含むこと |
