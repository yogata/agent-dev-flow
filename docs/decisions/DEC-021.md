...
- 2026-09-10 に Decision ライフサイクルの確認手続きに従い承認した（status: proposed → accepted）。公開入口2本（`scripts/install.ps1`、`scripts/self-sync.ps1`）が実在し、REQ-050 が承認済み要件として現行である実態との整合を解消する（REQ-001-021 との矛盾解消）。
