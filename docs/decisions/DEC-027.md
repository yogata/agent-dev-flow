...
- 2026-09-10 に Decision ライフサイクルの確認手続きに従い承認した（status: proposed → accepted）。REQ-048 再構築と Baseline V2 の定義・測定（docs/reports/req-048-baseline-v2-definition.md、同 measurement）が完了し、本 Decision の評価ループが現行根拠として運用されている実態との整合を解消する（REQ-001-021、REQ-057-015 との矛盾解消。verification-scope-catalog.md からの引用を正当化）。
