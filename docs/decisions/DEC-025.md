...
- 2026-09-10 に Decision ライフサイクルの確認手続きに従い承認した（status: proposed → accepted）。`docs/knowledge/`（Knowledge 文書種別）が実在し、REQ-056 が承認済み要件として現行、Capability Skill Extension 経由の旧接続契約（旧 REQ-056-002）は廃止済みである実態との整合を解消する（REQ-001-021 との矛盾解消）。
