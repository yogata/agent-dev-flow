...
- 2026-09-10 に Decision ライフサイクルの確認手続きに従い承認した（status: proposed → accepted）。REQ-044 が承認済み要件として現行であり、対象 script 群の標準 API 委譲（Bun.YAML、Project Extensions 限定の Zod、node:fs glob、node:util.parseArgs）と回帰テストが main で稼働している実態との整合を解消する（REQ-001-021 との矛盾解消）。
