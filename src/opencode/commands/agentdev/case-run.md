...
**検証失敗時（worktree 未作成、メインリポジトリにいる）**: 実行担当サブエージェントを起動**せず**停止し、Step 5（Worktree作成、ブランチ準備）へ戻るようユーザーに報告する。Step 6 へ進んではならない。
...
**L2 タイムスタンプ計測（REQ-0151-009、REQ-0130-028）**: task() 起動直前・直後に壁時計タイムスタンプ（JST、REQ-0114-082 の時刻形式に準拠）を記録し、Sisyphus-Junior 実行時間を計測する。併せて Step 5（Worktree作成）の開始・終了時刻、Step 8（worktree クリーンアップ）の開始・終了時刻を記録し、worktree 設定/クリーンアップ時間を計測する。計測した L2 タイムスタンプは Step 7 result、Step 8 完了報告に含める（case-auto が L1 計測で case-run task() の壁時計時間を読み取る際の内訳として使用）
