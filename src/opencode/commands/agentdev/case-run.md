...
**upstream handoff 停止判定**: Issue 本文、要件doc、Requirement Source に `agentdev_handoff: true` が含まれる場合、実装を開始せず停止する。agent-dev-flow repository への手動取り込み対象として報告する。判定は `agentdev-workflow-lifecycle` に従う
...
- **局所予防の範囲**: この確認は実装中の局所的な漏れ検出であり、`/agentdev/inspect-docs` の全体意味レビューの代替ではない

**Step 7**: 実装進捗確認
- work plan の全タスクが完了していることを確認。未完了タスクがある場合は実装フェーズを継続
- 実装中に判明したスコープ外項目は work plan 上で `> ℹ️ 別途確認: {項目名}` 形式に記録
- 全 work plan タスク完了確認後に提出フェーズへ移行
- **完了条件チェックボックスの最終評価・更新は case-close QG-4 の責務**（`agentdev-quality-gates` 参照）。case-run は完了条件チェックボックスを更新しない

### 提出フェーズ（Steps 8-12）

**べき等性**: PR既存時はSteps 8-10をスキップしStep 11-3のみ再実行後Step 12へ。

**Step 8**: 実装乖離検出（QG-3）→ `agentdev-quality-gates` の QG-3（Implementation Deviation Gate）に従って乖離検出を実行。判定基準・検査観点・乖離分類は同スキルの `references/qg-3-implementation-deviation.md` を参照
...
- intake と learning を混ぜた単一エントリにしない（G17）。capture 境界の詳細は `agentdev-workflow-orchestration` を参照
...
- **11-1: ローカル検証**（型チェック・Lint・ビルド・テスト）。検証成功時のみ11-2へ。失敗時 → `agentdev-workflow-orchestration` に従い Self-Healing を実行

- **11-2: PR作成**: `agentdev-gh-cli` に従い `--body-file` で実行。PR本文は `agentdev-workflow-templates` に準拠。Findingsセクション非空検証。書き込み後にVERIFY操作で内容検証

- **11-3: デプロイ検証**: `agentdev-workflow-orchestration` に従い検証。CI失敗時は同skillに従い自律修正
...
エラー発生時の対応は `agentdev-workflow-orchestration` に従う。エラー発生時は即座に停止し、エラー内容と再開ポイントをユーザーに報告。自動リトライは同一入力の機械的再試行およびStep 11-1/11-3の自律修正ループのみ許可。
...
- 完了判定はIssue本文の `完了条件` セクションを QG-3（`agentdev-quality-gates`）の乖離検出対象として参照する。完了条件チェックボックスの最終評価・更新は case-close QG-4 の責務
...
- G17: intake/learning 候補を混ぜた単一成果物にしない。capture 境界の詳細は `agentdev-workflow-orchestration` を参照
