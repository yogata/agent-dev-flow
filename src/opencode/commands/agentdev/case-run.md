...
本コマンドの workflow 実装本体を所有する Workflow Skill（`agentdev-workflow-case-run`）が、対応する project extension（`.agentdev/extensions/skills/agentdev-workflow-case-run.yaml`、kind: workflow-extension）を読み込む。
...
| STEP-S4 実行担当サブエージェント委譲 | 前置 gate 合格 | 委譲 result（4状態） | worktree root（相対パス）を引き渡し、result 契約に従って受領していること。委譲プロンプトが構造化文脈（10意味）を入力内に直列化していること（直列化形式は `agentdev-workflow-case-run` の委譲契約参照）。canonical_references が目的判別（正規原本、実行時投影、双方整合確認の別）を含むこと（判別は `agentdev-workflow-lifecycle` の参照先解決ポリシー参照） |
