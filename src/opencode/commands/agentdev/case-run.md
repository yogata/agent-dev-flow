...
**upstream handoff 停止判定**（REQ-0104-026）: Issue 本文、要件doc、Requirement Source に `apply_in_current_project: false` が含まれる場合、実装を開始せず停止する。agent-dev-flow repository への手動取り込み対象として報告する。判定は `agentdev-workflow-lifecycle/references/upstream-handoff.md` に従う
...
**Step 5.5: 実装中 SPEC / commands / skills 更新漏れの局所確認**（REQ-0115-022）: 実装完了後、変更差分（`git diff`）から以下の更新漏れを局所的に確認する:
...
- **局所予防の範囲**: この確認は実装中の局所的な漏れ検出であり、`/agentdev/docs-review` の全体意味レビューの代替ではない（REQ-0115-023）
...
**Step 7**: 乖離検出 → `agentdev-spec-compliance` に従ってチェック。品質メトリクス収集も実行。grep対象は `docs/` 全体（SHALL — REQ-0106-021）
    - **文書分類一貫性確認**（REQ-0108-186）: spec-compliance チェックに加え、`docs/specs/document-model.md` の Document Classification Policy に基づき、変更対象ドキュメントの分類が正しいことを確認する
