...
**Step 5.5: 実装中 SPEC / commands / skills 更新漏れの局所確認**: 実装完了後、変更差分（`git diff`）から以下の更新漏れを局所的に確認する:
...
**Step 7**: 乖離検出 → `agentdev-spec-compliance` に従ってチェック。品質メトリクス収集も実行。grep対象は `docs/` 全体（SHALL）
...
**Step 9.5**: 本筋外 Finding の PR 本文記録
...
**Step 10.5**: worktree クリーンアップ確認（SHALL）
