...
- **独立 OU の自動 Epic 化**: 複数の独立 OU（`depends_on` 空・L0 相当）を検出した場合、自動的に Epic Issue 化し Wave 1 に全 OU を配置する。これにより Standard flow は「真に単一 OU のみ」に縮退し、Standard/Epic 二系統を単一 Wave 実行モデルに統一する。詳細は `docs/specs/commands/case-open.md` の case-auto 並列委譲モデルセクション参照
...
OU 単位で処理する。Issue 化単位は REQ doc 単位ではなく OU 単位とする。子Issue 本文案作成・検査・Issue 作成は最大5件まで並列化できる（独立 OU・L0 相当・Epic Wave Model の最大5件上限に準拠）。並列実行安全ステージングプロシージャ（`agentdev-git-worktree`）に従い、共有作業ツリーでは明示パス指定でステージ・コミットし、他セッションの変更を排出しないこと。詳細は `agentdev-issue-management` を参照。委譲接続点: サブエージェントは子Issue本文候補とテンプレート充足検査のみを返し、親エージェントが `gh` 実行とVERIFYを行う

**Step 8-1**: 並列委譲と直列集約の分離 — 子Issue 本文案作成・検査・Issue 作成を並列対象とし、Epic Issue 作成・Wave 1 配置・Epic 本文ステータス追跡テーブル更新を直列集約対象とする。直列集約対象は並列委譲の完了を待ってから親エージェントが実行する。G04「全子Issue 作成完了後にテーブル更新（部分更新禁止）」は集約更新で維持する
