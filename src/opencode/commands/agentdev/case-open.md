...
3 要素のスキーマは `docs/specs/commands/req-define.md` の「draft-data test_strategy フィールドスキーマ」に従う。
...
属性値は `docs/specs/workflows/epic-wave-model.md` の「前工程完了度3段階分類（REQ-0146-010）」に従い、operation_units の `operation` 値（create/append/update/spec-create/spec-update）や artifact_actions の内容から判定する。
