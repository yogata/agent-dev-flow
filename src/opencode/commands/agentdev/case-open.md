...
- OU ID 指定なし、OU 2 件以上 → OU 一覧（`ou_id`, `target_req`, `target_spec`, `operation`, `result`）を表示して停止する。ユーザーに OU ID の指定を求める。ただし独立 OU（`depends_on` 空、L0 相当）が複数含まれる場合は停止せず自動的に Epic Issue 化する（REQ、Step 3-1 の自律構成生成へ分岐、独立 OU は Wave 1 に配置）
...
子Issue 本文案作成、検査、Issue 作成は最大5件まで並列化できる（REQ、Epic Wave Model の最大5件上限に準拠）。
