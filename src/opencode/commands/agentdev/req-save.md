...
3-3. **OU 選択ゲート**: ドラフトの `operation_units` セクションを確認し、処理対象 OU を決定する（REQ-0102-039〜041）:
     ...
     - OU ID 指定なし・OU 1 件 → その OU を自動選択して処理する
      ...
      - **SPEC 操作 OU の取扱い**: `operation` が `spec-create` / `spec-update` の OU は spec-save コマンドの対象であり、req-save は処理しない（スキップする）。req-save は REQ 操作（create/append/update）の OU のみ処理する（REQ-0136-013）
