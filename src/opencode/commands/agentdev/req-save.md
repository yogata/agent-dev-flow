...
**注記**: `REQ-{NNNN}.md` を永続的な基準ファイルとする。`{area}.md` への保存は行わない（retired ADR-0007、現在は REQ-0101 で規定）。
...
3b. 文書分類適合確認（REQ-0108-184）: REQ/ADR 保存前に、対象ドキュメントの種別が `docs/specs/document-model.md` の Document Classification Policy（REQ/ADR/SPEC/Guide/Report/DOC-MAP）に正しく分類されていることを確認する。分類ポリシーに適合しない場合は保存を停止し、理由を報告する
...
4b. **語彙・責務・runtime境界矛盾の防止**（REQ-0115-020, 021）: Step 4 の保存完了後、draft-meta と関連ドキュメント更新候補を使って、保存後に残る既知の矛盾を検出可能な範囲で防止する:
    ...
    - **SPLIT / REQ 再構成候補の検出時**: SPLIT や REQ 再構成候補を検出した場合は、`/agentdev/docs-review` の REQ structure review finding として扱う（REQ-0115-021）。本コマンド内では保存可能範囲を実行し、finding を完了報告の follow-up として明示する
    - **局所予防の範囲**: この step は保存時の局所的な矛盾防止であり、`/agentdev/docs-review` の全体意味レビューの代替ではない（REQ-0115-023）
  5. インデックス・ハブ更新:
    ...
    - **hash 一致検証**: `git rev-parse HEAD` で pull 後の commit hash を取得し、Step 2 で記録した読込時 hash と一致することを確認する
        ...
        - **一致しない場合**: リモートに新規変更が取り込まれたため、Step 2 の読込からやり直す（評価・承認を再実行）。構造化メッセージを表示:
          ...
          **アクション**: リモートに新規変更が取り込まれたため、読込からやり直します
          ```
3c. RU パス保存禁止（SHALL — REQ-0105-014）:
    - req-save は RU パスを docs 永続文書（REQ ファイルの Requirement Source セクション等）に記録しない（SHALL）
    - RU は一時成果物であり、docs の永続根拠に不適である（REQ-0105-045）
    - RU 削除は `case-open` の Issue作成 + VERIFY 成功後にのみ実行される（REQ-0105-015）
 ...
 11a. マルチREQ操作結果の保存（REQ-0102-020）: ドラフトに複数の `req-operation` + `target-req` ペアが含まれる場合、以下を保存する:
      - (a) 保存したREQドキュメントのリスト（REQ番号を含む）
      - (b) 各REQ操作から保存したREQドキュメントへのマッピング
      - (c) ソースRUからREQ操作へのマッピング
      - (d) case-openで消費可能な形式での保存結果
 11b. Issue作成の責任分離（REQ-0102-021）: req-saveはREQドキュメントの保存中にIssueを作成しない。Issue作成はcase-openの責任範囲とする
...
### Issue作成制約
- G11: req-saveはIssueを作成してはならない。Issue作成はcase-openの責任範囲である（REQ-0102-021）
