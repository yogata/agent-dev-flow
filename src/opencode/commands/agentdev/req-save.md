...
2. ドラフト読込: `.sisyphus/drafts/req-draft-*.md` を読み込む → 最新の1件を対象とする。見つからない場合はエラーで中止: `壁打ちドラフトが見つかりません。先に /agentdev/req-define を実行してください。`
    - **読込時 hash 記録**: `git rev-parse HEAD` で読込時点の commit hash を記録する（SHALL）
3. ドラフト検証: `draft-meta` セクションの必須フィールド（work_type, req-operation, topic-slug）が存在することを確認。欠損時はエラーで中止
3a. 分類ゲート検査: CREATE対象REQの要件テーブルの各要件行について、既存成果物への反映作業のみを表す行が残っていないか検査する
    ...
    - **ユーザー指示待ち**: 報告後、ユーザーの指示（該当行の削除・移送先変更・req-define への差し戻し等）を待つ。ユーザーの単純な続行指示のみでは Step 4 へ進めない。分類結果に対する明示的な判定変更指示（「反映作業ではなく要件行として扱う」等の理由付き指示）があった場合のみ Step 4 へ進む
3b. 文書分類適合確認: REQ/ADR 保存前に、対象ドキュメントの種別が `docs/specs/document-model.md` の Document Classification Policy（REQ/ADR/SPEC/Guide/Report/DOC-MAP）に正しく分類されていることを確認する。分類ポリシーに適合しない場合は保存を停止し、理由を報告する
    - **adr-revision-mode: full-reclassification**: draft-meta に `adr-revision-mode: full-reclassification` が指定されている場合、既存 ADR の full reclassification を許可する（SHALL, REQ-0112-056）。このモードは REQ-0112-044 の例外ルールに従い、既存 ADR を current baseline / retired / REQ・SPEC・guide 移管候補として再分類できる。実行前に以下をドラフトまたは保存結果へ明示的に記録すること（SHALL）:
      - 移管先情報: 移管先の REQ / SPEC / guide と該当セクション
      - 変更内容: 対象 ADR と変更種別（baseline維持 / retired移動 / REQ移管 / SPEC移管 / guide移管等）
4. REQ ファイル操作 → `agentdev-req-file-manager` の判定ロジックと採番ルールに従って実行。ドラフトに複数の `req-operation` + `target-req` ペアが含まれる場合は、各ペアを順次処理する（バルクUPDATE対応）:
    ...
    - **REQ再構成候補の検知**（SHOULD）: REQ保存処理中にREQ体系上の歪み（分散・肥大化・重複・不要化・ドリフト等）を検知した場合、REQ再構成intakeを `.agentdev/intake/inbox/req-restructure/` に保存する。検知観点は `agentdev-req-file-manager` の「REQ再構成検知観点」を参照する
4b. **語彙・責務・runtime境界矛盾の防止**: Step 4 の保存完了後、draft-meta と関連ドキュメント更新候補を使って、保存後に残る既知の矛盾を検出可能な範囲で防止する:
    - 語彙の不整合（旧語彙の使用、正規語彙との矛盾）
    - 責務の重複・欠落（REQ/SPEC/guides 間の同一責務の重複、または責務の空白）
    - runtime 境界の矛盾（command 定義と SPEC の不一致、skill 責務と command 責務の境界崩れ）
    - **SPLIT / REQ 再構成候補の検出時**: SPLIT や REQ 再構成候補を検出した場合は、`/agentdev/docs-review` の REQ structure review finding として扱う。本コマンド内では保存可能範囲を実行し、finding を完了報告の follow-up として明示する
    - **局所予防の範囲**: この step は保存時の局所的な矛盾防止であり、`/agentdev/docs-review` の全体意味レビューの代替ではない
  5. インデックス・ハブ更新:
    ...
    - **hash 一致検証**: `git rev-parse HEAD` で pull 後の commit hash を取得し、Step 2 で記録した読込時 hash と一致することを確認する
        ...
        - **一致しない場合**: リモートに新規変更が取り込まれたため、Step 2 の読込からやり直す（評価・承認を再実行）。構造化メッセージを表示:
          ...
          **アクション**: リモートに新規変更が取り込まれたため、読込からやり直します
          ```
3c. RU パス保存禁止（SHALL）:
    - req-save は RU パスを docs 永続文書（REQ ファイルの Requirement Source セクション等）に記録しない（SHALL）
    - RU は一時成果物であり、docs の永続根拠に不適である
    - RU 削除は `case-open` の Issue作成 + VERIFY 成功後にのみ実行される
 ...
 11a. マルチREQ操作結果の保存: ドラフトに複数の `req-operation` + `target-req` ペアが含まれる場合、以下を保存する:
      - (a) 保存したREQドキュメントのリスト（REQ番号を含む）
      - (b) 各REQ操作から保存したREQドキュメントへのマッピング
      - (c) ソースRUからREQ操作へのマッピング
      - (d) case-openで消費可能な形式での保存結果
 11b. Issue作成の責任分離: req-saveはREQドキュメントの保存中にIssueを作成しない。Issue作成はcase-openの責任範囲とする
