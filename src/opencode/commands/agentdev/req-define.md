...
`agentdev-req-analysis` の分析観点に従って網羅。詳細ゲートは `agentdev-req-analysis` を参照
  - **5-1. 変更影響候補抽出**: 変更影響候補を抽出し、ドラフトに保持する。RU の frontmatter、本文から対象領域キーワードを抽出し、glob/grep で関連 REQ/ADR/SPEC を事前特定してサブエージェント調査委譲の調査優先対象リスト（ヒントでありハードフィルタではない）を構築すること（REQ）。REQ が要求する実ファイル完全列挙は維持すること。詳細手順は `agentdev-req-analysis`（`references/investigation-scope-refinement.md`）を参照。委譲接続点: サブエージェントは探索結果、分類候補、根拠のみを返し、親エージェントがドラフト反映を判断する
