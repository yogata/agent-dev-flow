...
本コマンドは実行時に自分に対応する project extension（`.agentdev/extensions/commands/req-define.yaml`）を読み込む（ADR-0135）。
...
`agentdev-req-analysis` の分析観点に従って網羅。詳細ゲートは `agentdev-req-analysis` を参照
 ...
 - **5-5. 実行主体分類表（REQ-0146-007）**: 委譲契約を定義する場合、各委譲について実行主体分類表（adapter skill / command / subagent / harness）を必須として含めること。分類定義は delegation-contracts SPEC（extension 経由）の「実行主体分類表（委譲契約必須項目、REQ-0146-007）」セクションを参照。委譲を含まない要件では本項目は省略可
