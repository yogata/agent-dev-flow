...
`--auto` で自動 promote される高確信度カテゴリ、投入先、実行ログ、誤検知 revoke 手順の詳細は workflow-contracts SPEC（doc-input 経由で解決）の「inspect-promote 自動 promote」セクションに原本を置く。
...
2. `.agentdev/doc-inputs/commands/inspect-promote.yaml` を読み込む
...
`--auto` が指定された場合、分類結果のうち workflow-contracts SPEC（doc-input 経由）の自動 promote 対象カテゴリに合致し、かつ安定契約例外および否定文脈を満たさない高確信度検出事項を HITL 承認なしで `.agentdev/intake/promoted/inspect-auto-{timestamp}-{slug}.md` へ投入する。
...
- G07: `--auto` は自動 promote 対象カテゴリ（workflow-contracts SPEC 参照、doc-input 経由）に合致する高確信度検出事項のみを投入し、意味判断、曖昧な分類、ADR 要否判断を含む検出事項は手動分類へ回す
