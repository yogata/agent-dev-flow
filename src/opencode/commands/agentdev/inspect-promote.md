...
本コマンドの workflow 実装本体を所有する Workflow Skill（`agentdev-workflow-inspect-promote`）が、対応する project extension（`.agentdev/extensions/skills/agentdev-workflow-inspect-promote.yaml`、kind: workflow-extension）を読み込む。
...
| STEP-5 adversarial-review | review 挿入境界到達（default-on、skip 条件該当時は省略） | review 結果と反映後の分類案 | accepted finding が分類案へ反映されていること |
