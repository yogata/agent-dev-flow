...
- squash merge 先とブランチ同期の対象は main とする。利用者向け操作と挙動を変更しない
...
- Design status 昇格（draft → accepted）は、対象 Design が `draft` かつ今回の実装が Design 内容を検証済みの場合のみ実行する（case-ready / case-revise は accepted を付与しない）
