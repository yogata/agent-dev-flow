...
Command→Skill 参照妥当性と Skill 構造を read-only で診断し、finding、分類、根拠、推奨 route を提示する。診断結果は `.agentdev/diagnostics/inbox/` へ出力する。

## 基本原則: 診断専用（Read-Only）

診断を基本とし、許可される side effect は `.agentdev/diagnostics/inbox/diagnostics-skills-finding-*.md` の生成のみ。
...
- `.agentdev/diagnostics/inbox/diagnostics-skills-finding-{topic}.md`（finding file 出力）

## Steps

1. **診断対象の読込**: Command / Skill 定義を読み込み、Command→Skill 参照、Skill frontmatter、本文構造、references 利用、template / script 参照を把握する
2. **各診断観点の評価**: `agentdev-diagnostics-skills` に従い、参照妥当性、粒度、段階的開示、責務境界、canonical name、内部構造依存を評価する
3. **分類**: finding ごとに診断分類ラベルを付与する
4. **route 提示**: 修正は実行せず、推奨 route を提示する
5. **finding 出力**: finding を `.agentdev/diagnostics/inbox/diagnostics-skills-finding-{topic}.md` へ出力
6. **完了報告**: 完了報告 template に従って出力

## Guardrails

- G01: ファイルを変更・作成・削除しない。ただし `.agentdev/diagnostics/inbox/diagnostics-skills-finding-*.md` の生成は例外として許可する
