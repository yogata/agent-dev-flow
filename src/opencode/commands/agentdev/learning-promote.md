...
6. **evaluation-report.md の生成・更新**（REQ-0105-054）:
...
7. **廃棄判定**（11カテゴリ + duplicate）:
   - **主入力**: Phase 3 で生成した evaluation-report.md の問題クラスラスタ（raw learning item の再分類は禁止）（REQ-0103）
...
10. **ユーザー承認**:
    ...
    - 判定の変更指示があれば Step 7〜8 を再実行
    - 承認したクラスタのみ処理
    - 承認しない → 「昇華をキャンセルしました」と報告して終了
    - **自動確定禁止**: AI 単独で promote / prune の最終確定を行ってはならない（MUST NOT）（REQ-0105-055）
