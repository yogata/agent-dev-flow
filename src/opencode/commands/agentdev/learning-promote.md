...
反映ルート: promoted → `/agentdev/backlog-review`（RU 生成）→ `/agentdev/req-define` → `/agentdev/case-auto`。
...
- 採用済み成果物の受け渡しは `/agentdev/backlog-review` 経由とする（case-auto への直接受け渡しは行わない。反映ルート: promoted → `/agentdev/backlog-review`（RU 生成）→ `/agentdev/req-define` → `/agentdev/case-auto`）
