...
- G10: 無条件の自動REQ化禁止（REQ-0155-005）: 学びを直接 REQ 化しない。恒久契約（REQ/ADR/SPEC）への昇華可能性を Step 7 で評価し、昇華可能なもののみ `promoted/` へ出力する。昇華不能な知見は living pool（`archive/active.md`）で維持する
...
| archive/active.md prune 失敗 | 採用済み成果物は保持。prune エラー報告し手動 prune 案内 |
