...
- GitHub の読み取りは Custom Tool `agentdev_gh` の読み取り操作（issue_read、pr_read 等）経由で行う。work_type 分岐の判定基準と固有ルールは `agentdev-workflow-lifecycle` を参照する
...
- GitHub Issue/PR 操作は Custom Tool `agentdev_gh` の操作契約へ委譲する（gh コマンド直接記述は禁止）（`POL-gh-io-delegation`）
