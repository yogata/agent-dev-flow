...
本コマンドは project read contract 手順（ADR-0133）に従う:
1. `.agentdev/config.yaml` を読み込む
2. `.agentdev/read-contracts/commands/spec-save.yaml` を読み込む
...
**read contract 更新要否の確認（REQ-0157）**: SPEC の追加、移動、分割が `.agentdev/read-contracts/**` に影響するか確認する。移動または分割により read contract 参照先 SPEC パスが変わる場合、当該 read contract の paths を更新する。read contract 参照先 SPEC を移動した場合はエラーとし、spec-save 自身は移動を完了させずユーザー判断を仰ぐ（IR-056 check #6 strict 違反を防止）。SPEC 新規作成で既存 command/skill の実行時参照が増える場合、対応 read contract の `must_read` または `conditional_read` への追加をユーザーに提案する（spec-save 自身は read contract を直接編集しない）。
