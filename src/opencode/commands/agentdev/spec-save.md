...
本セクションは SPEC 廃止確定時（SPEC ライフサイクルの `superseded` 遷移、または retired ディレクトリへの移動）に、下流（配布 Command/Skill 群、Guide 群、REQ 群）へ残る旧 SPEC パス、ID 参照の横断クリーンアップを実行するステップの**追加候補**を記録する。既存の spec-save 手順（Step 1〜11）に代わるものではなく、既存フローを破壊しない追加ステップである。本候補は REQ-0124-024（廃止 REQ/SPEC 由来記述残置検出カテゴリ）および integrity-rule-catalog.md の新規 IR 候補（retired-reference-residual）と整合する。本候補の実行可否、タイミングは別途 SPEC 確定を待って確定する。

### 対象トリガ

- SPEC の廃止確定時（`status: superseded` 遷移、retired ディレクトリへの移動、SPEC ファイル削除）
