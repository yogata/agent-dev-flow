...
8-2. **独立 OU 並列委譲（REQ-0114-087〜093）**: Step 8-1 の例外。未処理 OU のうち `depends_on` 空・L0 相当の独立 OU が複数存在する場合、それらを最大5件まで並列委譲で処理する（REQ-0114-087・Epic Wave Model の最大5件上限に準拠）。Step 8-1 と同様に Step 2 に戻り、case-open task に独立 OU 群をまとめて渡す。case-open が自動 Epic 化した後は Step 4-1（Wave 反復制御）経由で処理する:
