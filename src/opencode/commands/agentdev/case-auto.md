...
case-run は Sisyphus-Junior、ulw-loop への task 委譲モデル（`task(subagent_type="Sisyphus-Junior", load_skills=["agentdev-case-run-execution-adapter"])` + 委譲 prompt 内 `/ulw-loop` command）を使用する。
...
**工程別タイムスタンプ計測（L1）（REQ-0151-008、REQ-0114-094）**: 各工程（req-save/ spec-save/ case-open/ case-run/ case-close）の task() 起動直前・直後に壁時計タイムスタンプ（JST、REQ-0114-082 の時刻形式に準拠）を記録する。記録した工程別タイムスタンプは Step 8（完了報告）、Step 7（停止時報告）の工程別内訳として使用する。現行の全体開始・終了時刻記録（REQ-0114-082/083）を工程別内訳へ拡張するものであり、全体時刻は廃止せず工程別内訳を併記する。ulw-loop 内部メトリクス（L3）は対象外（REQ-0151-010）。case-run 内の Sisyphus-Junior 計測（L2）は case-run result に含まれ、case-auto は case-run task() の壁時計時間（L1 計測）として扱う（L2 内訳は case-run result から読み取る）
...
停止時は完了済み OU、進行中 OU、未実行 OU、再開可能な次コマンドを報告する
  ...
  - **工程別タイムスタンプ内訳（L1）（REQ-0151-008）**: Step 4 で記録した工程別（req-save/ spec-save/ case-open/ case-run/ case-close）の task() 起動前後タイムスタンプ、工程別所要時間を含める。スキップした工程（例: spec-save スキップ時）は除外可。case-run の L2 内訳（Sisyphus-Junior 実行時間、worktree 設定/クリーンアップ時間）は case-run result から読み取って含める
