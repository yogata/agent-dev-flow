...
8-1. **Standard flow 逐次OU処理ループ**: Standard flow の case-close task 完了後、未処理 OU が残存する場合は次 OU の処理を自動的に開始する:
 ...
 - **独立 OU 並列委譲の例外**: `depends_on` が空の独立 OU（L0 相当）が複数存在する場合は並列委譲で処理できる。後述「並列委譲モデル」参照

## 並列委譲モデル

case-auto は独立 OU（`depends_on` 空・L0 相当）または同一 Wave 内子Issue を最大5件まで並列委譲できる（Epic Wave Model の最大5件上限に準拠）。詳細は `docs/specs/workflows/epic-wave-model.md`・`docs/specs/commands/case-auto.md` 参照。

 - **独立 OU の自動 Epic 化**: case-open は複数の独立 OU を検出した場合、自動的に Epic Issue 化し Wave 1 に全 OU を配置する。これにより Standard flow は「真に単一 OU のみ」に縮退し、Standard/Epic 二系統を単一 Wave 実行モデルに統一する
 - **並列委譲の実施**: 複数独立 OU を case-open が自動 Epic 化した後、case-run(#epic) が Wave 1 の子Issue を Sisyphus-Junior(ulw-loop) に並列委譲する（既存 Epic Wave モデル・最大5件）
 - **結果集約**: 並列委譲された単位の成功・失敗は case-auto が集約し、最終判定に反映する。一部失敗時は Step 7（停止条件の検出）に従い、完了単位・進行中・未実行・再開コマンドを報告する
 - **直列集約対象**: 採番・index 更新・draft 更新・commit・push・Epic 本文ステータス追跡テーブル更新は並列委譲の完了を待ってから親コマンド（各工程の task）が実行する。Epic 本文ステータス追跡テーブルの更新は case-close(#epic) の単一書き手責務
