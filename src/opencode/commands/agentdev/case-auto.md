...
**Phase 別結果・フォールバック理由・破棄回復記録（REQ-0114-102〜107）**: 完了報告には Phase 1（case-open 順次実行）、Phase 2（case-run 並列実行）、Phase 3（case-close 順次実行）の各 Phase の実行結果を含める。Phase 2 を順次フォールバックで実行した場合はその理由を記録する。Phase 2 の bg task 破棄を検知して回復した場合は、検知した状態区分（commit 済みで PR 未作成、未コミット変更残存）と回復結果を記録する。
...
- G28: case-auto は委譲工程の完了結果（Issue/PR番号、pass/warn/fail）のみを親コンテキストに保持し、委譲工程内部の調査過程、中間ログ、読解メモを親コンテキストに累積しないこと。case-run インライン実行時のコンテキスト管理は harness の機能で対応し、REQ-0114-073（親コンテキスト非累積）は case-run インライン実行時の例外として取り扱う
