...
7. **保存**: `.agentdev/intake/promoted/` に保存する。詳細は `agentdev-intake-pipeline` を参照。委譲接続点: 親エージェントのみが保存する
...
10. **.agentdev/intake 変更の commit と push**: `.agentdev/intake/` 配下の変更のみを commit/push する。詳細は `agentdev-intake-pipeline` を参照。委譲接続点: 親エージェントのみが commit/push を行う
