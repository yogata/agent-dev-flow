...
| `observation_write` | 観測 JSON（1 Workflow 実行 = 1 JSON）の形式検証と `.agentdev/jev-observations/` への原子的書込み（一時ファイル + rename）。JSONL は生成しない。`observationId` 付きは追記完成 mode で、入力は追記対象 field（LLM 最終判断関連 field: llmFinalJudgment、llmTreatment）のみを受け付ける。run 級 field（workflow、provider、outcome、durationMs、inputs 等）と Jev 側観測項目は evaluate 時点部分レコードに既に記録済みのため、入力に再送しても受理されない（追記不要）。同一 JSON へ judgmentId 単位で追記し、完了状態 field（recordState）を complete へ更新する（冪等・重複 JSON なし）。`observationId` なしは完成観測（フルレコード形状）の新規書込み（後方互換経路）。recordState partial の直接書込みは拒否する |
