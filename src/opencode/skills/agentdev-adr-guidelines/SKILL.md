...
本スキルの原本仕様は `agentdev-adr-guidelines` SPEC である。
...
## ADRのライフサイクル
ADRの状態は以下のいずれかをとる。
一度 Accepted になった ADR は意味的に不変とし、決定を覆す場合は新しい ADR を作成して古いものを Superseded/Deprecated にする。
ただし明示承認済みの非意味修正（タイポ、リンク切れ、表記統一等）に限り直接更新を認める。判定は後述「accepted ADR の更新規則」に従う。
...
## 運用ルール
- **不変性**: 承認済み ADR は意味的に不変（Immutable）として扱う。背景が変わり決定を覆す場合は、新しい連番で ADR を作成する。非意味修正の直接更新は「accepted ADR の更新規則」を参照。
...
accepted ADR は意味的に不変とする。ただし明示承認済みの非意味修正に限り、直接更新を認める。
詳細は SPEC `agentdev-adr-guidelines` の「accepted ADR の更新規則」を正とし、本節は実行入口の要約を示す。
...
| 意味変更（後継 ADR 必須） | 新規 ADR を作成し、旧 ADR を superseded | 決定内容の追加・削除、適用範囲の変更、必須条件・制約の変更、正規所有者の変更、採用方式の変更、外部から観測可能な結果の変更 |

### 直接更新の実行条件

- 対象 ADR の frontmatter `status` が `accepted` であり、変更箇所が非意味修正リストのいずれかに該当する
- 直接更新前に明示承認記録が存在する
- 意味変更を表記修正として扱わない（意味変更の場合は後継 ADR を作成する）
- Report へ規範要件または必達条件を移さない
- accepted ADR の過去版を無言で書き換えない

### 憲章 ADR の移行時識別子の扱い

憲章 ADR（charter ADR）に残存する移行時識別子は次の通り処理する。判定の詳細は SPEC に委ねる。

| 識別子 | 扱い | 理由 |
|---|---|---|
| `WS-9` | 直接削除可 | 非意味ラベル。決定内容を変えない |
| `案B` ラベル | 案番号のみ削除可（決定内容は具体文で維持） | 決定内容を維持する非意味修正 |
| 10シナリオの抽象条件化 | 後継 ADR 必須（直接編集不可） | 意味変更に該当する |
| 10シナリオの定義 | SPEC が所有 | — |
