...
本スキルの原本仕様は [`agentdev-adr-guidelines` SPEC](../../../../docs/specs/skills/agentdev-adr-guidelines.md) である。
...
**方針**: false negative（ADRが必要なのに見逃し）を防止する。
false positive（不要なADRの作成）は後に整理できるため、微妙な場合はADR側に寄せる。
ただし「ADR作成可否条件」の作成不可条件（作業手段を主題とするもの等）に該当する場合は寄せの対象外とし、retire/supersede または REQ/SPEC/guide で処理する。
