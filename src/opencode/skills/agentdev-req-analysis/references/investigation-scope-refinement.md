...
実績 ses_10c29a13 で req-define のサブエージェント調査委譲に全体の約 34% の時間を消費した。
入力スコープが広すぎたことが主因であるため、決定的処理で優先調査対象を事前特定し、サブエージェント調査委譲が優先的に調べるべき対象を絞り込む。

**ヒントでありハードフィルタではない**: 絞り込み結果はサブエージェント調査委譲の調査優先対象リスト（priority targets）として渡す。
サブエージェント調査委譲はこのリストを優先調査の目安とし、リスト外のファイルを調査対象から除外してはならない。
実ファイル完全列挙の要求は維持する。
...
サブエージェント調査委譲は `high` → `medium` → `unscoped` の順で調査効率を高める。
