...
修正の要否を検討する際、実装面と Design 面の両面を分析し、各面の修正対象と修正内容を明示する。
片面のみの分析で修正要否を断定しない。
...
project 固有の判断知識は、learning 昇華先ルーティング（バックログ統合の要件が正規所有する learning 昇華先のルーティング契約）を経由して docs/knowledge/ へ保存され、リスク導出規則として次の分析で使われる成長する資産として扱う。
