...
- [構成変更とタスク依存の相互作用確認](#構成変更とタスク依存の相互作用確認)
- [検証手段の質基準](#検証手段の質基準)
...
要件分析で要件行候補を確定する際に適用する要件行の品質基準。
原本は document-type-responsibilities Design「要件行の書き方」「要件性」「粒度」節である。

要件行候補は次の要件性チェックリストを全て満たす。
...
- 肯定文主文: 主たる文意は肯定文で記述し、否定文は境界条件、例外、補足として併記する
- 長大行の分割: 1要件行に複数ルール、長大列挙、複文が混在する場合は1判断につき1行へ分割する
- 配置妥当性: Design 相当内容（スキーマフィールド、enum 値一覧、判定表等）のみを主たる文意とする行は Design 等への移送候補とし、安定契約例外に該当する場合のみ REQ に要約として残す
...
片面のみの分析で修正要否を断定しない。
コードレベルの work plan は case-run 責務としつつ、どの実現面を変更すべきかの確定を case-run へ先送りしない（実現面確定の先送り禁止）。
...
- 実現面の変更方針は、`realization_actions` セクションへ構造化して case-open へ引き継げる形で確定する（構造化ハンドオフ契約）
...
要件が構成変更とタスク依存（dependsOn）の組合せを含む場合の、要件定義時の必須確認項目である。

**対象条件**:

- 構成変更: ビルド構成、検証構成、参照構成の変更（例: tsconfig project references の変更）
- タスク依存: タスクランナーの dependsOn によるタスク実行順序の変更
- 上記2種の変更が同一の要件に含まれること

**相互作用の機序**:

project references を用いた構成では、参照先パッケージの型情報を参照先プロジェクトのビルド出力（dist）から解決する。
このため typecheck タスクのビルド依存削除と参照構成の維持を同一の変更内で行うと、物理的に両立せず、実行段階で typecheck が停止する場合がある。
組合せの両立性は各変更単独の確認では判定できず、発見が実行段階まで遅延する。

**必須確認項目**:

構成変更 × タスク依存（dependsOn）の組合せを含む要件では、要件定義時に最小再現での typecheck 検証を必須確認項目として実施する。
最小再現では、対象の組合せに関係する構成と依存のみを含む構成で typecheck を実行し、実行段階で停止しないことを確認する。

**限定**:

最小再現検証は要件定義段階の軽量確認であり、実装検証を代替しない。

**blocked 時の記録慣行**:

最小再現検証で組合せの両立不能が確認された場合、blocked としてユーザー確認に基づく意思決定（blocked → 親判断）を行い、その記録慣行を維持する。
