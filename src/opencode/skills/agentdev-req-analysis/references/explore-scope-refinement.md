...
REQ-0102-002 が要求する実ファイル完全列挙は維持する。
...
**完全列挙の維持（REQ-0102-002）**: 上記 glob は関連候補の事前特定に用いるが、REQ/ADR の実ファイル完全列挙自体は別途 `agentdev-req-analysis` の「既存REQ/ADRの定量的照合」で実施する。
...
委譲時最小契約（ADR-0112 §5）に従い、リストは探索のヒントであり、explore 委譲の出力契約（探索結果、分類候補、根拠）を変更しない。
