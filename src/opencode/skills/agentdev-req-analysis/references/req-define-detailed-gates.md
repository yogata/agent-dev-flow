...
各要件の対象ドキュメント種別が `docs/specs/document-model.md` の Document Classification Policy に適合しているか検証する。
...
2. **関心分類数**: 以下4シグナルの検出数を集計する（`agentdev-req-structure-diagnostics` スキルの SPLIT 観点に準拠）:
 ...
 - 複数 lifecycle 段階の混在
3. **成果物種別数**: `docs/specs/req-impact-map.md` の「影響する Artifact」列、または要件本文の対象成果物記述から集計する。
