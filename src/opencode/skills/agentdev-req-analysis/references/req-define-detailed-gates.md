...
REQ 要件行として残った各行について、SPEC 分離基準（REQ-0101-068）違反の残留がないか検出する。検出シグナルは `check_integrity.ts` IR-044 と同じ9種別（schema field・enum 値一覧・fixture detail・checker 個別ルール・false positive 抑制・Step 番号・Phase 番号・内部アルゴリズム・作業履歴）を参照する。手順:
