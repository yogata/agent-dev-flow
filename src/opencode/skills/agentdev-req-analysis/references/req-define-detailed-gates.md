...
REQ 要件行として残った各行について、SPEC 分離基準違反の残留がないか検出する。検出シグナルは SPEC 分離基準の9種別（schema field、enum 値一覧、fixture detail、checker 個別ルール、false positive 抑制、Step 番号、Phase 番号、内部アルゴリズム、作業履歴）を参照する。手順:
...
両系統は後方互換性のため既存の `create`/ `append`/ `update` を維持する。SPEC 候補（Step 5-2/5-3 で分離した `draft-meta.spec-candidates`）が存在する場合、対応する SPEC 操作 OU を生成し `target_spec` に想定配置先 SPEC パスを設定する。
