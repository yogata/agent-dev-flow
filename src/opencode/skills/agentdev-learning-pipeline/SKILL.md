...
description: Learning pipeline（capture → promote）の共通知識。inbox entry schema、問題クラス分類基準、8軸評価ディメンション、evaluation-report schema、処分区分、既存対策照合、promoted artifact schema、prune 方針、各層の責任分界を定義する。USE FOR: learning-promote の実行時参照、learning pipeline の拡張・変更時の基準確認。DO NOT USE FOR: agentdev-learning-capture（capture専用として独立）、req-define（promoted artifact の参照側だが本 skill は参照しない）、一般的なコマンド作成。
---
...
- **promote**（learning-promote command）: 問題クラス分類・8軸評価・evaluation-report 生成・inbox→archive 移動・昇華判定・promoted artifact 生成・prune を一括実行
...
| evaluation-report.md | promote 内部で生成される境界 artifact。毎回上書きされ長期履歴ではない。promote の分析フェーズで生成され、昇華判定フェーズの主入力として取り込みされる | 境界 artifact。promote 内部の分析→昇華判定間の受け渡し専用。履歴蓄積は行わない | promote が生成（上書き）し、読み取り・取り込みする。capture は参照しない |
| promoted/ | promoted artifact の staging 領域。生成された artifact は `/agentdev/backlog-review` が読み込み、RU 化後に `/agentdev/req-define` に合流する。`.opencode/` や実装コードへの直接反映は禁止。`case-run` への直接受け渡しも禁止 | staging 専用。promote が生成し、backlog-review が取り込みする。pipeline 外への直接反映は不可 | promote が promoted artifact を生成する。backlog-review が明示的に読み取る |

**制約**: raw learning item を runtime command / skill の直接参照対象にしない。学びは昇華（promote → promoted artifact → backlog-review → RU → req-define）を経て初めて command / skill / template / AGENTS.md / docs へ組み込まれる。
...
| promote | learning-promote（command） | 正規化・問題クラス分類・8軸評価・evaluation-report 生成・inbox の entry を archive に移動・inbox クリア・廃棄判定・promoted artifact 生成。archive から staged / rejected / duplicate を prune し、deferred / 未処理を保持 |
...
ADR 候補分類の前に、`agentdev-adr-guidelines` の除外基準を適用すること（SHALL, REQ-0112-054）。以下に該当する item は ADR 候補から除外し、REQ/SPEC/guide/command 等の適切な反映先候補として扱う。
...
learning-promote は除外した item について、evaluation-report に除外理由と代替反映先候補を記録すること（SHALL）。除外理由は「どの除外基準に該当したか」と「根拠事実」を含める。
...
## Promoted Artifact Schema

learning-promote が出力する promoted artifact の形式。`/agentdev/backlog-review` が読み込み、RU 化後に `/agentdev/req-define` に合流する。promoted artifact は backlog-review 以前の段階（pre-backlog-review）であり、RU への変換は backlog-review が行う。
...
```
promoted/ → /agentdev/backlog-review → /agentdev/req-define → /agentdev/req-save → /agentdev/case-open → /agentdev/case-run
```

- promoted artifact は `/agentdev/backlog-review` が読み込み、RU 化後に `/agentdev/req-define` の明示入力として扱われる
- promoted artifact の形式要件は本ドキュメント内「Promoted Artifact Schema」セクションを参照

- `case-run` への直接受け渡しは禁止（`backlog-review` → `req-define` を経由すること）

## Staging Stub Archive Rules

取り込み済み promoted artifact の archive ルール。`case-close` が本 skill を load して実行する。

### 対象

- `.agentdev/learning/promoted/*.md`（learning-promote が生成した promoted artifact）
- imported と判定された artifact のみ（機械的4条件全充足）。不採用 artifact は対象外
...
1. artifact 本文の末尾に `## 取り込み記録` セクションを追記:
   ```markdown
   ## 取り込み記録
...
- imported と判定されなかった artifact
- artifact パスが Issue に記録されていない場合
