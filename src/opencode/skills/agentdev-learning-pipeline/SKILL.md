...
**living pool 維持（REQ-0155-005）**: 昇華不能な知見（`deferred` 判定、情報が断片的、出現回数が少ない等）は `deferred.md` の living pool で維持し、REQ 化しない。
...
| 10 | deferred | まだ昇華の余地がない、情報が断片的、出現回数が少ない。**living pool（`deferred.md`）で維持し REQ 化しない**（REQ-0155-005） |
...
- **prune 非対象**: deferred/ 未処理/ 再評価対象のエントリは deferred.md に残す（REQ-0147-007）
