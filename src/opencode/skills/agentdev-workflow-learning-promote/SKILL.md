...
各 STEP は再開ポイント（resume point）を持ち（DEC-{N}、`docs/designs/<foundations/v4-durable-state-and-recovery>.md`）、会話コンテキストに依存せず、永続状態（inbox.md / deferred.md / evaluation-report.md / promoted/ の実ファイル状態、分類確定状態）から再開点を再構成する。
