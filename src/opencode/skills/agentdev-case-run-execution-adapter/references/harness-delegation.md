...
複数の実行担当サブエージェントを並列委譲する場合（Epic Wave 並列委譲、case-auto orchestration stage 3）、起動バーストによる実行基盤・モデル provider への瞬間的なリクエスト集中を抑えるため、委譲起動ごとに10秒の起動間隔を置く（v4-runtime-execution-model Design「runtime 制御ループ」節〔起動間隔・並列数制御〕）。
...
- 親子実行関係は委譲単位識別子と委譲 prompt の構造化文脈から導出する（親実行: ADF 工程 workflow_phase の対象 Issue 実行。子実行: 本委譲単位識別子）。導出可能な親子表示を重複記録しない
- 委譲目的は委譲 prompt 内の実行 command 指定と category で表現される（処理区分の正規記録先）。委譲目的を独立した識別 key として重複記録しない
- 親子実行関係の識別は ADF が発行する委譲単位・実行単位識別子を正規手段とし、harness 側識別子（OpenCode session ID 等）は取得可能な場合の付加情報に限定する
- 実行担当サブエージェントは PR 作成時、PR 本文の実行識別情報セクション（`agentdev-workflow-templates` の実行識別情報セクション規約参照）の `adf_delegation` へ委譲単位識別子を転記する。これにより委譲実行と PR の対応付けが機械的に可能になる
...
| 正規参照先 | `canonical_references` | 当該作業で使用すべき解決済み参照先（正規原本、実行時投影、双方確認の別を含む。判別は `agentdev-workflow-lifecycle` スキルの参照先解決ポリシー参照） |
...
inputs:
  structured_context:
    ...
    canonical_references:
      - {当該作業で使用すべき解決済み参照先。正規原本（source）、実行時投影（projection）、双方確認の別を含む。判別は agentdev-workflow-lifecycle スキルの参照先解決ポリシー参照}
    stop_conditions:
...
- canonical_references の目的判別（正規原本確認、実行時投影確認、双方整合確認）は、工程間と共通の参照先解決ポリシー（`agentdev-workflow-lifecycle` スキルの `references/reference-resolution.md`）に従う。本節は判別の内容を重複して定義しない。
...
`writing` category は執筆作業（docs 記述、article 作成、REQ/ Decision/ Design 本文執筆等）を想定した category であり、文章規範等の発火スキルと結合する設計である（文章表層品質の共通検査は textlint 基盤（`agentdev-textlint-guard`）が担う）。
...
委譲起動手段が制限された harness（実行担当サブエージェント型の不許可、起動 API の制限等）で実行担当の接合を起動できない場合、直ちに `delegation-unavailable` と判断せず、能力検出、接合形態の決定、記録の経路に従う（case-run Design「実行担当サブエージェント委譲フェーズ」節の起動不能時経路参照）。
本経路は委譲起動**前**の事前判定であり、後述「委譲起動失敗、異常終了時事後処理」（委譲起動**後**の異常終了に対する事後処理）とは対象段階が異なり、排他的でなく接続する。
配布 SKILL（`agentdev-case-run-execution-adapter` SKILL.md）の委譲契約自体は変更しない（起動手段は harness 責務）。

1. **起動不能の事前判定（能力検出）**: 委譲起動前に、実行担当サブエージェント型の利用可否、起動 API の状態、実行 command の起動可否等の能力を検出する。検出された制限は直ちに `delegation-unavailable` の根拠とせず、次の接合形態決定の入力とする
2. **接合形態の決定**: 能力検出の結果、委譲起動が不可能な場合は、委譲実行者によるプロセス内直接実装を接合形態として決定する。プロセス内直接実装では、result 契約（4状態）、3点ゲート（4状態 result・commit hash・PR URL）、PR 本文 SSoT の委譲契約を self-apply する（Case #3086 で実証済みの履行形態）。これは配布 SKILL の委譲契約の変更ではなく、同一契約の自己適用である。プロセス内直接実装も不可能な場合に限り `delegation-unavailable` とする（実行未試行のため Issue を `pending` に戻す）
3. **記録**: 接合形態の決定事実と実行結果を SSoT に記録する。`completed-pr` の場合は PR 本文（`## Findings / Capture候補` に接合形態がプロセス内直接実装〔self-apply〕である旨を明記）、`blocked`/ `failed` の場合は Issue コメントに構造化記録する。記録方式は後述「委譲起動失敗、異常終了時事後処理」と同一の SSoT 契約に従う

接合形態がプロセス内直接実装に決定された場合の実行（実装、検証、PR 作成）は、委譲契約の通常経路と同一の品質ゲート、test strategy 項目の test-fix ループ、traceability check を適用する。
