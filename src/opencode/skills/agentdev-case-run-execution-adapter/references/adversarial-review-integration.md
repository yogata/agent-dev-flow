...
発動条件の判定は Issue 本文の実行契約（review 発動契約）を正とする。Issue 本文が非発動を記す場合、その契約に従い非発動と判定し、節末「非発動時の判定理由記録と代替自己反証」の記録義務に従う。
...
これらは adapter 委譲内で解決せず、ユーザー判断、REQ 更新プロセス（req-define、再合意済み Definition 変更は case-revise → case-ready）、`/agentdev/issue` コマンド（`agentdev_gh` issue_update）経由の Issue 本文更新等の正規経路へ引き渡すため blocked とする。
...
発動契約非該当で非発動とする場合、実行担当サブエージェントは判定理由を必須記録する。
記録先は case-run 委譲では PR 本文、case-close では対応記録コメント（issue_comment_bug_record.md 系の「adversarial-review 判定」欄）とする。
非発動時は代替としての自己反証を実施・記録し、silent skip を発生させない。代替自己反証の3要素:

- 却下案の検討と却下理由
- 緩和策の検討と適用可否
- unresolved な本質的争点なしの確認

非発動と判定した場合、実行担当サブエージェントは上記3要素を検討し、結果を判定理由とともに同一記録先へ記録する。
発動条件と審議プロトコルの正（adversarial-review Design）は本節で変更しない。
