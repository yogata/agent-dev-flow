...
## ハーネスタイムアウト事後処理（OU-013 / REQ-0130）

ハーネス起動がタイムアウトした場合、即 `failed` とせず**実装完了・検証未完了**として扱い、以下の手順で事後処理する:

1. **プロセス強制終了**: タイムアウト検知時、即座にハーネスプロセス（bunx/npx・oh-my-openagent・派生 node プロセス）を強制終了する
2. **worktree git status 確認**: worktree で `git status`・`git diff --stat` を実行し、未コミット変更の有無を確認する
3. **変更残留時の分類**:
   - **未コミット変更あり**: 実装が進捗している可能性が高い。以下の検証ステップに進む
   - **未コミット変更なし**: 実装が開始されていない・またはハーネスがクリーンアップ済み。`failed` として処理し、Issue コメントに状況を構造化記録する
...
5. **手動修正または PR 化**:
   - 検証が通る場合: 未コミット変更をコミットし、PR を作成して `completed(pr)` として処理する。PR 本文の `## Findings / Capture候補` に「ハーネスタイムアウト・事後処理で PR 化」を記録する
   - 検証が通らない・実装が不完全: `blocked`（回答可能な場合）または `failed`（repository context で回答不能）として処理し、Issue コメントに状況を構造化記録する

タイムアウト事後処理で PR 化した場合、`completed(pr)` の SSoT は PR 本文（他の completed と同じ）。タイムアウト事実は PR 本文の Findings セクションに明記する。
