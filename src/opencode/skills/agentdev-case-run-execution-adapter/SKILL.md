...
case-run が1 Issue 単位（または1 Wave 単位）の実装作業を実行担当サブエージェント（Sisyphus-Junior）へ接続する際のアダプタープロトコル（adapter protocol）を定義する知識ベース。委譲境界および CLI subprocess 廃止・task()（adapter skill + 委譲 prompt 内 `/ulw-loop` command）委譲に基づく。対象を case-run に限定する。
