...
内部管理 ID 除去後に配布物へ残る二次被害（構文破損・主要構造重複・壊れた参照残骸・責務説明矛盾）を検出する。検出パターンと NG 分類は `docs/specs/docs-spec-rebuild-integrity.md` に準拠する。ID 汚染（前節）が 0 件でも本検査は実施する。
...
| 主語/目的語欠落文 | ID 除去により文の主語・目的語が欠落し文意が成立しない | `case-run Step 5 で実行担当サブエージェントへ引き継ぐプロンプトには、以下を必須項目として明記する（OU-012/）。` |
...
NG 分類は推奨アクション（MOVE/ UPDATE 等）とは別軸で付ける。検出事項が属する分類が不明な場合は「要ヒューマンレビュー」と明記する。
