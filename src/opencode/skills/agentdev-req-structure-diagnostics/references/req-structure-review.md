...
### 番号空間衝突の検査観点候補（移行・統合時）

6観点診断に加え、移行・統合で番号空間を専有する文書体系（REQ、Decision、Design）を診断する場合は、次の検査観点候補を確認する:

- 移行先番号空間の実ファイル一覧（inventory）との衝突が検証済みであるか（採番ロジックの内部状態のみに依存した検証になっていないかを含む）
- 採番時と Design 対応表確定時の両方に衝突チェックが前置されているか

衝突が未検証の場合は、問題候補（観点 `DRIFT`、確信度 `medium`、推奨アクション `UPDATE`）として出す。
