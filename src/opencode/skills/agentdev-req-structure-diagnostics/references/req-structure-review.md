...
要件行数、関心分類数、成果物種別数の定量閾値と SPLIT シグナル加算ルールは `docs/specs/req-health-metrics.md`に定義する。
...
`docs/specs/document-model.md` の SPEC Separation Criteria に基づき、現行 REQ の要件テーブル行を対象に、HOW 詳細または内部パラメータが要件行の主たる文意になっていないかを検出する。
...
検出パターンと NG 分類は `docs/specs/docs-spec-rebuild-integrity.md` に準拠する。
...
検出事項には以下の NG 分類を付ける（`docs/specs/docs-spec-rebuild-integrity.md` NG 分類表に準拠）:
