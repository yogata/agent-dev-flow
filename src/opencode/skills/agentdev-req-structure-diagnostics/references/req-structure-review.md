...
要件行数、関心分類数、成果物種別数の定量閾値と SPLIT シグナル加算ルールは `docs/specs/quality/req-health-metrics.md`に定義する。
...
`docs/specs/foundations/document-model.md` の SPEC Separation Criteria に基づき、現行 REQ の要件テーブル行を対象に、HOW 詳細または内部パラメータが要件行の主たる文意になっていないかを検出する。
...
検出パターンと NG 分類は `docs/specs/integrity/docs-spec-rebuild-integrity.md` に準拠する。
...
判定の SSoT 優先順位: 現行 REQ（REQ-0130〜REQ-0133, REQ-0114）> 承認済み ADR（ADR-0114, ADR-0125, ADR-0127, ADR-0128）> SPEC（`docs/specs/commands/`, `docs/specs/workflows/epic-wave-model.md`）> command 本体 > skill。

### NG 分類

検出事項には以下の NG 分類を付ける（`docs/specs/integrity/docs-spec-rebuild-integrity.md` NG 分類表に準拠）:
...
- 検査対象は配布物のみ。`docs/`、`scripts/`、`.agentdev/`、`.sisyphus/` は対象外。
...
SPEC は 3 層構造（`docs/specs/commands/`、`docs/specs/skills/`、`docs/specs/workflows/`）を持ち、横断 SPEC（`workflows/`）は個別 command/skill の現在動作を含まない（OU-002 原則）。
これに違反する配置を検出する。
