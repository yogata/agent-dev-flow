# REQ Structure Review

REQ structure 診断の判定ロジック（REQ参照ID整合性・第一参照導線・active/retired/世代境界・6観点診断・未処理artifact確認・診断結果出力スキーマ）を定義する。各判定は read-only であり、ファイル変更や artifact 処理は行わない。
...
`docs/specs/document-model.md` の SPEC Separation Criteria に基づき、active REQ の要件テーブル行を対象に、HOW 詳細または内部パラメータが要件行の主たる文意になっていないかを検出する。検出時の観点は `MOVE`、推奨アクションは `MOVE` または `UPDATE` とする。
...
- active REQ の要件テーブル行を優先して確認する。目的・適用範囲に同種の HOW 詳細があり、要件行の解釈に影響する場合は補助根拠として扱う。
- 単語の出現だけではなく、当該行の主たる文意が SPEC・rule catalog・command reference・skill reference・test docs に置くべき詳細かを判定する。
- 要件行が外部契約を要約し、詳細値だけを例示している場合は安定契約例外候補として扱い、即時 high finding にしない。
- 同一 REQ 内で複数種の SPEC分離基準違反シグナルが出た場合、根拠をまとめて1件の MOVE finding 候補として出す。
...
| 根拠 | 検出されたシグナルの具体的内容 |
| シグナル数 | 検出されたシグナル数（通常は2以上で finding 化、SPEC分離基準違反の high-specificity signal は1以上で finding 候補化） |
