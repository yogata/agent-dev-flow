...
- 配布物統合性検出（docs-spec-rebuild-integrity SPEC 準拠）: ID 除去後の構文健全性（frontmatter 重複、見出し重複、Markdown 構文破損）、文意保持（壊れた括弧、壊れた参照表現、主語/目的語欠落文）、責務整合（command 本体と SPEC 間の責務説明照合、case-open/run/close/auto の責務境界一致、command と関連 skill 間の責務説明照合）、NG 分類（false positive/ pre-existing/ 今回修正対象）
