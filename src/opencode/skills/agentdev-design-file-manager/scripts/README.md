# `agentdev-design-file-manager` scripts

Design 固有の決定的処理スクリプト群（AG-{NNN}/006、design-principles.md 第5節）。
...
├── tsconfig.json
├── lib/
│   ├── result.ts                    # 結果型と stdout/stderr ヘルパー（Design 固有のみ）
│   └── fs-helpers.ts                # ファイル I/O ヘルパー（Design 固有のみ）
├── src/
│   └── search-target-area.ts        # Design ファイル内 target_area 見出し検索
└── tests/
...
# 個別スクリプト実行
bun src/search-target-area.ts "target_area文字列" ../../../docs/designs/{domain}/<existing-spec>.md
```
