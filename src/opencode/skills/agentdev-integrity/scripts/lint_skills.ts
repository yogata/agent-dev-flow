...
const SCRIPT_USAGE = "bun run .opencode/skills/agentdev-integrity/scripts/lint_skills.ts [--help] [--json] [--dry-run]";
...
function lintSkill(
  ...
  if (!fs.existsSync(skillMdPath)) {
    results.push(ng("Structure", "SKILL.md existence", `SKILL.md not found in ${dirName}`, dirName));
    // Cannot continue without SKILL.md
    return results;
  }
  results.push(ok("Structure", "SKILL.md existence", `${dirName}/SKILL.md exists`));

  const content = fs.readFileSync(skillMdPath, "utf-8");
  const frontMatter = parseFrontMatter(content);

  // Check 2: Directory naming vs frontmatter name
  if (frontMatter) {
    ...
    if (fmStripped !== dirStripped) {
      ...
      results.push(ok("Structure", "Directory naming vs frontmatter name", `${dirName} matches frontmatter`));
    }
  } else {
    results.push(warn(
      "Structure",
  ...
  // Check 3: Description presence
  if (frontMatter && frontMatter.description.length > 0) {
    results.push(ok("Content", "Description presence", `${dirName} has description`));
  } else {
    results.push(ng("Content", "Description presence", `Missing or empty description in ${dirName}/SKILL.md frontmatter`, dirName));
  }

  // Check 4: USE FOR section
  if (hasSection(content, "USE FOR")) {
    results.push(ok("Content", "USE FOR section", `${dirName} has USE FOR section`));
  } else {
    results.push(warn("Content", "USE FOR section", `Missing USE FOR section in ${dirName}/SKILL.md`, dirName));
  }

  // Check 5: DO NOT USE FOR section
  if (hasSection(content, "DO NOT USE FOR")) {
    results.push(ok("Content", "DO NOT USE FOR section", `${dirName} has DO NOT USE FOR section`));
  } else {
    results.push(warn("Content", "DO NOT USE FOR section", `Missing DO NOT USE FOR section in ${dirName}/SKILL.md`, dirName));
  }

  // Check 6: See Also references
  ...
  if (seeAlsoRefs.length > 0) {
    ...
    if (brokenRefs.length > 0) {
      ...
      results.push(ok("Content", "See Also references", `${dirName} See Also references all valid`));
    }
  } else {
  ...
  if (mentionedResources.length > 0) {
    ...
    if (allExist) {
      results.push(ok("References", "Resource references", `${dirName} referenced resources all exist`));
    }
  } else {
    results.push(ok("References", "Resource references", `${dirName} has no resource references to check`));
  }

  // Check 8: SKILL.md bloat (>500 lines)
  ...
  if (lineCount > 500) {
    ...
    results.push(ok("References", "SKILL.md bloat", `${dirName}/SKILL.md has ${lineCount} lines (within 500 line limit)`));
  }

  // Check 9: agentdev- prefix
  if (dirName.startsWith("agentdev-")) {
    results.push(ok("Namespace", "agentdev- prefix", `${dirName} uses agentdev- prefix`));
  } else {
    results.push(info("Namespace", "agentdev- prefix", `${dirName} does not use agentdev- prefix`, dirName));
  }
...
function main(): void {
  ...
  const report: IntegrityReport = {
    ...
    script: SCRIPT_NAME,
    scanned: { skills: skillDirs.length },
    summary,
    results: allResults,
  };
