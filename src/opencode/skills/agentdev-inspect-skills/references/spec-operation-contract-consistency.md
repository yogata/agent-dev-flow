...
> **原本**: 本ファイルは inspect-skills 診断観点「SPEC 操作契約テーブル ↔ references/contracts.md フィールド一致性」（REQ / AG-{NNN}）の判定基準詳細、対象 SPEC 範囲、フィールド対応規則を集約する運用ビューである（REQ 準拠）。
...
両者は独立ファイルとして維持され（CR-{NNN}: 単一情報源化は導入しない）、本診断は両者の不一致を検出事項として報告するのみで、ファイル修正、自動再同期は行わない。
...
両者を単一情報源へ統合する生成スクリプト、ビルドステップ、自動同期機構は導入しない（CR-{NNN}）。
