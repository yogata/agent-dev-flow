...
> **原本**: 本ファイルは inspect-skills 診断観点「SPEC 操作契約テーブル ↔ references/contracts.md フィールド一致性」の判定基準詳細、対象 SPEC 範囲、フィールド対応規則を集約する運用ビューである。
...
`docs/designs/` 配下を走査し、`## 操作契約` 見出しを含む SPEC ファイルを特定する。
