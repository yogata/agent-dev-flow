...
文書中の `/agentdev/*` や `/ulw-loop` 等の command 名が出現する文脈で、「スキル」「skill」という表記が併用されていないかを確認する。機械的検出（IR-051）と重複するが、本診断は文脈判断を含む。
...
文書中の `oh-my-openagent`（harness）や `Sisyphus-Junior`（subagent）等の名前が出現する文脈で、「スキル」「skill」という表記が併用されていないかを確認する。機械的検出（IR-051）と重複するが、本診断は文脈判断を含む。
...
委譲元、委譲先の記述で実行主体の分類が一貫しているかを確認する。例: case-run は command、Sisyphus-Junior は subagent、agentdev-case-run-execution-adapter は skill、oh-my-openagent は harness。これらの分類が委譲契約記述全体で一貫しているかを検査する。

## 出力形式

検出した誤認は SKILL.md「出力形式」セクションの Finding 形式で報告する。Classification には `execution-subject-misclassification` を使用する。Recommended route には `command` / `skill` / `references` のいずれかを提示する（誤認の修正先が command 定義、skill 本文、references のいずれかによる）。
