...
import { createFixture, REQ_001_NODE, DEC_001_NODE } from "./fixture.ts"
import {
...
describe("TS-{NNN}: fixture repo → build → check → query", () => {
  it("builds 5 files, check passes, queries respond", async () => {
    ...
    expect(neighbors.nodes).toContain(DEC_001_NODE)

    const provenance = await queryGraph(graph, { kind: "provenance", id: REQ_001_NODE })
    expect(provenance.provenance.length).toBe(1)
  })
})

describe("TS-{NNN}: default indexed_paths (3 only) and node_types (3 only)", () => {
...
describe("TS-{NNN}: works without project augmentation", () => {
  it("build, check, query all succeed with no augmentation file", async () => {
    ...
    expect(path.nodes[0]).toBe(REQ_001_NODE)
    expect(path.nodes.at(-1)).toBe(featureSpecNode)
  })
})

describe("TS-{NNN} (partial): provenance completeness", () => {
