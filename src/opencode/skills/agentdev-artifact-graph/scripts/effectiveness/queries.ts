...
/**
 * Real producer artifact IDs are generated by the production formatters so
 * this diagnostic continues to query the live self-hosting repository graph
 * without embedding concrete IDs in the distributed source.
 */
const REQ_012 = formatReqId(12)
const REQ_013 = formatReqId(13)
 ...
 * Graph からは見えない）。一方、独立探索 (rg \bREQ-{NNNN}\b) は文字列表現を全件
...
const Q5_SUPERSEDED_CURRENT_REFS: EffectivenessQuery = {
  ...
  groundTruthRationale:
    `Graph 上で decision:DEC-{NNN} へ入ってくる references エッジの source は ` +
    `docs/decisions/README.md の索引行のみ（markdown link で DEC-{NNN} を明示）。` +
    `supersedes の source である DEC-{NNN} 自身は除外。` +
    `独立探索は追加で DEC-{NNN}.md 本文中の言及も拾うが、後継 Decision からの言及は` +
...
/**
 * 全 6 query の代表 suite。
 * REQ-{NNNN}-006 が列挙する 6 category と 1:1 対応する。各クエリは real artifact 参照を持ち、
 * mock/stub は一切使用しない。
 */
