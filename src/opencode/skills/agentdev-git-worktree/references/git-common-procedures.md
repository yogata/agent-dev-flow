...
2. **明示パスステージ + `git commit -- <paths>` の義務付け**:
...
共有作業ツリーで git 操作を行う全 command（case-open, case-ready, case-revise, case-close, learning-promote, backlog-review, intake-capture, intake-from-github, intake-promote, inspect-docs, inspect-skills, inspect-promote）は本プロシージャに従うこと。
...
他セッションが main ブランチを checkout している場合も ref の占有で失敗する。
...
| リスク3（main 以外のブランチ占有） | `git fetch origin main:main` による非チェックアウト同期（後述）。現在のブランチを維持したまま main ref を更新 |
...
現在 checkout 中のブランチを切り替えずにローカルの main ref を更新する。
worktree が main 以外のブランチを保持している場合や、他セッションが main ブランチを占有している場合に安全な同期手段。

**制約**: ローカルの main ブランチがリモートから分岐している場合（独自 commit がある場合）は non-fast-forward エラーとなる。
...
### エラー: リスク3 main 以外のブランチ占有・fetch 失敗

```markdown
## Git 同期リスク検出エラー（main 以外のブランチ占有・fetch 失敗）

**現在ブランチ**: {current_branch}
**停止理由**: main 以外のブランチ占有を検出し `git fetch origin main:main` を試行したが、ローカルの main ブランチがリモートから分岐しているため non-fast-forward エラーとなった
**ユーザーアクション**: ローカルの main ブランチの分岐状況を確認し、手動で同期してください
**raw git output**:
{git_error_output}
```
...
- 「`agentdev-git-worktree` の git 同期リスク事前検出プロシージャに従い、3リスクの事前検出と代替同期手順選択を実行」

---

## 10. Windows git 出力のエンコーディング処理

Windows PowerShell 環境で git の外部コマンド出力を処理する場合の注意事項。
PowerShell はネイティブコマンド（git、gh）の UTF-8 出力をコンソールコードページ（cp932）でデコードするため、パイプ・変数キャプチャ・リダイレクト経由の文字列処理で mojibake と静かな情報欠落が発生する（Issue #2634 の4障害事例由来）。

### 手順

1. **OutputEncoding の UTF-8 設定**: git 出力を PowerShell で文字列処理する前に、`[Console]::OutputEncoding` を UTF-8 に設定する
2. **件数クロスチェック（必須）**: 出力の文字列解析（grep、行数カウント、文字列突合等）の結果に対し、生バイト系コマンドによる件数クロスチェックを実行する。例: 文字列解析で N 件を検出した場合、`git grep -c` 等の件数カウントと突合する
3. **欠落時の取扱い**: 件数が一致しない場合、文字列解析の結果を検証結果として採用せず、出力の受け取り方法を変更する（UTF-8 明示のファイル退避、Read ツール参照等）

### 静かな欠落を疑う運用指針

エンコーディング破損と情報欠落は例外を投げず、部分的な成功として通過する。
検証・比較系タスクでは、取得文字数と件数の突合を検証手順に含める。
期待値との突合ができない出力は、単体で検証結果の根拠としない。

### gh 出力の取り扱い（重複回避）

gh 出力のエンコーディング防御手順（`gh api --input` 経路、`--json` 全体取得 + `ConvertFrom-Json` パース、UTF-8 明示ファイル退避・書き戻し）は `agentdev-workflow-case-run` の「Windows PowerShell 外部 CLI 出力のエンコーディング防御」（`references/single.md` の関連ガイドライン）を参照し、本プロシージャでは繰り返さない。
Issue/PR 操作は Custom Tool `agentdev_gh` を標準とし、`gh` CLI を直接実行する場合に限り上記防御手順を適用する。
...
- 「`agentdev-git-worktree` の squash merge 後分岐ハンドリング手順に従い、ローカル先行 commit 検出、内容重複確認、reset を実行」
...
`git fetch origin` 後、main の remote ref を基準に `git rebase origin/main` を実行し機械的解消を試みる（rebase プロシージャは worktree-operations.md「Merge Conflict 対応パターン」参照）。
rebase 基準は PR の squash merge 先（main）と同一である。
