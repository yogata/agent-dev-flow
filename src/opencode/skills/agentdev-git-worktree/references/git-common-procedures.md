...
共有作業ツリー（main worktree）で複数セッションが並行実行される環境で、他セッションの変更を巻き込み、破壊しない git ステージ、コミット規律を定義する。`case-run` の worktree 隔離フェーズ（専用 worktree + branch で index が独立）は構造的に安全なため本プロシージャの対象外とする。
...
**重要**: 重複なしの場合でも、ローカル未コミット変更が存在する場合は後続の「1. 実行前同期」プロシージャが改めて検出、停止する。本チェックは PR 対象ファイルとの重複に特化した早期警告であり、既存のローカル変更検出を置き換えない。
