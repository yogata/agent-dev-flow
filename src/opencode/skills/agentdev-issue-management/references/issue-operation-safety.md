...
**単一書き手制約（ADR-0125）**: Epic Issue 本文ステータス追跡テーブルの更新は **case-close のみ**が実施する。case-run は Epic Issue 本文を読み取るのみ（書き込まない）。case-auto は Wave 反復制御のみを行い、Epic Issue 本文に直接書き込まない。last-write-wins 競合防止は case-close の単一書き手で維持される。
...
- **Epic Issue 本文ステータス追跡テーブルの更新を case-close 以外のコマンド（case-run・case-auto）が直接行うこと禁止**（ADR-0125 単一書き手制約）。case-run は読み取りのみ、case-auto は Wave 反復制御のみとし、Epic Issue 本文への書き込みは case-close に限定する
