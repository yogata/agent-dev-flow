...
**upstream handoff metadata 付与**: artifact が AgentDevFlow 本体の不具合・改善点を扱う場合、分析結果に `agentdev_handoff: true` を記録する。判定は `agentdev-workflow-lifecycle/references/upstream-handoff.md` に従う
...
2. 各 RU について:
    ...
    - **upstream handoff metadata 転記**: 分析結果に `agentdev_handoff: true` が存在する場合、RU frontmatter に転記する。RU 本文に現在プロジェクトでは実装しない upstream handoff 用 RU であることを記載する
