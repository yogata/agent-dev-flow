...
本ファイルは `agentdev-learning-capture` SKILL.md の補助資料であり、捕捉対象カテゴリと具体例、学び抽出〜通知の手順実例、 Tips、 pipeline 全体像を扱う。SKILL.md 本文では実観測原則、 trigger、禁止事項、 split rule のみを提示し、具体手順と記入例は本ファイルを参照する。
...
学びがあると**エージェントが判断**した場合、**エージェントが**基準テンプレート（`references/capture-entry-template.md`）に従って13フィールド形式でエントリを生成する。全13フィールドを必ず含めること。エージェントは推論でフォーマットを組み立てず、基準テンプレートから取得すること。
...
学びの追加は通知前に実行される。ユーザー承認を待たずに直接追記する。
