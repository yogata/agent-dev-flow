...
- `.agentdev/drafts/**` 配下のファイル作成・更新のみ（ファイル編集スコープ制約）。`git` コマンドは実行しない（git コマンド実行禁止制約）
...
| STEP-2 | 壁打ち対話（引き継ぎ判定含む） | 入力ソース確定 | 壁打ちで確定した要件内容、`agentdev_handoff` 判定 | [references/input-and-dialogue.md](references/input-and-dialogue.md) |
...
- **session由来RU**: STEP-1/2 で `source_type: chat` かつ `generated_by: session` のRU を受領した場合、session由来RU 消費契約（一時成果物ライフサイクル要件 + artifact-contracts Design が正規原本）に従う
...
- REQ の保存内部責務（`agentdev-req-file-manager` 委譲）は Design 対応、実装対応、検証対応のいずれかを作成する責務を持たない。新規要件の保存時点でこれらの対応が存在しないことを理由に保存を失敗させない
- 保存対象の要件行のうち検証対応を任意とする行がトレーサビリティポリシーへ明示登録されているかを保存結果に記録できる。未登録（検証対応必須扱い）であることを理由として保存を失敗させない
