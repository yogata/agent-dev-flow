...
1. **前提とする固定知識の範囲**: docs/ ディレクトリ構成（requirements/decisions/specs）と req-define command の公開契約のみを前提とする。Design ディレクトリの内部構成は仮定しない
...
- **実証Case判定・評価契約確定**: 実証必要性の推論・提案と評価契約の確定を要件展開工程の一部として実行する。実証Caseと評価契約の意味論は評価ブランチ実証ワークフロー要件が正規所有し、本スキルは実行位置と手順を提供する。実証Caseでも Git 副作用を持たない（評価ブランチ・worktree 準備の実行主体・手順は req-define command Design（extension 経由）が所有する）
