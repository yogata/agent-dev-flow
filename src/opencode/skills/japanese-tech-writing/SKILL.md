...
description: "日本語の技術文書・書籍原稿の文章規範。整形（一文一行、引用ブロック、脚注、コラム記法）、パラグラフライティング、論証の厳密さ、読み手の負荷の管理、視点と語り、演出の抑制、LLM っぽい空句の禁止、冗長の排除を定める。USE FOR: 日本語で技術書の章・草稿・記事・解説文を執筆するとき、日本語原稿を推敲・リライトするとき、文章規範の原本参照。DO NOT USE FOR: 英語文書の執筆、コード実装やテスト実行、REQ/ADR/SPEC の品質査読、文書種別の配置基準や用語政策。"
---
