...
- 全 work_type 共通の docs 更新責務（bugfix も含む。REQ-0104-034）
...
| case-auto | req-save → case-open → case-run → case-close を自走（明示指定時のみ） |
...
| intake-from-github | クローズ済みissue/PRから残課題抽出・分類・intake item 生成 |
| intake-promote | inbox 内の intake item をレビュー・採用・却下・保留判定し、promoted artifact に整形 |
| backlog-review | promoted artifact（intake/learning）の分析・HITL・RU 生成・git永続化 |
| learning-promote | inbox.md と archive/active.md をセマンティック分析・evaluation-report 出力・昇華判定し promoted artifact 生成 |
| docs-check | AgentDevFlow artifact 整合性検査（self-hosting repo 専用） |
| docs-review | docs 全体の意味整合レビューと REQ 再構成診断（`/agentdev/docs-review`） |
