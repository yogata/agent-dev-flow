...
- `agentdev_handoff: true` は AgentDevFlow 本体向け handoff marker である。
...
AgentDevFlow 本体改善要求を現在プロジェクトの通常要件docとして定義せず、upstream handoff 用 RU 入力として整理する。出力に `agentdev_handoff: true` を含める。

### backlog-review

AgentDevFlow 本体改善 artifact に upstream handoff metadata（`agentdev_handoff: true`）を付与する。

### backlog-review

upstream handoff metadata（`agentdev_handoff: true`）を RU frontmatter に転記する。RU 本文に現在プロジェクトでは実装しない upstream handoff 用 RU であることを記載する。
