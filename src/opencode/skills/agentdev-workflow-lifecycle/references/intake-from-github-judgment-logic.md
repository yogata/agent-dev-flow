# Intake from GitHub Judgment Logic

<!-- 元コマンド: intake-from-github.md -->
<!-- 抽出日: 2026-06-07 -->

intake-from-github コマンドの Steps 1〜5 における抽出ロジック（期間解釈・構造的検出・LLM全文解析・intake item生成）を定義する。

## Step 1: 期間解釈

ユーザーの自然言語による期間指定を解釈し、GitHub CLI の検索クエリ用日付範囲（`since` / `until`）に変換する。現在日付は実行時のシステム日付を使用する。
...
取得した Issue/PR の本文およびコメントから、未チェックのチェックボックス（`- [ ]` または `* [ ]`）を構造的に抽出する。チェック済み（`- [x]`）は除外。
