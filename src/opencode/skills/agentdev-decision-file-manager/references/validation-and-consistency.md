...
本チェックリストの見出し（`accepted Decision 直接編集チェックリスト`）は歴史的経緯により ADR という語を含まない `accepted Decision` 表記を採用する。これは本スキルが ADR から Decision へ移行した成果物であることによる（AG-010）。v2:ADR-* 形式の過去参照は原本 SPEC 側で保持する。
