...
## Frontmatter 規約（ADR-0102, REQ-0103-044, Case 5 / RU-0020）
...
## Runtime Path 規約（ADR-0102, ADR-0104）

Command本文でパスを参照する際、runtime path を使用する。Source path は開発時のみ有効であり、個別プロジェクトでの実行時には存在しない。
...
Commandの Steps / Guardrails 内で template や reference のパスを記述する際は、必ず `.opencode/skills/...` で始めること。`src/opencode/...` は runtime 環境に投影されない（ADR-0105）。
...
- [ ] パス参照が runtime path（`.opencode/...`）を使用しているか（`src/opencode/...` は禁止）
