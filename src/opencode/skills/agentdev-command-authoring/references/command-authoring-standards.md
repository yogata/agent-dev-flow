...
Command本文でパスを参照する際は runtime path を使用する。Source path は agent-dev-flow 開発リポジトリ内でのみ有効であり、個別プロジェクト（consumer repository）には投影されない（ADR-0102, ADR-0104, ADR-0105）。
...
### Runtime Path 確認（ADR-0102, ADR-0104）
...
- **ADR-0107**: Command/Skill/Template/Script責任分界の正式定義
