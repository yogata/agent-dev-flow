...
- `implementation_pattern` / `secondary_pattern` / `load_skills` 等の dev メタデータは frontmatter に記述禁止（Case 5 / RU-0020, 規定の行番号範囲）
...
| 手法 | 説明 | 適用基準 |
|------|------|---------|
| 同型分岐の共通化 | 個別 step への重複記述ではなく、共通手順 + 差分表で表現（SHOULD） | 同じ構造の分岐が2箇所以上 |
| 表現の削減 | セマンティクス不変で、理由説明の過多・例の過多・SHALL/MUST の反復・長文表現を削減（SHOULD） | 同一意図の反復表現あり |
| セマンティクス維持確認 | 薄型化後にセマンティクス維持の確認メモを残す（SHALL）。薄型化は REQ-0111-004 に従う | 全 token 削減作業 |

### before/after 報告形式

token 削減の成果は以下の3区分で報告する（SHALL）:
...
既存コマンドを拡張する場合、既存の Step 内容を変更せず、分岐点のみで新規パスを注入する（SHALL）。
...
ディレクトリのリネームには `git mv` を使用し、Write/Delete による実装を禁止する（SHALL）。
...
Command本文でパスを参照する際は runtime path を使用する。Source path は agent-dev-flow 開発リポジトリ内でのみ有効であり、個別プロジェクト（consumer repository）には投影されない。

### Runtime path と Source path の対応

| 用途 | Runtime path（✅ 使用する） | Source path（❌ 使用しない） |
|------|---------------------------|---------------------------|
...
- [ ] セマンティクス不変で削減可能な反復表現（理由説明・例・SHALL/MUST反復・長文）がないか
...
- [ ] `load_skills` は frontmatter に記述していないか（禁止）
- [ ] `implementation_pattern` / `secondary_pattern` は frontmatter に記述していないか（禁止）
- [ ] command 本文で参照している Skill が command-map.md の pattern 定義と整合しているか

### load_skills Prompt Budget

- [ ] `load_skills` が frontmatter に混入していないか（禁止）
- [ ] `implementation_pattern` / `secondary_pattern` が frontmatter に混入していないか（禁止）
...
| 判定 | 記述例 | 理由 |
|------|--------|------|
| ✅ OK | `.opencode/skills/{name}/templates/close-report.md` | runtime path（投影先でアクセス可能） |
| ✅ OK | `.opencode/commands/agentdev/case-close.md` | runtime path（commandの実際の配置場所） |
| ❌ NG | `src/opencode/skills/{name}/templates/close-report.md` | source path（runtime 環境に存在しない） |
| ❌ NG | `src/opencode/commands/agentdev/case-close.md` | source path（開発リポジトリ内のみ有効） |

- [ ] Command本文内のパス参照が runtime path（`.opencode/...`）を使用しているか
...
- [ ] 廃止済みスキル（`agentdev-workflow-reporting`）を参照していないか
...
- [ ] 委譲定義を追加する場合、最小構成（delegation_type, skill, reference, inputs, on_result）を満たしているか
- [ ] delegated_check を使用する場合、親コマンドが検証結果の最終判断を保持しているか
- [ ] サブエージェント出力を中間成果として扱い、verbatim 期待を記述していないか
- [ ] SPEC/REQ/ADR 文書への置換は verbatim 置換方針に従っているか

## 検査観点展開基準

Command に検査観点の詳細を展開すべきでない基準（ADR-0112 §4）。検査観点は Skill reference に分離し、Command は委譲定義のみを持つ。
...
- 委譲先 Skill 名と reference path
- 委譲種別（delegation_type）
- 入力と出力の定義
- on_result（採否方針）
...
Command に委譲を定義する際の最小構成（ADR-0112 §5）:
...
skill: {skill-name}
reference: {skill-reference-path}
inputs:
  - name: {input-name}
    description: "{input-description}"
on_result: {adopt|reject|adopt_partial}
```
...
| フィールド | 必須 | 説明 |
|---|---|---|
| `delegation_type` | ✅ | ADR-0112 の taxonomy から選択 |
| `skill` | ✅ | 委譲先の skill 名 |
| `reference` | ✅ | 委譲先が参照する skill reference / SPEC section の runtime path |
| `inputs` | ✅ | 委譲先に渡す入力の定義（最低1つ） |
| `on_result` | ✅ | 親コマンドの採否方針 |

### 禁止事項

- Command に委譲先の判定ロジックを記述しない
- `inputs` に自由記述のプロンプトを含めない（構造化フィールドのみ）
- `on_result` を省略しない（親コマンドが最終判断を保持することを明示）
...
- **入力**: {検査対象}
- **on_result**: 採用・修正採用・却下の判断は本コマンドが行う
- **制約**: 検証結果の最終判断は本コマンドが保持する（ADR-0112 §4）
```
...
| 結果の中間成果扱い | サブエージェント出力は中間成果（ADR-0112 §6）。全体採用・部分採用・却下が可能 |
| 禁止: 結果の自動承認 | delegated_check の出力を自動的に最終結果として扱わない |
| 禁止: 副作用の委譲 | 検証結果に基づくファイル変更・git操作は親コマンドが実行 |

## 中間成果基準

サブエージェント出力を中間成果として扱う基準（ADR-0112 §6）。

### 基準

| 基準 | 説明 |
|---|---|
| 採否判断の明示 | 委譲定義の `on_result` で採否方針を宣言 |
...
- サブエージェント出力を verbatim でそのまま最終成果として扱わない
- 採否判断なしに出力をファイルに書き込まない
- 中間成果の品質評価をサブエージェントに委譲しない（親コマンドの責務）

## verbatim 置換方針

サブエージェント出力によるファイル置換の方針（ADR-0112 §6）。
