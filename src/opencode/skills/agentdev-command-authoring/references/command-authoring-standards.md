...
Command本文でパスを参照する際は runtime path を使用する。Source path は agent-dev-flow 開発リポジトリ内でのみ有効であり、個別プロジェクト（consumer repository）には投影されない（ADR-0013, ADR-0018, ADR-0019）。
...
### Runtime Path 確認（ADR-0013, ADR-0018）
