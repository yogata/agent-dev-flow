...
Command のトップレベル Step は整数のみで表記する。小数 Step と英字サブステップは使用しない。
...
サブステップは、親 Step 内の局所的な確認・VERIFY・分岐を表す場合に限り `N-M` 形式で使用できる。独立した進行判断、保存、Issue / PR 更新、commit、push、ユーザー確認を伴う処理はトップレベル Step へ昇格する。
