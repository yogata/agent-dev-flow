...
本スキルの references には専門診断の判定ロジック（SPLIT シグナル閾値、REQ 参照 ID 整合性の具体的検出手順、文章表層規則（textlint 規則構成）等）を再定義しない。
...
| guides 意味診断 | 横断契約矛盾の抽出（本スキル直接判定） | 該当なし（本スキル直接判定） |
...
本スキルと他の検査能力との責務重複がないことを、以下の境界で保証する。
