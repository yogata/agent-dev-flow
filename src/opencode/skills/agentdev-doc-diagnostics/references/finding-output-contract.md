...
専門 skill の内部スキーマを再定義せず、変換契約のみを本節で定義する。
...
- 横断契約矛盾で上位文書（REQ、承認済み Decision）を正として下位文書が矛盾する場合は high
...
- 配布物の存在しない command 参照は README listing と command 本文から `/agentdev/` 参照を抽出し、実在 command 一覧と照合する機械的判定のため high
- 配布物のエンコーディング不整合（UTF‑8 BOM の有無、CRLF/LF 混在）は先頭バイト列と改行コードの出現集合から機械的に一意に判定できるため high
...
- 検出事項ファイルは `.agentdev/inspect/inbox/` に配置し、`inspect-promote` への入力となる
- reject された検出事項は即時削除する
- promote された検出事項は `.agentdev/inspect/promoted/` へ保存し、`backlog-review` の入力となる
- 検出事項の source_type は `inspect`
...
2. 承認済み Decision（`status: accepted` の Decision）
3. Design（`* Design`）
4. guides（補助文書）
...
本スキルが関与する診断結果の副作用は以下のみを許可する（inspect lifecycle 準拠）。
