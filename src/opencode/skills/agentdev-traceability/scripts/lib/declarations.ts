...
// - 宣言形式は DECLARATION_RE（下記定義）が規定する。
//   role は decision / design / implementation / verification、
//   REQ-ID は REQ-{NNNN}-{MMM}（TIM の標準成果物役割 4種）
// - 1ファイルに複数の宣言行を含められる。解析結果は和集合とする
// - マーカー文字列自体はファイル種別に依存しない
...
export interface CoverDeclaration {
  ...
  /** 対応対象成果物のリポジトリ相対パス（inline / sidecar 正規化後も同一軸）。 */
  readonly file: string;
  /** 宣言位置。inline は 1-based 行番号、sidecar 正規化分は 0（位置情報を保持しない）。 */
  readonly line: number;
  /** 正規化前の直列化種別（同一論理対応関係の正規化、重複整合検証の情報源区分）。 */
  readonly source: "inline" | "sidecar";
  /** 宣言を保持するファイル（inline は file と同一、sidecar は sidecar パス）。 */
...
const DECLARATION_RE =
  /ADF-COVERS\((decision|design|implementation|verification)\):\s*(REQ-\d{3,4}-\d{3}(?:\s*,\s*REQ-\d{3,4}-\d{3})*)/;
 ...
 * - 正規位置の行が対応宣言マーカーと開き括弧を持つ形状（括弧内は識別子）だが完全形式でない場合、ロールが既知4種なら
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isCoverRole(role)) {
      ...
      issues.push({
        ...
        line: i + 1,
        text: raw.trim(),
        detail: `未知の成果物役割 ${role}（既知: decision / design / implementation / verification）`,
      });
    }
  }
