...
`agentdev-workflow-templates` の選定ルールに従いコメント用テンプレートを読み込み、`agentdev_gh` の comment_create 操作でコメントを追加する（Epic flow では Epic Issue にコメント追加）→ VERIFY。
...
- Custom Tool `agentdev_gh`: コメント追加（comment_create）
