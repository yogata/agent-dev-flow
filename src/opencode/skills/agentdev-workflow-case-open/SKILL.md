...
1. **前提とする固定知識の範囲**: docs/ ディレクトリ構成（requirements/decisions/specs）と case-open command の公開契約のみを前提とする。`docs/specs/**` 内部構成（`foundations`, `responsibilities` 等）は仮定しない
...
- Issue 本文生成と execution contract 確定（EC-1〜EC-8）
...
- QG-2 完了条件網羅性検証（`agentdev-quality-gates`）
...
| STEP-2 | Issue本文生成・execution contract 確定 | 処理対象確定 | Issue 本文候補（EC-1〜EC-8 反映済み、QG-2 検証済み） | [references/issue-body-and-execution-contract.md](references/issue-body-and-execution-contract.md) |
...
- `agentdev-learning-capture` / `agentdev-intake-pipeline`: deviation capture 委譲（STEP-2/5 で実観測時）
