...
| スクリプト | 役割 | 入力 | 出力 JSON |
|-----------|------|------|-----------|
| `check-frontmatter-consistency.ts` | frontmatter id ↔ ファイル名整合性（REQ/Decision 横断） | argv[2]=dir, argv[3]=kind(req\|decision、adr はレガシー互換) | `{ ok, errors[], warnings[] }` |
