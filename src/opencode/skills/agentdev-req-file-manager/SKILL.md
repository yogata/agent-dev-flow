...
- **補助セクション（任意）**: `SPEC候補` — req-define が REQ 要件行候補から分離した SPEC 相当行と想定配置先 SPEC を記載（REQ-0136-009）。req-save が REQ ファイル保存時に本セクションを除去し、内容は `draft-meta.spec-candidates` 経由で spec-save が消費する（ADR-0123）。最終 REQ ファイルに本セクションは残さない
