...
配置先: `src/opencode/skills/agentdev-req-file-manager/scripts/`（REQ）。
...
これにより REQ番号採番、ADR番号採番、要件行ID採番、frontmatter 整合性確認、エントリ存在確認、変更範囲検証、target_area 検索を LLM 推論ではなく機械的に実行する（design-principles.md 第5節「決定的処理の Script 委譲原則」）。
