...
- **SC-001**: {Design候補の内容}
 - 想定配置先Design: {`docs/designs/{domain}/{file}.md`、または「新規Design候補: target_design: {operation, domain, slug}」}
 - 分離根拠: { の Design分離基準該当種別（schema field/ enum 値一覧/ 判定表/ fixture detail/ retry 回数/ Step 番号 等）}
