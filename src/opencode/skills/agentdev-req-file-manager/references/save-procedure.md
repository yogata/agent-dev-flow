...
Definition 保存内部責務が req-define のドラフトを REQ / Decision ファイルとして保存する際の詳細手順を定義する。
...
Definition 保存内部責務が扱う operation type は REQ 操作（`create` / `append` / `update`）のみとする。
Design 操作（`artifact: design`、operation `create` / `append` / `update`）は `target_design` フィールドで識別され、Design 保存内部責務が消費するため Definition 保存内部責務の対象外とする。
...
Definition 保存内部責務が Decision を CREATE する場合、frontmatter の標準フィールド `related_reqs` に、要件doc（draft-data）の該当 Decision 対象操作で確定した関連 REQ を決定的に保存する。
...
本工程では保存可能範囲を実行し、完了報告の follow-up として明示する。
...
RU は一時成果物であり、削除は case-ready 成功後にのみ実行する。blocked / failed / 中断時は保持する。
