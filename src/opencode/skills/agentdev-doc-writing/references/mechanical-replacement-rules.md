<!-- ADF-COVERS(implementation): REQ-053-023 -->
# 機械判定可能な文書品質是正ルール

中黒、em-dash、LLM 表現、一文一行の 4 規範について、文脈判断を伴わず機械的に適用できる判定アルゴリズムを集約する。
機械置換手順の設計原則（3段階）と参照検査観点の原本も本ファイルが担う。
判定基準が実行者に依存せず、inspect-docs 自動検出、CI 検証、横断修正で再現性が確保されることを目的とする。

> **原本**: 中黒、em-dash、一文一行の機械判定アルゴリズムは本ファイルが原本。
> LLM 表現の検出、書き換え辞書は [rewrite-patterns.md](rewrite-patterns.md) を参照。
> 執筆規範の SSoT は `japanese-tech-writing` スキル（AGENTS.md 経由）、用語政策の原本は document-type-responsibilities Design を参照。

## 目次

- 対象読者
- 対象範囲と適用除外
- 1. 中黒（・）許容範囲判定
- 2. em-dash（—）置換判定
- 3. LLM 表現機械判定
- 4. 一文一行機械判定
- 4 規範の分業と相互参照
- 機械置換手順の設計原則（3段階）
- 参照検査観点（参照実在確認、変動値分離）
- 再現性の担保
- See Also

## 対象読者

docs/ 配下の REQ/Decision/Design/guides/README、および docs を生成、編集する command/skill の自然言語記述に対し、機械判定可能な是正を適用する `agentdev-doc-writing` スキル利用者、inspect-docs 実装者、CI 検証設定者。

## 対象範囲と適用除外

機械判定は以下の 4 規範に限定する。
文脈判断が必要な表現（「最も」「重要な」等）は機械判定対象外とし、サンプリング査読へ委譲する（後述「3. LLM 表現機械判定」の対象外リスト参照）。

適用除外（機械判定を実行しない領域）:

- ユーザー発言の引用、既存文書の引用
- 他ファイルの見出し、節名の引用（`「...」` で括られた参照）
- 禁止表現そのものの批評、説明（禁止表現をルールとして直接説明、例示する文脈のみ。例: 「において」は「で」に置換する、というルール説明文中の対象語。Design やガイドの行為描写中に禁止表現と同一の語が含まれても、ルール説明文脈でなければ是正対象とする）
- コードブロック、インラインコードの内部
- YAML frontmatter のキー名、識別子値

## 1. 中黒（・）許容範囲判定

中黒（`・`）は原則として日本語並列に使わない（japanese-tech-writing L18）。
ただし以下の 4 条件のいずれかを満たす場合は許容し、それ以外の日本語並列は是正対象とする。

### 許容条件（いずれかを満たす場合は許容）

| 条件 | 機械判定方法 | 例 |
|---|---|---|
| Design 明示例 | Design `document-type-responsibilities.md`「中黒使用の許容範囲」節に列挙された固定複合名詞の内部と一致 | `実行時・編集時`、`コマンド・スキル・テンプレート・スクリプト` |
| 時系列対比ペア | 中黒の左右がいずれも `時` で終わる時刻表記（`NN時・NN時` 形式） | `9時・17時`、`開始時・終了時` |
| code block 内 | fenced code block（``` 〜 ```）の内部行 | `・` を含むコード例 |
| YAML title | YAML frontmatter の `title:` フィールド値 | `title: REQ・ADR・Design 横断品質` |

固定複合名詞、単一固有名詞の許容範囲は Design「中黒使用の許容範囲」節が原本。
本ファイルは Design 明示例との照合で機械判定可能な部分のみを扱う。

### 判定手順

1. 行単位で `・` を検索する（テーブル行 `|...|` も対象に含む。テーブルセル内の中黒は是正対象であり、適用除外ではない）
2. 当該行が code block 内または YAML frontmatter 内であれば許容（検出A、B）
3. 中黒の左右が `NN時`（数字 + `時`）であれば許容（検出C）
4. 中黒を含む前後文字列が Design 明示例リストと完全一致すれば許容（検出D）
5. 上記いずれにも該当しない `・` を含む行を是正対象として抽出する

### 是正対象の例

- 流動的並列: `要件・設計・実装を分担する` → 読点（、）または箇条書きへ
- 識別子の並列: `REQ・REQ` → スラッシュ（`/`）または箇条書きへ
- コード値の並列: `feature・fix・chore` → スラッシュまたは箇条書きへ

## 2. em-dash（—）置換判定

em-dash（`—`、`―`）は japanese-tech-writing L17 に従い、同格、補足は括弧（`（）`）で、言い換え、敷衍は句点で2文分割または読点でつなぐ。
コロン（`:`）による置換は行わない。

### 検出パターン

| パターン | 検出方法 | 置換先 |
|---|---|---|
| 本文中の em-dash | 半角空白挟み ` — ` （` ` + U+2014 + ` `）を行単位で検索 | 文脈に応じて `（）` または句点分割、読点接続 |
| テーブルセル em-dash | `| — |` （パイプ + 半角空白 + em-dash + 半角空白 + パイプ）を検索 | パターン D の判別基準で N/A プレースホルダと判別したセルを `| - |` （ハイフン1文字）へ置換 |

テーブルセル `| — |` は本文中の em-dash とは意味が異なる。
検出セルは「パターン D の判別基準」で N/A プレースホルダと意図的マトリックス表記に機械分類し、N/A プレースホルダのみ `| - |` へ置換する。

### パターン D の判別基準（N/A プレースホルダと意図的マトリックス表記）

テーブルセル `| — |` の検出は機械的であり、検出セルは次の2分類のいずれかに該当する。

| 分類 | 意味 | 判別基準（機械判定） | 処置 |
|---|---|---|---|
| N/A プレースホルダ | 該当なし、対象外を意味するセル | 検出セルを含むテーブルブロック（`|` で始まる連続行）内に肯定記号（`✓` 等）が併存しない | `| - |` へ置換 |
| 意図的マトリックス表記（肯定表現の —） | 肯定記号と `—` を対で用い、該当、非該当を表現する表記体系として設計されたマトリックス | 検出セルを含むテーブルブロック内に肯定記号（`✓` 等）が併存する | 維持（置換しない） |

判別は肯定記号併存の有無の機械判定で完結し、文脈判断を要しない。
意図的マトリックス表記の実例は integrity-contracts Design「Workflow × 使用ツールマトリックス」である（同節は肯定表現で一元管理する SSoT と宣言し、`✓` と `—` を対で用いる）。
表冒頭の宣言文（「肯定表現で一元管理する」等）は判別結果の確認資料とする。

### 残存セルの実態と扱い（判別基準確定時点）

判別基準の確定時点（main 8649c006、2026-08）における `| — |` 残存セルの分類と扱いは以下の通り。

| 所在 | セル数 | 分類 | 扱い |
|---|---|---|---|
| integrity-contracts Design「Workflow × 使用ツールマトリックス」 | 7 | 意図的マトリックス表記 | 恒久維持 |
| inspect-promote Design（`--auto` opt-in 発動表） | 2 | N/A プレースホルダ | 置換対象（残存） |
| project-extensions Design（extension 状態分類表の備考列） | 1 | N/A プレースホルダ | 置換対象（残存） |
| 旧 agentdev-artifact-graph SKILL.md（問い合わせプロファイル表の探索方向列。当該スキルは廃止済み） | 1 | N/A プレースホルダ | 解消済み（スキル実体の廃止に伴い解消） |
| document-type-responsibilities Design、本ファイルのパターン説明行 | - | 適用除外（パターン説明文脈） | 維持 |

置換対象（残存）3 セルの是正は本ルールの適用段階で実施する（規則確定と是正実施の分離）。
旧 src/opencode 側 1 セル（旧 `agentdev-artifact-graph` スキル）は機械是正 PR #2154 の merge 後に導入されたものであり、導入時検証での検出漏れを示す事例として扱う（当該スキルの廃止に伴い解消済み）。

### 置換手順

1. テーブルセル `| — |` を検索し、パターン D の判別基準で分類する
2. N/A プレースホルダと判別したセルを `| - |` へ置換する（安全な機械置換、パターン D）。意図的マトリックス表記は置換しない
3. 本文中 ` — ` を検索する
4. 各検出箇所の前後関係を確認し、以下の文脈判定パターン表に従って処置を決定する
5. コロン（`:`）による置換は行わない

#### 本文中 em-dash の文脈判定パターン

| パターン | 文脈 | 判定 | 処置 |
|---|---|---|---|
| A | label—explanation（ラベルと説明の同格強調） | 置換 | 括弧（`（）`）へ置換 |
| B | 句点直後の em-dash（新たな文の開始） | 置換 | 句点で2文分割、または読点で接続 |
| C | 括弧内の既存 em-dash（括弧内で既に補足構造が成立） | ママ | そのまま保持（括弧内は既に補足構造） |
| D | テーブルセル `| — |`（N/A プレースホルダ） | 機械置換 | `| - |` へ置換（手順1〜2で実施済み。意図的マトリックス表記は判別基準により維持） |
| ママ | 上記 A〜D のいずれにも該当しない文脈 | ママ | 査読で文脈確認の結果、置換不要と判断したもの |

本文中 ` — ` は検出自体は機械的だが、パターン A〜C およびママの判定は文脈依存である。
検出後のパターン分類は査読対象とし、上記判定表に従って処置を決定する。

## 3. LLM 表現機械判定

LLM 表現は、表層形式置換で cover できるもの（機械判定対象）と、文脈判断が必要なもの（機械判定対象外）に区分する。
包括的な表現パターン辞書は [rewrite-patterns.md](rewrite-patterns.md) を参照。
本節は機械判定可能な代表例のみを列挙する。

### 機械判定対象（表層形式置換で cover 可能）

検出パターンに対して書き換え先が一意に定まる表現。
文脈によらず機械的に置換できる。

| 検出パターン | 書き換え | 解説 |
|---|---|---|
| `において` | `で` | 「において」は文語調。「で」で足りる |
| `非常に` | 削除 | 主観的強調で根拠を伴わない |
| `極めて` | 削除 | 同上 |
| `することとしている` | `する` | 述語の先送りをやめる |
| `について` | 直接内容を書く | 「関して」は迂遠 |
| `に関して` | 直接内容を書く | 同上 |

包括的な検出、書き換え辞書は [rewrite-patterns.md](rewrite-patterns.md) の「接続の型」「空虚な動詞」「空虚な形容・副詞」「ラベル前置き」節を参照。
本表は機械判定で即時適用可能な代表例である。

### 機械判定対象外（文脈判断が必要、サンプリング査読へ委譲）

以下の表現は文脈によって正当な使用があり、一律置換すると文意を損なう。
機械判定の対象外とし、サンプリング査読で運用する。

| 表現 | 機械判定対象外の理由 |
|---|---|
| `最も〜` | 比較軸が文脈で明示されている場合は正当。機械判定では比較対象の有無を判定できない |
| `重要な〜` | 重要性の理由が本文で示されている場合は正当。機械判定では根拠の有無を判定できない |
| `必要な〜` | 必要性が本文で説明されている場合は正当。同上 |
| `適切に〜` | 基準が文脈で明示されている場合は正当。同上 |
| `〜を行う` | 固定複合名詞の一部（「決定事項を行う」等でない場合）は正当。複合名詞か否かの判定が文脈依存 |

これらの表現は [rewrite-patterns.md](rewrite-patterns.md) に検出パターンとして掲載されるが、本ファイルの機械判定アルゴリズムでは自動置換の対象としない。
サンプリング査読で文脈を確認した上で書き換え要否を判断する。

## 4. 一文一行機械判定

一文一行規範（japanese-tech-writing 整形規範）を機械判定する。
対象は prose line（本文行）であり、テーブル行、unordered list 行、見出しは本アルゴリズムの対象外とする。
ordered list 行は prose として分割対象とする（後述「ordered list 扱い」）。

### 検出条件

1行に複数の `。`（句点）を含む prose line を検出する。

### 例外規則（括弧内 `。` の除外）

括弧（`（）`、`()`）内の `。` は文境界ではなく、括弧内の補足情報の区切りである。
文境界判定から除外するため、括弧の内側にある `。` はカウントしない。

括弧の種類と扱い:

| 括弧 | 扱い |
|---|---|
| `（）` （全角丸括弧） | 内部の `。` は文境界から除外 |
| `()` （半角丸括弧） | 同上 |
| `「」` （かぎ括弧） | 引用内部の `。` は対象行の文境界から除外 |
| `【】`、`『』` | 同上 |

### ordered list 扱い

ordered list（`1. `、`2. ` 等の番号マーカー行）は本アルゴリズムの除外対象とせず、prose として分割する。
分割時の運用は次の通り。

1. 最初の文は番号マーカーを保持した行とする
2. 2文目以降の継続文はマーカーなしの後続行とする

この運用は実装（apply-mechanical-replacement.ps1、`scripts/self/maintenance/` 配下）と整合する。
実装の X-4 除外判定は unordered list マーカー（`^\s*[-*+]\s`）のみに一致し、ordered list 行を除外しない。
Apply-X4Split は先頭セグメントを番号マーカーごと保持し、2文目以降のセグメントにはマーカーを付与しない。
過去是正実績（PR #1091、PR #2154）も同一の運用で分割している。

### 分割手順

1. 対象行が prose line であることを確認する（テーブル行 `|...|`、unordered list 行 `- ...`/`* ...`/`+ ...`、見出し `# ...` は除外。ordered list 行 `1. ...` は除外せず prose として扱う）
2. 括弧の内側にある `。` を一時的にマスクする（文境界カウントから除外）
3. 残った `。` が2個以上であれば是正対象とする
4. 各 `。` の直後で改行し、1行1文へ分割する
5. 括弧内の `。` は元の位置を保持する（マスク解除）

### 判定例

| 入力行 | 判定 | 理由 |
|---|---|---|
| `本規範は機械判定を前提とする。文脈判断は査読に委譲する。` | 是正対象 | `。` が2個、改行で分割 |
| `本規範（機械判定を前提とする。文脈判断は査読に委譲する）を採用する。` | 許容 | 括弧内 `。` は除外、文境界は1個のみ |
| `| 列A | 列B |` | 対象外 | テーブル行 |
| `- 項目1` | 対象外 | unordered list 行 |
| `1. 対象行も分割される。継続文は次行に出力される。` | 是正対象 | ordered list 行は prose として分割。継続文はマーカーなし後続行 |

## 4 規範の分業と相互参照

中黒、em-dash、一文一行は判定手順（検出、許容/是正の二値判定）であり、LLM 表現は検出、書き換え辞書である。
判定アルゴリズムの性質が異なるため、両者を混載せず分業する。

| 規範 | 性質 | 本ファイルでの扱い | [rewrite-patterns.md](rewrite-patterns.md) での扱い |
|---|---|---|---|
| 中黒 | 許容範囲判定（検出→二値判定） | 機械判定アルゴリズム（原本） | 扱わない |
| em-dash | 置換判定（検出→テーブルは判別基準込みの機械置換、本文は査読） | 機械判定アルゴリズム（原本） | 扱わない |
| LLM 表現（表層形式置換） | 検出→書き換え（一意） | 機械判定対象の代表例 | 検出→書き換え辞書（包括的） |
| LLM 表現（文脈判断必要） | 検出→書き換え（文脈依存） | 機械判定対象外、サンプリング査読への委譲を明示 | 検出→書き換え辞書（包括的） |
| 一文一行 | 改行分割（検出→分割位置） | 機械判定アルゴリズム（原本） | 扱いません |

既存 [rewrite-patterns.md](rewrite-patterns.md) は表現パターンの検出→書き換え辞書として現状維持する。
本ファイルから同ファイルへの相互参照を張り、機械判定アルゴリズム側に LLM 表現の機械判定対象を集約する。

## 機械置換手順の設計原則（3段階）

機械置換（機械的テキスト置換、複数ディレクトリ横断の是正を含む）は、次の3段階を設計原則として実行する。
本原則はコーパス横断の機械是正を実行する際の標準手順であり、本ファイルが手順の原本である。
各段階を省略した置換を実行しない。

1. **old 側 grep 実在確認**: 置換対象の旧パターンが対象範囲に実在することを grep で確認し、実在件数と所在を把握する。0 件の場合は置換を実行しない。0 件はパターン設計の誤り、対象範囲の取り違え、是正済みのいずれかを示す前提破綻であり、置換の要否自体を再評価する
2. **置換実行**: 段階1で確認した旧パターンに対してのみ置換を実行する。置換件数が実在件数と一致することを確認する
3. **置換後 MISS 確認**: 旧パターンを段階1と同一のパターン、対象範囲、コマンドで再 grep し、0 件（MISS）であることを確認する。残存が1件でもあれば置換漏れとして再処理する

段階1と段階3の grep は証跡としてコマンド全文と出力を残す（「機械置換 PR のエビデンス記述基準」参照）。
段階3の必須化の経緯（完了宣言と残存の不一致事象）は「機械横断是正 PR における Step 3-4 の必須化」参照。

## 参照検査観点（参照実在確認、変動値分離）

機械置換と参照を含む査読には、次の2観点を適用する。

### 参照実在確認

参照先（ファイル、セクション、見出し、precedent 行、検出パターン等）が実在することを、grep またはファイル読込で検証してから操作、査読する。
参照先の不存在は、参照の誤記、参照先の改名や廃止、前提の変更のいずれかを示す。
実在確認を経ていない参照に基づく置換、判定を行わない。

### 変動値分離

相対行番号等の編集で変動する値は、行内容から分離して査読する。
証跡と照合の錨は行内容（grep 可能な文字列）とし、行番号は補助情報に限定する。
行番号は前後の編集でずれるため、行番号を錨とした照合は置換前後の証跡比較で誤検出と見逃しを生む。

## 再現性の担保

機械判定アルゴリズムは同一入力に対して常に同一の判定結果を返す。
判定結果が実行者に依存しないことが、inspect-docs 自動検出、CI 検証、横断修正での再発防止の前提である。

Wave 2 既存 OU（#1076 em-dash、#1078 中黒、#1079 LLM 表現、#1080 一文一行）の修正サンプルへ本ルールを再適用する手順:

1. 各 OU の修正後 prose を入力とする
2. 上記 4 アルゴリズムを順に適用する（中黒 → em-dash → LLM 表現 → 一文一行）
3. 各アルゴリズムの判定結果が0件（是正対象なし）であることを確認する
4. 判定結果が1件以上出る場合は、Wave 2 修正が不完全か本アルゴリズムが過剰検出のいずれかである。両者を突き合わせて原因を特定する

手順3で0件となることで、Wave 2 修正が本アルゴリズムと整合し、再現性が確認されたことになる。

### 機械横断是正 PR における Step 3-4 の必須化

上記 Step 3-4（再 grep 0 件確認）は推奨ではなく**必須**である（L-012、PR #1090 / #1122 由来。PR #1090 が「7 ディレクトリ完全対応」と完了宣言したが 3 件残存し、後続 PR #1122 で回帰修正した事象への対策）。

- 機械横断是正（機械的テキスト置換・複数ディレクトリ横断是正）を含む PR の完了に先行して、Step 3-4 を必ず実行すること
- 再 grep 結果（0 件）を PR 本文の品質メトリクス欄に記載することを運用化する
- Step 3-4 の実行と PR 本文記載は QG-3（観点 5）/ QG-4（観点 7）で担保される

### 機械置換 PR のエビデンス記述基準（PR #2154 実態不一致の補正方法）

PR #2154 は検証エビデンスで「`| — |` プレースホルダ grep（docs + src/opencode）: 0 件（mechanical-replacement-rules.md のパターン説明行を除く）」と記述した。
当時の docs/designs には実セル 10 件（意図的マトリックス表記 7、N/A プレースホルダ 3）と document-type-responsibilities Design のパターン説明行 1 件が残存しており、記述と実態が不一致していた。
機械置換 PR のエビデンス記述は以下に従い、再発を防止する。

1. grep コマンドは対象ディレクトリを明示したコマンド全文を記録する
2. 例外（パターン説明行、適用除外領域）はファイル単位で件数込みで列挙する
3. 残存セルがある場合は「0 件」と記述せず、残存数と分類（意図的マトリックス表記か未是正か）を記録する
4. 「0 件」宣言は再 grep の実行結果（コマンド全文と出力）を伴う場合のみ行う

### 機械置換スクリプト

`scripts/self/maintenance/apply-mechanical-replacement.ps1` は本ファイルの algorithm を実装するツールである。
Wave 横断是正で共用し、algorithm SSoT と同期する。

スクリプトのヒューリスティックは本ファイルの判定手順に従う。
X-6（「において」検出）のヒューリスティックについて、PR#1249 で false negative を検出した。
検出正規表現の網羅性を確認し、検出漏れがないことを検証手順に含めること。

## See Also

- LLM 表現パターン辞書: [rewrite-patterns.md](rewrite-patterns.md)
- 英語抽象語の検出→書き換え: [rewrite-patterns.md](rewrite-patterns.md)
- backticks 識別子/一般名詞 判定閾値: backticks-identifier-threshold Design（識別子 backticks 必須、一般名詞 backticks 任意の機械判定閾値。inspect-docs 検出処理が参照）
- 用語政策 原本: document-type-responsibilities Design「中黒使用の許容範囲」「em-dash 置換形式」「LLM 表現の検出→書き換え方針」節
- 執筆規範 SSoT: `japanese-tech-writing` スキル（AGENTS.md 経由）
