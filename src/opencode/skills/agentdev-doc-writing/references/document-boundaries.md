...
> **原本**: 配置基準、用語政策は document-type-responsibilities Design（「文書種別責務」「移送判断」節）、執筆規範は `japanese-tech-writing` スキル（AGENTS.md 経由）。
...
### Design 相当内容（原則 Design へ移送）

REQ 要件行が以下のみを主たる文意とする場合、当該内容は Design 等へ配置する対象とする。
...
Design 相当内容のみを主たる文意とする場合は「移送: Design」を提示する。
安定契約例外に該当する場合は REQ に残せる。

2. **Decision か?**（将来を制約する意思決定を記録しているか。）
意思決定ではなく要件、仕様の記述の場合は移送先を提示する。
詳細は [decision-writing-quality.md](decision-writing-quality.md) 参照。

3. **Design か?**（現在の実装構成を成立させる詳細を記述しているか。）
