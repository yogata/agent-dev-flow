...
| 誤認 | 例 | 委譲契約上の影響 |
|------|----|------------------|
| command を skill と呼ぶ | 実行 command を skill と記述、`load_skills` に command 名を指定 | command 名が `load_skills` に指定され実行時エラー、契約不整合を引き起こす |
| harness を skill と呼ぶ | 外部実行基盤を skill と記述 | harness と skill の責務境界が曖昧になり、委譲契約の SSoT が崩れる |
| subagent を skill と呼ぶ | 実行担当サブエージェントを skill と記述 | 委譲起動と `load_skills=...` 参照が混同される |
...
2. **委譲起動記述の検査**（subagent 型名が skill として言及されていないか。）
`agentdev-*` skill 名が subagent 型として指定されていないか。
3. **`/` 先頭識別子の skill 表記検査**（`/agentdev/*` 等の command 名や実行 command が「スキル」「skill」と呼ばれていないか。）
4. **harness 名の skill 表記検査**（外部実行基盤等、OpenCode プラグインとして提供される実行エンジンが「スキル」「skill」と呼ばれていないか。）
5. **委譲契約の主語一貫性**（委譲元、委譲先の記述で実行主体の分類が一貫しているか。例: case-run は command、実行担当サブエージェントは subagent、`agentdev-case-run-execution-adapter` は skill、外部実行基盤は harness。）
