...
<!-- 実証Case識別情報: 実証フラグ、対象評価ブランチ（Epic 実証では共有評価ブランチ）、所属実証単位を記録し、Epic Issue から共有評価ブランチを特定できるようにする（REQ-043-013）。通常Caseでは本セクションを省略する。実証Case専用要素（評価契約・対象評価ブランチ）は presence-based 判定の新契約必須セクション一覧に含めない -->
