...
- adf_execution_unit: （実行単位の flow 種別。standard または epic。対象 Issue 番号は本 Issue の番号を正として導出する）
...
<!-- 実証Case識別情報: 実証フラグ、対象評価ブランチ、所属実証単位、評価契約（実証・評価ワークフロー契約の投影）を記録する。
通常Caseでは本サブセクションを省略する。
