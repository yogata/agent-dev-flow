...
| SPEC確定候補 | 【任意】 | case-run / driver が実装時に発見した SPEC レベルの詳細（schema・enum・判定表・内部アルゴリズム等）。`Findings / Capture候補` とは別セクション（ADR-0123 Decision #4）。case-close Step 3 の SPEC 確定チェック入力となる | セクションごと省略 |
