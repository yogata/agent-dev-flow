...
| 例 | ADR-0111の次 → ADR-0112 |

**注意**: 欠番が存在しても、新規ADRで欠番を埋めない。常に最大番号+1で採番する。

#### Baseline range 例外（REQ-0112-047）

ADR体系の全面改定時（REQ-0112-044）は、`ADR-0101` 以降の番号帯を baseline range として一括採番できる。この場合、最大番号+1の通常採番を上書きし、baseline range の最小番号から連番で採番する。

**適用条件**:
- ADRの全面改定が REQ-0112-044 の例外要件を満たしていること
- baseline range 採番は一度のみ実行され、以降の新規ADRは通常採番に戻る

**例**: baseline range により ADR-0101〜ADR-0111 を一括採番。次の新規ADRは ADR-0112 から通常採番。
...
- `ADR-0101.md` → frontmatter `id: ADR-0101`（必須）
- 不一致の場合はエラーとして扱う
...
- **superseded**: 他のADRに置き換えられた決定。`superseded_by` フィールドに後継ADR番号を記載。`accepted` status の ADR のみ現行根拠として使用する（REQ-0112-006）。`proposed`/`deprecated`/`superseded` の ADR を現行要件判断の根拠として引用しない
...
- Retired View と current ADR の番号重複がないことを確認（REQ-0112-050）
