...
### 8. PR 対象範囲 vs 全体 判定マトリクス（REQ-0146-015）

完了条件が横断評価（REQ-0119-036 等、全成果物を横断する評価）を含む場合、PR が一部ファイル修正の時に「PR 対象範囲」で評価するか「全体（リポジトリ全体、または当該 SPEC/REQ 全体）」で評価するかを判定する（境界ケース #1532/TS-006 由来）。
...
> **参照**: case-close Step 2 は本マトリクスに従い PR 範囲 vs 全体の評価スコープを決定する（OU-007、AG-007）。

### 9. 識別子中心評価の運用実例集（REQ-0146-015）

完了条件の評価軸を「識別子（ファイルパス、REQ ID、NG ID、IR ID 等）」を主評価値とし、「実測値（件数、行数等）」を補助値として扱う運用の実例集。主評価値は安定（実装進行中に変動しない）で、補助値はスナップショット（変動する）ため、識別子を主軸とすることで QG-3 staleness check、QG-4 最終評価が安定して参照解決する（境界ケース #1532/TS-006 に関連）。
...
case-run の QG-3 前置 staleness check は完了条件の識別子（ファイルパス、NG ID、IR ID）を入力として、検出時点から起票時点までの状態変動を検出する。件数（補助値）は変動するため主軸にしない。

**実例3: QG-4 完了条件最終評価**

case-close は完了条件を主評価値（識別子）で最終評価する。補助値（件数等）は QG-4 観点8「PR 対象範囲 vs 全体 判定マトリクス」に従い全体再評価した最新値を参照する。
...
> **REQ-0146-011 参照に関する注意**: REQ-0146-015 は「識別子中心評価（REQ-0146-011）」と参照するが、現 REQ-0146-011 の実際の内容は「case-open は子 Issue 本文に『前工程完了度』属性を埋め込む」であり、「識別子中心評価」とは意味的に不一致である。本観点9 は REQ-0146-015 の後段（運用実例集の蓄積、主評価値と補助値の使い分け）に基づき実装する。REQ-0146-011 の内容修正、または新規 REQ の追加は別途 user 確認が必要（本 PR の Findings セクション参照）。
