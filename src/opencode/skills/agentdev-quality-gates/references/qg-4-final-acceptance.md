...
### 9. 識別子中心評価の運用実例集（REQ-0146-011, REQ-0146-015）

REQ-0146-011 が定める識別子中心評価（識別子を主評価値、実測値を補助値とする運用）の実例集（REQ-0146-015 が本 reference への蓄積を指示）。主評価値は安定（実装進行中に変動しない）で、補助値はスナップショット（変動する）ため、識別子を主軸とすることで QG-3 staleness check、QG-4 最終評価が安定して参照解決する（境界ケース #1532/TS-006 に関連）。
