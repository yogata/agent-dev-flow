...
QG-{N}〜QG-{N} 全 Gate が共通して従う契約を定義する。
...
- QG-{N}/ QG-{N}/ QG-{N} は `pass`/ `warn`/ `fail` の 3 値を使用する。
- QG-{N} は最終受け入れの二値性が強く、`pass`/ `fail` を基本とする（`partial` は CI 保留等の例外的状況のみ）。
...
QG-{N} は乖離検出結果を含むため、上記に加えて乖離報告を付加する。
...
例外は case-run Step 11-1/ 11-3 の自律修正ループ（CI/lint 失敗の機械的再試行）のみで、これは Gate の外にある。
