...
本スキルは Workflow Skill としてレビュー後ルーティングルールを提供する。本スキル自身は workflow STEP を所有せず、case-run / case-update の各 Workflow Skill が所有する STEP から参照される（`<workflows/workflow-skill-model>` SPEC）。

### ルーティング結果と Input Resolution

レビュー NG 時の次コマンド推論結果は、呼出元 STEP から次 STEP への遷移入力として扱われる。Input Resolution は durable state 優先順位（`<workflows/input-resolution-and-durable-state>` SPEC）に従い、ルーティング結果は自然言語の引き継ぎのみに依存せず、Issue ラベル、PR 番号等の identifier と組み合わせて復元される。
