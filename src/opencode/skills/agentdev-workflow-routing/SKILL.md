...
Input Resolution は永続状態の優先順位（`<foundations/v4-durable-state-and-recovery>` Design）に従い、ルーティング結果は自然言語の引き継ぎのみに依存せず、Issue ラベル、PR 番号等の identifier と組み合わせて復元される。
