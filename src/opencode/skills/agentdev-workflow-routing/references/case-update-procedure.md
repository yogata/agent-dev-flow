...
case-update が既存 Case の本文更新、コメント追加、REQ ファイル更新、レビュー NG コメント投稿を行う際の詳細手順を定義する。
親エージェントは Issue 更新、REQ ファイル更新、commit、push、ユーザー確認を担当する。
サブエージェントへ委譲する場合は状態読解、分類候補、根拠抽出のみを依頼する。
...
該当テンプレートの必須セクションが全て本文に含まれることを確認し、`gh issue edit` で更新する。
...
この確認はレビュー NG 時の局所的な漏れ検出であり、`/agentdev/inspect-docs` の全体意味レビューの代替ではない。
