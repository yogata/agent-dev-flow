...
case-run Step 6 で実行担当サブエージェント（driver）へ引き継ぐプロンプトに**必須**で含める項目。
...
## 前工程完了度に応じた振る舞い指針（REQ）

subagent は子 Issue 本文の「前工程完了度」属性（epic-wave-model SPEC の REQ 3段階分類参照）に応じて以下の振る舞い指針に従う。
属性は case-open が子 Issue 本文に埋め込む（REQ）。
