...
サブエージェントがファイル編集を行う際の安全手順。
過去のインシデント（関数本体の誤削除）を防止するためのルール集。

## 目次

- [編集安全手順](#編集安全手順)
- [大規模ファイルの分割委譲ガイダンス](#大規模ファイルの分割委譲ガイダンス)
- [機械的置換の AST-grep 推奨](#機械的置換の-ast-grep-推奨)
- [driver 起動プロンプトテンプレート（Windows + ジャンクション環境）](#driver-起動プロンプトテンプレートwindows--ジャンクション環境)
- [環境制約（Windows + ジャンクション）](#環境制約windows--ジャンクション)
- [前工程完了度に応じた振る舞い指針](#前工程完了度に応じた振る舞い指針)
...
2. **oldString に関数定義全体や大きなコードブロックを含めない**（関数シグネチャ + 波括弧等のマッチは禁止）
...
実行担当サブエージェント（driver）へ引き継ぐプロンプトに**必須**で含める項目。
ジャンクション未伝播により worktree 内 `.opencode/` が空になる事象を事前通知し、driver が手動両辺編集、同期スクリプト非依存で作業するよう誘導する。
...
- **source/ projection 手動両辺編集**: ジャンクション未伝播時は同期スクリプトで自動化できず、source（`src/` 配下）と projection（`.opencode/`）を手動で両辺編集する運用をとる。agent-dev-flow 自己ホストの場合は source 編集後にメインリポジトリのジャンクションが merge 後に自動反映するため worktree での projection 手動編集は不要だが、consumer プロジェクト（ジャンクションなし、`.opencode/` が git 管理対象）では両辺の手動同期が必要
- **同期スクリプト非依存**: `self-sync.ps1`、`install.ps1` を worktree 内で実行してジャンクションを再作成しない（worktree 汚染、競合リスク）。ジャンクション再作成は case-run 終了後にメインリポジトリで実施する
- **`.opencode/skills/repo-*` は実ディレクトリ**: integrity 検査基盤等の `repo-` プレフィックススキルはジャンクションではなく実ディレクトリのため worktree にも伝播する。これらは worktree 内で直接編集してよい
...
- 同期スクリプト（self-sync.ps1 等）を worktree 内で実行しないでください。
- `.opencode/skills/repo-*`（実ディレクトリ）は worktree にも伝播しているため直接編集してかまいません。
```

## 前工程完了度に応じた振る舞い指針

subagent は子 Issue 本文の「前工程完了度」属性（epic-wave-model Design の REQ 3段階分類参照）に応じて以下の振る舞い指針に従う。
属性は case-open が子 Issue 本文に埋め込む。

### 完全完了（通常実行）

- **前提**: Definition 保存（case-ready / case-revise の内部責務）等の前工程で実施済み、追加作業不要
...
- **振る舞い**:
 ...
 - 前工程相当作業（Design 編集、REQ 更新等）は実施しない
...
3. docs/ 配下の関連ファイル（Design/REQ）が既に更新されているか
