...
**禁止事項**: intake/learning を混ぜた単一成果物を作成してはならない（SHALL）。
...
- **case-close の責務**: PR 本文から intake / learning を分離回収し、それぞれの domain state に保存する
...
| コマンド | Capture 関与 | 詳細 |
|----------|-------------|------|
| case-run | **記録のみ** | 本筋外 finding を PR 本文に記録。`.agentdev/intake/inbox/` と `.agentdev/learning/inbox.md` の直接変更は禁止（SHALL） |
| case-close | **回収・保存** | PR 本文から intake / learning を分離回収。自身の実観測も Split Rule に基づいて保存。ユーザーに「学びがあるか」質問禁止（SHALL） |
