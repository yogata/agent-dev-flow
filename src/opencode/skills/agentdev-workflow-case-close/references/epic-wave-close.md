...
#### E6-2: 廃止キーワード全文検索（操作廃止系変更を含む Wave 境界の定型手順）

現在 Wave の子Issue 群で操作廃止系の変更（コマンド・操作・手順の廃止、名称の変更を含む）を行った場合、廃止キーワードの全文検索を定型手順として実行する。
廃止した操作名・旧名称のキーワードでリポジトリ全文（配布 command/skill ソース面を含む）を検索し、残存参照を検出した場合は当該 Wave を完了扱いにせず、残存箇所と対応方針を構造化エラーで報告して停止する（Wave 境界での横断的残存の後続 Wave での遅延発覚を予防する）。
