...
> 本 reference は `agentdev-workflow-inspect-promote` SKILL.md の制御平面（STEP 一覧）STEP-1〜STEP-3 詳細である。
...
- ガードレール（promote された検出事項のみを `.agentdev/inspect/promoted/` へ保存する）
- 不変条件（`--auto` は自動 promote 対象カテゴリ（v4-responsibility-boundaries Design 参照、extension 経由）に合致する高確信度検出事項のみを投入し、意味判断、曖昧な分類、Decision 要否判断を含む検出事項は手動分類へ回す）
