...
- [反復開発（最も効果的なプロセス）](#反復開発最も効果的なプロセス)
- [フィードバックループ](#フィードバックループ)
- [評価先行構築](#評価先行構築)
- [共通パターン](#共通パターン)
- [記述削減・抽象化の前段チェック（固定トークン事前 grep）](#記述削減抽象化の前段チェック固定トークン事前-grep)
- [アンチパターン一覧](#アンチパターン一覧)
...
配布物本文の記述削減・抽象化の前に、対象ファイルを参照する `*.test.ts`・checker の固定トークンを事前 grep して影響を確認する（必須ステップ）。固定トークンの存在は本文の記述から読み取れないため、確認なしに削減するとテスト不合格・チェック違反として初めて発覚する（PR #2186、PR #1728/#1735 系の再発防止）。

自動化は必須としない。grep 結果の影響判断（残す・削る）を含むため、手順の明文化を主体とする。

### grep 対象の代表例

| 分類 | 代表例 |
|---|---|
| routing token | 検査器エントリポイント名、CLI 引数（`--profile` 等）、result 状態語 |
| 期待値固定セクション | テスト・checker が期待値として保持する見出し・セクション文言（検証表の行ラベル、必須セクション名） |
| 概念名文字列 | checker の `content.includes` 由来の文字列（`capture-boundaries` 等の責務・契約の概念名） |

### 手順

1. 編集対象ファイルのパス・ファイル名・スキル名・コマンド名を grep パターンとして、対象リポジトリの `*.test.ts` と checker スクリプト（配布境界・構造・docs 系 checker 等）を grep する
2. ヒットしたテスト・checker の期待値として固定されている routing token・セクション文言・概念名文字列を確認する
3. 固定トークン依存が検出された場合は回避パターンから対応を選択する。検出されなければ記述削減・抽象化を進める

### 両立運用の回避パターン

- **セクション原形維持**: 固定トークンを含むセクションは公開 interface として原形を維持し、削減を周辺の工程詳細の再要約に限定する（PR #2186 の解消パターン）
- **概念名の括弧書き残存**: 本文から具体参照を除去する際、checker が要求する概念名文字列は括弧書きで本文に残し、機械検査契約と両立する（PR #1728/#1735 の解消パターン）
