...
本ファイルは `agentdev-skill-authoring` SKILL.md の補助資料であり、スキルを反復的に開発するためのワークフロー（反復開発、フィードバックループ、評価先行構築、テンプレートパターン、条件付きワークフロー）と、避けるべきアンチパターン一覧を扱う。SKILL.md 本文では開発姿勢の要点のみを提示し、具体的手順とパターン一覧は本ファイルを参照する（REQ-0113-010）。
