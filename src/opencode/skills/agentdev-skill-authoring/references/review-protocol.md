...
本ファイルは `agentdev-skill-authoring` SKILL.md の補助資料であり、SKILL.md の品質を5観点でチェックする4種プロトコル（frontmatter、予算、構造、助言）とサブエージェント編集安全性の実施手順を定義する。SKILL.md 本文では観点の一覧のみを提示し、個別のチェックリストと詳細規定は本ファイルを参照する（REQ-0113-010）。
