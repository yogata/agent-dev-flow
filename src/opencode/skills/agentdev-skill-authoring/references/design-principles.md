...
本ファイルは `agentdev-skill-authoring` SKILL.md の補助資料であり、SKILL.md 本文に圧縮して記載するとエントリポイントとしての役割を損なう設計原則の詳細（簡潔さ、自由度、トークン予算管理、行数ガバナンス、抽出ルール）を扱う。SKILL.md は本ファイルを参照しつつ、本文では要約と判断表のみを保持する。
