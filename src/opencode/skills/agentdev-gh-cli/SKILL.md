...
GitHub Issue/PR の I/O 手続きと VERIFY を集約する中央集権的な I/O 境界（REQ-0149, ADR-0130）。
command と skill は gh コマンドを直接記述せず、本スキルの手続きへ委譲する（REQ-0149-001/006/007）。
...
各手続きの操作契約（引数、戻り値、エラー扱い）は [references/contracts.md](references/contracts.md) 参照。標準版（GitHub 版）の具体的実装手順は [references/standard-procedures.md](references/standard-procedures.md) 参照。
...
書き込み操作の直後に実施する。観点は4つ: エンコーディング、Markdown 構造、テンプレート必須セクション、リポジトリ参照リンク正規化。
...
PR 関連手続きはスキップせず、Case ファイルの対応セクションで代替する。手続きと Case ファイルセクションの対応は SPEC agentdev-gh-cli.md 参照。

## 適用対象

- GitHub Issue/PR を操作するすべての command と skill（REQ-0149-001）
...
- [REQ-0149](../../../../../docs/requirements/REQ-0149.md)
- [ADR-0130](../../../../../docs/adr/ADR-0130.md)
