...
`agentdev-gh-cli` の VERIFY 手続きの実装観点と検査項目。
...
Windows 環境で WRITE 手続きを実行する場合、書き込み操作の前にコンソールエンコーディング初期化（[standard-procedures.md](standard-procedures.md) WRITE 標準手順「コンソールエンコーディング初期化」）が実行されていることを確認する。
...
- 書き込み元テキストと読み戻しテキストの LF（U+000A）出現数が一致することを確認。LF 数の不一致は Markdown 構造破壊（事実上の1行化、空行の消失）を示唆する
- 制御文字（U+0000-U+001F, U+007F-U+009F）が混入していないか確認（改行 U+000A, タブ U+0009 は除外）
- 非Unicode文字への置換が発生していないか確認

#### (b) Markdown構造検証

- 見出し行の直前が空行またはファイル先頭であることを確認。見出し前の空行欠損は Markdown レンダラが見出しを認識しない原因となる
- 見出し `##` は行頭から始まることを確認（行中 `##` を含まないこと）。行中 `##` はレンダラにより通常テキストとして扱われる
- テーブル: 行ごとの列数が一致しているか
