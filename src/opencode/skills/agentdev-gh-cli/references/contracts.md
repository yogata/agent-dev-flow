...
`agentdev-gh-cli` が提供する I/O 手続きと VERIFY の操作契約。
...
基盤手続き一覧を UPDATE せず、拡張手続きとして新設する2手続き。I/O 手続きに属し、本文生成、完了判定を含まない。READ 手続きであるため WRITE 手続きの VERIFY 4観点は適用せず、代わりに事後条件（出力検証）を用いる。
...
| 入力 | PR 番号 |
| 出力 | 変更ファイルパス一覧（文字列配列） |
| 事後条件 | 出力配列の各要素が PR の変更ファイルパスと一致すること |
| エラー扱い | gh CLI が非零終了時は失敗。JSON の `files` 配列抽出に失敗した場合も失敗 |
...
|---|---|
| 入力 | PR 番号 |
| 出力 | `MERGEABLE` / `CONFLICTING` / `UNKNOWN` |
| 事後条件 | gh CLI が返した `mergeable` 値をそのまま enum に写すこと |
| エラー扱い | gh CLI が非零終了時は失敗。`mergeable` 値が想定 enum 外の場合は失敗 |
