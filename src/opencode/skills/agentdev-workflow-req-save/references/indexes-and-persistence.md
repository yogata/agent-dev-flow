...
- **未分類行の記録（段階ゲート）**: STEP-4 で検出した検証対応要否未分類行（検証対応宣言なし かつ 検証対応要否カタログ未登録の要件行）を、OU 結果の書き戻しに要件行ID一覧として含める。case-open の段階ゲート（未分類行残存時の停止）がこの記録を参照して分類完了の案内に用いる。未分類行の存在は commit・push の妨げにならない
...
STEP-4 で検出した検証対応要否未分類行がある場合は、完了報告に未分類行の要件行ID一覧を明示的に記録する。
未分類行の記録には、分類完了が case-open または実装着手前までの必須条件であること（検証対応任意行としての検証対応要否カタログ登録、または検証対応宣言を持つ恒久検証手段の整備のいずれかで完了）を案内する。
未分類行の存在だけを理由として保存を失敗扱いにしない。
...
- 完了報告本文（種別、保存結果、未分類行の要件行ID一覧）

### Completion Verification

- 選択種別が実行結果（SPLIT 検出有無、Epic 規模）と一致していること。未分類行を検出した場合に完了報告へ記録していること
