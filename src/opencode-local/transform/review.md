...
ローカル版生成（`transform/generate.md` に従う生成）の結果が、`src/opencode-local/` のスキーマ、定義、ガードレールに合致しているかを検証する。
必須チェックが1つでも失敗した場合、結果は `FAIL` とする（REQ-0141-028）。
...
以下の6区分について確認する（REQ-0141-028, AG-009）。
各項目は必須チェックであり、1つでも失敗すれば結果は `FAIL` となる。
...
検出された違反は以下の形式で列挙する。
違反とは「必須操作、必須入力、必須出力として残る GitHub Issue / PR 参照」または「確認対象一覧の各必須チェック失敗」を指す。

GitHub 固有参照の違反/非違反判定基準は `transform/spec.md` の「残存 GitHub 固有参照の違反判定基準」参照（REQ-0141-029）。
本レビューではこれに「確認対象一覧の必須チェック失敗」を加えたものを違反として扱う。
...
冒頭の規定（必須チェック1件でも失敗で `FAIL`、部分通過の `PASS` 扱い禁止）に従う。
`FAIL` の場合は生成物を修正して再レビューし、再レビューでも `FAIL` の場合は利用者に手動確認を促す。
