...
生成時に遵守するガードレール（原本保護、安全ゲート、配置規則、バックエンド制約、GitHub 依存除去、Case ファイル引き継ぎ、リポジトリ管理対象外）は `transform/spec.md` の「ガードレール一覧」に集約している（REQ-0141-014, AG-009, AG-010）。本プロンプトは同ファイルを入力として読み込む前提で参照する。

## レポート出力

変換後に生成レポートを出力する。レポートの必須項目（REQ-0141-028, AG-009）は `transform/spec.md` の「レポートフォーマット」参照。

## 残存 GitHub 固有参照の違反判定基準

生成物に残存する GitHub 固有参照の違反/非違反判定基準（REQ-0141-029）は `transform/spec.md` の「残存 GitHub 固有参照の違反判定基準」に根拠列付きで集約している。違反が1件でも残存する場合、レポート結果は `FAIL` とする。
