...
| Accepted ADR 引用 | `check_integrity.ts` | accepted 以外の ADR 引用検出（current baseline + retired 区別、SHOULD, REQ-0108-125, REQ-0112-050） |
...
| RFC 2119 marker | `check_integrity.ts` | （SHALL）/（SHOULD）/（MAY）/（MUST）括弧付き marker 横断検出（REQ-0108-236, REQ-0102-028） |
...
| REQ verification basis | `check_integrity.ts` | REQ 要件行の検証基準が RFC 2119 語ではなく必達要件判定であること（REQ-0115-044） |
...
`/repo/docs-check` command の Step 2 レポート形式に準拠。
