...
| 語彙ポリシー | `check_integrity.ts` | active 対象範囲の語彙ポリシー違反検出（REQ-0108-236, REQ-0102-028） |
| Cross-REQ vocab | `check_integrity.ts` | active REQ 間の語彙矛盾検出（REQ-0108-239） |
...
| Skill-internal section ref | `check_integrity.ts` | command から skill 内部構造への不正参照検出（REQ-0108-244） |
...
### skill-internal-section-reference 検出

command から skill 内部構造への不正参照を検出する。

**検出対象パターン**:
- 自然言語ラベルからのファイル推測参照: `workflow classification`、`Issue 生成プロトコル`、`達成判定プロトコル` 等
- skill 内部 protocol/Step/Section/見出し名参照: `skill の .*protocol`、`skill の「Step`、`Section [0-9]` 等（command から他 skill への参照に限る）
- command 固有 Step 番号の skill 側保持: skill 内に `case-run Step [0-9]`、`case-close Step [0-9]`、`req-define Step [0-9]` 等

**目的**: 不存在ファイル推測・skill 内部構造依存・command Step 保持の再発防止
