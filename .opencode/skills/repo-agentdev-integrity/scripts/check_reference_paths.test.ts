...
describe("checkScriptTemplateReferencePaths", () => {
  ...
  it("resolves skill-relative paths from SKILL.md", () => {
    ...
    mkdirp(join(skillsDir, "agentdev-test-skill", "scripts"));
    writeFileSync(
      join(skillsDir, "agentdev-test-skill", "scripts", "validate.ts"),
      "// ok",
      "utf-8",
    );
    writeFileSync(
  ...
  it("cross-skill bare reference is detected as ng", () => {
    ...
    writeFileSync(
      join(skillsDir, "agentdev-other-skill", "references", "shared.md"),
      "# Shared\n",
      "utf-8",
    );
    writeFileSync(
  ...
  it("command file bare reference is not cross-skill detected", () => {
    ...
    mkdirp(join(skillsDir, "agentdev-other-skill", "references"));
    writeFileSync(
      join(skillsDir, "agentdev-other-skill", "references", "guide.md"),
      "# Guide",
      "utf-8",
    );
    writeFileSync(
  ...
  it("command-local template ok when file exists", () => {
    ...
    writeFileSync(
      join(cmdDir, "templates", "test-cmd", "standard.md"),
      "# Standard\n",
      "utf-8",
    );
    writeFileSync(
    ...
    const ngResults = refResults.filter((r) => r.level === "ng");
    const cmdNg = ngResults.filter((r) =>
      r.evidence?.includes("test-cmd/standard.md"),
    );
  ...
  it("command-local template ng when file missing", () => {
    ...
    expect(
      refNg.some((r) => r.evidence?.includes("test-cmd/standard.md")),
    ).toBe(true);
  });
