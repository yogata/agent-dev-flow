// ADF-COVERS(implementation): REQ-057-031, REQ-057-032
// ADF-COVERS(verification): REQ-001-001, REQ-001-002, REQ-001-003, REQ-001-006, REQ-001-008, REQ-001-010, REQ-001-011, REQ-001-012, REQ-001-013, REQ-001-014, REQ-001-015, REQ-001-021, REQ-001-025, REQ-001-026, REQ-001-027, REQ-001-028, REQ-001-030, REQ-001-038, REQ-001-039, REQ-001-040, REQ-001-041, REQ-001-047, REQ-001-056, REQ-001-057, REQ-001-058, REQ-001-059, REQ-001-060, REQ-001-062, REQ-001-065
// ADF-COVERS(verification): REQ-036-027
// ADF-COVERS(verification): REQ-002-007, REQ-002-008, REQ-002-009, REQ-002-010, REQ-002-019, REQ-002-034
// ADF-COVERS(verification): REQ-004-003, REQ-004-013, REQ-004-031, REQ-004-032, REQ-004-033
...
// v2:REQ-0158-004: docs/designs/**/*.md 再帰収集ヘルパー。
// AC-7: collectAllArtifactPaths, checkDocMapSpecSync, checkSpecReadmeIndexSync,
// checkUpdateNotesInDocs, scanned.Designs, Design inventory 照合、DOC-MAP と Design の
// 照合処理は再帰前提へ更新する（docs/designs/*.md 直下のみ前提の廃止）。
// opts.excludeReadme: docs/designs/README.md は Design status の単一追跡情報源
// （v2:REQ-0154-001/003、v2:REQ-0158-005）であり、Design 本文検査では除外する。
// Design inventory/status 同期検査と DOC-MAP との照合では対象とするため、
...
function collectSpecMarkdownRecursively(
  ...
  walkMarkdown(designsDir, collected);
...
// v2:REQ-0158-005: docs/designs/README.md は Design status の単一追跡情報源。
// Design 本文検査では除外し、Design inventory/status 同期検査と DOC-MAP 照合では対象とする。
function isSpecReadme(fullPath: string, designsDir: string): boolean {
  const readmePath = path.join(designsDir, "README.md");
  try {
...
// Design inventory source: docs/designs/README.md (v2:REQ-0154-001/003). Reading the
// explicit list (instead of enumerating the directory) keeps missing Designs
...
// Capture group 1 holds the Design entry. The leading (?:^|[|(])\s* boundary
...
// placeholders (designs/commands/<x>.md) are not captured. The segment class
// excludes "/" so each match is a single domain/file entry.
const DESIGN_ENTRY_RE = new RegExp(
  `(?:^|[|(])\\s*((${DESIGN_DOMAINS})/[A-Za-z0-9._-]+(?:\\.md|/))`,
  "gm",
);

function checkDesignsExistence(designsDir: string, root: string): CheckResult[] {
  const results: CheckResult[] = [];
  const readmePath = path.join(designsDir, "README.md");
  const readmeContent = readText(readmePath);
  ...
  if (!readmeContent) {
    results.push(
      info(
        "Designs",
        "designs-existence",
        `docs/designs/README.md not found at ${resolveRelative(readmePath, root)}; designs-existence skipped`,
      ),
    );
    return results;
  }
  ...
  DESIGN_ENTRY_RE.lastIndex = 0;
  while ((m = DESIGN_ENTRY_RE.exec(readmeContent)) !== null) {
    entries.add(m[1]);
  }
  ...
  if (entries.size === 0) {
    results.push(
      ng(
        "Designs",
        "designs-existence",
        `No Design entries found in ${resolveRelative(readmePath, root)}`,
      ),
    );
    return results;
  }
  ...
  for (const rel of [...entries].sort()) {
    const fullPath = path.join(designsDir, rel);
    if (fs.existsSync(fullPath)) {
      results.push(ok("Designs", "designs-existence", `${rel} exists`));
    } else {
      results.push(
        ng(
          "Designs",
...
function collectAllArtifactPaths(root: string): string[] {
  ...
  // v2:REQ-0158-004: docs/designs/**/*.md 再帰収集に更新
  const collections: { dir: string; recursive?: boolean }[] = [
    ...
    { dir: path.join(root, "docs", "designs"), recursive: true }, // 4. spec (recursive)
...
function checkExpandedLegacyNamespace(
  ...
  // NEW: docs/designs/**/*.md (v2:REQ-0158-004: recursive)
  const designsDir = path.join(root, "docs", "designs");
  if (fs.existsSync(designsDir)) {
    for (const f of collectSpecMarkdownRecursively(designsDir))
      filesToCheck.push(f);
  }
...
function checkDocMapSpecSync(root: string): CheckResult[] {
  ...
  // v2:REQ-0158-004/005: docs/designs/**/*.md を再帰収集。docs/designs/README.md は
  // DOC-MAP との照合では対象とする（Design status の単一追跡情報源、v2:REQ-0154-001/003）。
  const designsDir = path.join(root, "docs", "designs");
  const specFiles = collectSpecMarkdownRecursively(designsDir);
  ...
  for (const link of links) {
    ...
    if (rel.startsWith("docs/designs/")) {
...
function checkSpecReadmeIndexSync(root: string): CheckResult[] {
  const results: CheckResult[] = [];
  const designsDir = path.join(root, "docs", "designs");
  // v2:REQ-0158-004/005: docs/designs/**/*.md を再帰収集。docs/designs/README.md 自身は
  // Design inventory/status 同期検査の追跡情報源（v2:REQ-0154-001/003）として扱うため、
  // 収集対象から除外する（自身との照合を避ける）。
  const specFiles = collectSpecMarkdownRecursively(designsDir, {
    excludeReadme: true,
  });
  const specRelPaths = specFiles.map((full) =>
    path.relative(designsDir, full).replace(/\\/g, "/"),
  );
  ...
  // v2:REQ-0158-004: README.md は個別 Design ファイル（basename or 相対パス）と
  ...
  // v2:REQ-0158-004: 各 Design ファイルについて、basename が個別参照されているか、
  ...
  for (const name of missingFromIndex) {
    results.push(
      ng(
        "Inventory",
        "spec-readme-index",
        `${name} exists but not referenced in designs README`,
      ),
    );
  }
  ...
  if (missingFromIndex.length === 0) {
    results.push(
      ok(
        "Inventory",
        "spec-readme-index",
        "All design files referenced in designs README",
      ),
    );
  }
  return results;
...
function checkRuidGroundReference(root: string): CheckResult[] {
  const results: CheckResult[] = [];
  // v2:REQ-0158-004: docs/designs/**/*.md 再帰対応。flat dirs (requirements/guides/adr) は
  ...
  if (fs.existsSync(designsDir)) {
    for (const f of collectSpecMarkdownRecursively(designsDir)) allFiles.push(f);
  }
      ...
      if (phaseInNarrative && legacyOnlyPhasePattern.test(lines[i])) {
        ...
        results.push(
          ng(
            ...
            `Workflow status / 6 micro-phase state management pattern detected in REQ/Design`,
            relPath,
            i + 1,
            {
              evidence: lines[i].trim(),
              expected:
                "REQ/Design must not define workflow status state management",
              route: "intake",
            },
          ),
        );
  ...
  if (!foundViolation) {
    results.push(
      ok(
        "LifecycleBoundary",
        "workflow-status-prohibition",
        "No workflow status / 6 micro-phase patterns in REQ/Design",
      ),
    );
  }
  return results;
...
function checkNonAcceptedArtifactRefsInFiles(
  ...
  for (const scanTarget of filesToScan) {
    ...
    if (stat.isDirectory()) {
      const isDesignsDir = scanTarget === path.join(root, "docs", "designs");
      const fileList = isDesignsDir
...
function checkAbolishedSkillReferences(root: string): CheckResult[] {
  ...
  // Scope: docs/designs/**/*.md (runtime guidance) — v2:REQ-0158-004: recursive
  const designsDir = path.join(root, "docs", "designs");
  if (fs.existsSync(designsDir)) {
    for (const f of collectSpecMarkdownRecursively(designsDir)) {
      filesToScan.push(f);
...
const DOCUMENT_CLASSIFICATIONS = ["REQ", "Decision", "Design", "Guide", "Report", "DOC-MAP"] as const;
  ...
  if (fs.existsSync(reportsDir)) {
    ...
    results.push(
      ok(
        "ClassificationPolicy",
        "report-collection",
        `Report collection at docs/reports/ contains ${reportFiles.length} file(s)`,
      ),
    );
  } else {
    results.push(
      info(
        "ClassificationPolicy",
        "report-collection",
        "Report collection directory docs/reports/ does not exist (no reports recorded yet)",
      ),
    );
  }
...
function checkCommandCaptureDuties(cmdDir: string, root: string): CheckResult[] {
  ...
  // （docs/designs/workflows/capture-boundaries.md「各コマンドの capture 責務」セクション）
...
// ─── IR-044: REQ/Design boundary violation detection (v2:REQ-0108-259, v2:REQ-0108-260) ──
// Detects Design detail contamination in active REQ requirement lines.
// 9 Design separation criteria violation signals (v2:REQ-0101-067, 068):
...
// v2:REQ-0145-012: META 規則行 exemption。REQ/Design 責務範囲を規定する行（Design 種別を
// 名指しして責務境界を宣言する行）を機械的に判定し免除する。件数・内容規定を含む行は
// 予防的ガード句 (v2:REQ-0145-013) により免除を拒否し、true positive 保護を維持する。
// 当該行は Design 詳細の記述ではなく責務範囲の規定である（v2:REQ-0145-012 準拠）。
function isMetaRuleLine(line: string): boolean {
  if (hasCountOrContentRule(line)) {
    return false;
  }
  // (1) Design への切り出し・配置・混入排除を宣言する行
  if (/切り出し|切り出す|配置する\s*(対象|こと)|混入\s*させ(ない|ぬ)/.test(line)) {
    return true;
  }
  // (2) REQ/Design 定義構造（REQ は X、Design は Y の文書種別定義）
  if (/REQ\s*は.*Design\s*は/.test(line)) {
    return true;
  }
  ...
  // (4) Design / catalog / reference への委譲（委譲先に Design 系キーワードを含む行のみ）
  if (/委譲/.test(line) && /Design|catalog|カタログ|reference|リファレンス/.test(line)) {
    return true;
  }
  return false;
}

function checkReqDesignBoundaryViolation(root: string): CheckResult[] {
  const results: CheckResult[] = [];
  const reqDir = path.join(root, "docs", "requirements");
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      for (const { signal, pattern } of IR044_SIGNAL_PATTERNS) {
        if (pattern.test(stripped)) {
          ...
          // v2:REQ-0145-012: META 規則行 exemption。REQ/Design 責務範囲を規定する行は免除する。
          ...
          results.push(
            warn(
              ...
              `REQ requirement line may contain Design detail ("${signal}") — consider moving to Design/rule catalog/command reference (IR-044, v2:REQ-0108-259)`,
              relPath,
              i + 1,
              {
                evidence: `${reqItemId}: ${description.trim()}`,
                expected:
                  "REQ should describe external contract / state / behavior; Design detail belongs in Design docs",
                route: "req-define",
              },
            ),
          );
  ...
  if (!foundViolation) {
    results.push(
      ok(
        "CanonicalConflict",
        "req-spec-boundary-violation",
        "No REQ/Design boundary violations detected in active REQs (IR-044, v2:REQ-0108-259)",
      ),
    );
  }
  return results;
...
// ===== IR-054: draft Design 放置検出 (v2:REQ-0154-002) =====
// 閾値: 30日（docs/designs/integrity-rule-catalog.md「IR-054 閾値設計」が原本）
const IR054_DRAFT_STALE_DAYS = 30;
const IR054_DATE_PATTERN = /^(\d{4})-(\d{2})-(\d{2})$/;

function checkDraftSpecStaleness(designsDir: string, root: string): CheckResult[] {
  const results: CheckResult[] = [];
  if (!fs.existsSync(designsDir)) return results;

  // docs/designs/**/*.md を再帰収集（README.md は対象外）
  const collected: string[] = [];
  walkMarkdown(designsDir, collected);
    ...
    if (ageDays > IR054_DRAFT_STALE_DAYS) {
      ...
      results.push(
        warn(
          ...
          `Draft Design stale: age=${ageDays}d (threshold=${IR054_DRAFT_STALE_DAYS}d), updated=${updated}. Triage: (a) promote to accepted via case-close Design determination, (b) update content and bump updated, or (c) retire (IR-054, v2:REQ-0154-002)`,
          resolveRelative(fullPath, root),
          1,
          {
            evidence: `status: draft, updated: ${updated}, age: ${ageDays}d`,
            expected: `draft Design updated within ${IR054_DRAFT_STALE_DAYS} days, or promoted to accepted`,
            route: "intake",
          },
        ),
      );
  ...
  if (!foundStale) {
    results.push(
      ok(
        "Designs",
        "draft-spec-staleness",
        `No stale draft Designs detected (checked ${draftCount} draft Designs, threshold ${IR054_DRAFT_STALE_DAYS}d) (IR-054, v2:REQ-0154-002)`,
      ),
    );
  }
  return results;
...
const IR055_HEURISTIC_PATTERNS: ReadonlyArray<{ name: string; pattern: RegExp }> = [
  { name: "docs/designs/", pattern: /\bdocs\/designs\//g },
...
//
// Design: docs/designs/integrity/integrity-contracts.md「NG baseline 運用手順」.
...
// Report returned by applyNgBaseline: three NG classification counts per design
...
// Design categories: baseline-known (legacy entries, no provenance), approved
...
// Selectively merge approved additions into the baseline. Per Design the update
...
// Detects残留する旧Design直下パス参照 (`docs/designs/<name>.md`) と link mode 統一
...
// 対照表: docs/designs/integrity/obsolete-path-map.yaml
...
function isIr057ExemptPath(relPath: string): boolean {
  // IR-057 Design exemption 表に基づくパス単位免除は共有純粋関数へ委譲（REQ-0144-024）。
...
// IR-057 文書レベル履歴注記 exemption（Issue #1768、IR-057 Design「例外登録」セクション、v2:REQ-0144-024）。
  ...
  for (const fullPath of targetFiles) {
    ...
    for (const entry of map.entries) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        // backticks 内 (例: `docs/designs/system.md`) も免除
        ...
        results.push(
          ng(
            "CanonicalConflict",
            "obsolete-spec-path",
            `Old design direct path reference '${pattern}' detected (current: ${entry.new})`,
            relPath,
            i + 1,
            {
              evidence: line.trim(),
  ...
  if (!foundViolation) {
    results.push(
      ok(
        "CanonicalConflict",
        "obsolete-spec-path",
        "No obsolete design direct path references or legacy local generation vocabulary detected (IR-057, v2:REQ-0158-002)",
      ),
    );
  }
  return results;
        ...
        results.push(
          ng(
            ...
            block.startLine + 1 + mismatchIndex,
            {
              ...
              expected: `AUTOGEN block contents must match IR-* related_req / related_design derivation`,
              route: "intake",
            },
          ),
        );
...
// New-PluginLoaderShimContent と同一内容を保つこと（OpenCode は
// .opencode/plugins/ depth-1 のファイルのみ自動読み込みする）。
function buildPluginLoaderShimContent(packageName: string): string {
  return (
    "// Generated by scripts/install.ps1 / scripts/self-sync.ps1 - do not edit.\n" +
    `export { default } from "./${packageName}/plugin.ts";\n`
  );
}

// Windows junction の readlink は \\?\ / \??\ プレフィックス付き絶対パスを
// 返すことがあるため、比較用に正規化する。Windows では大文字小文字を無視する
// （self-sync.ps1 の Resolve-Path -eq 比較に相当する環境非依存判定）。
...
type PluginProjectionLinkState =
  | { kind: "missing" }
  | { kind: "not-link" }
  | { kind: "broken" }
  | { kind: "link"; target: string };

// 投影パッケージディレクトリの link 状態を判定する。パス不在は missing、
// junction / symlink 以外は not-link、reparse 先の解決不能や readlink 失敗は
// broken（self-sync.ps1 check の Missing / not a junction / Broken junction
// の3区分に対応する DIVERGENCE 扱い）。
function inspectProjectionLinkState(
  projPath: string,
): PluginProjectionLinkState {
  let lstat: import("fs").Stats;
  try {
    lstat = fs.lstatSync(projPath);
  } catch (e) {
  ...
  if (!lstat.isSymbolicLink()) return { kind: "not-link" };
  try {
    return { kind: "link", target: fs.readlinkSync(projPath) };
  } catch {
    return { kind: "broken" };
  }
}
...
/**
 * repo-local Plugin の自己ホスト投影対称性検査
 * （docs/designs/local/runtime-package-boundary.md「自己ホスト投影対称性検査の
 * 機械検査契約」、Issue #2787）。
 *
 * 対称性の契約は3点: 本 Design の投影契約、plugin README の配布宣言、
 * scripts/self-sync.ps1 の動的列挙。判定の正合は self-sync.ps1 の動的列挙
 * （src/opencode/plugins/agentdev-* ディレクトリ、depth-1 loader shim 生成）
 * との一致を最優先する。
 *
 * 検出対象: (a) 投影欠落（junction 不在 / junction 以外 / broken・wrong target /
 * shim 不在）、(b) 余剰投影（正本にない ADF 管理投影）、(c) shim 内容不一致。
 * 対称性破れは [DIVERGENCE] 相当の ng 検出として docs-check 系検査結果へ報告する。
 * 投影の再同期（self-sync.ps1 再実行）は環境操作であり、本検査は repo 内で完結する。
 * 管理物判定不能な junction は REQ-058-008 非破壊境界に従い info 報告のみ。
 */
export function checkRepoLocalPluginProjectionSymmetry(
  root: string,
): CheckResult[] {
  const results: CheckResult[] = [];
  const sourcePluginsDir = path.join(root, "src", "opencode", "plugins");
  const projectionPluginsDir = path.join(root, ".opencode", "plugins");

  // 正本 plugins ディレクトリ不在（自己ホスト構成でない環境）では検査対象なし。
  if (!fs.existsSync(sourcePluginsDir)) {
    return results;
  }

  // 正本列挙: src/opencode/plugins/agentdev-* ディレクトリの動的列挙。
  // scripts/self-sync.ps1 Get-SelectiveJunctionTargets と同一条件（ハードコードなし）。
  const sourcePackages = (
    fs.readdirSync(sourcePluginsDir, { withFileTypes: true }) as import("fs").Dirent[]
  )
    .filter((e) => e.isDirectory() && e.name.startsWith("agentdev-"))
    .map((e) => e.name)
    .sort();
  const sourcePackagesSet = new Set(sourcePackages);

  const projEntries: import("fs").Dirent[] = fs.existsSync(projectionPluginsDir)
    ? (fs.readdirSync(projectionPluginsDir, {
        withFileTypes: true,
      }) as import("fs").Dirent[])
    : [];

  // (a) 投影欠落・(c) shim 内容不一致: 正本パッケージごとの投影対象構造を検査する。
  for (const pkg of sourcePackages) {
    const projPkgPath = path.join(projectionPluginsDir, pkg);
    ...
    const linkState = inspectProjectionLinkState(projPkgPath);
    if (linkState.kind === "missing") {
      results.push(
        ng(
          "PluginProjection",
          "repo-local-plugin-projection-symmetry",
          `Repo-local Plugin projection missing: plugins/${pkg}`,
          resolveRelative(projPkgPath, root),
          undefined,
          {
            evidence: `projection-missing:plugins/${pkg}`,
            expected: `junction .opencode/plugins/${pkg} -> src/opencode/plugins/${pkg} (resync is an environment operation: scripts/self-sync.ps1)`,
            route: "intake",
            finding_category: "obsolete-structure",
            finding_level: "strict",
          },
    ...
    } else if (linkState.kind === "broken") {
      results.push(
        ng(
          "PluginProjection",
          "repo-local-plugin-projection-symmetry",
          `Repo-local Plugin '${pkg}' projection junction is broken or unresolvable: plugins/${pkg}`,
          resolveRelative(projPkgPath, root),
          undefined,
          {
            evidence: `junction-broken:plugins/${pkg}`,
            expected: `junction .opencode/plugins/${pkg} -> src/opencode/plugins/${pkg} (resync is an environment operation: scripts/self-sync.ps1)`,
            route: "intake",
            finding_category: "obsolete-structure",
            finding_level: "strict",
    ...
    } else if (linkState.kind === "not-link") {
      results.push(
        ng(
          "PluginProjection",
          "repo-local-plugin-projection-symmetry",
          `Repo-local Plugin '${pkg}' projection path exists but is not a junction: plugins/${pkg}`,
          resolveRelative(projPkgPath, root),
          undefined,
          {
            evidence: `not-a-junction:plugins/${pkg}`,
            expected: `junction .opencode/plugins/${pkg} -> src/opencode/plugins/${pkg} (resync is an environment operation: scripts/self-sync.ps1)`,
            route: "intake",
            finding_category: "obsolete-structure",
            finding_level: "strict",
      ...
      linkState.kind === "link" &&
      normalizeLinkTargetForCompare(linkState.target) !==
        normalizeLinkTargetForCompare(expectedSource)
    ) {
      results.push(
        ng(
          "PluginProjection",
          "repo-local-plugin-projection-symmetry",
          `Repo-local Plugin '${pkg}' junction points to an unexpected target (expected: ${expectedSource}, actual: ${linkState.target})`,
          resolveRelative(projPkgPath, root),
          undefined,
          {
            evidence: `junction-wrong-target:plugins/${pkg}`,
            expected: `junction .opencode/plugins/${pkg} -> src/opencode/plugins/${pkg} (resync is an environment operation: scripts/self-sync.ps1)`,
            route: "intake",
            finding_category: "obsolete-structure",
            finding_level: "strict",
    ...
    const shimPath = path.join(projectionPluginsDir, `${pkg}.ts`);
    const shimContent = readText(shimPath);
    if (shimContent === null) {
      results.push(
        ng(
          "PluginProjection",
          "repo-local-plugin-projection-symmetry",
          `Repo-local Plugin loader shim missing: plugins/${pkg}.ts`,
          resolveRelative(shimPath, root),
          undefined,
          {
            evidence: `shim-missing:plugins/${pkg}.ts`,
            expected: `depth-1 loader shim .opencode/plugins/${pkg}.ts re-exporting ${pkg}/plugin.ts (resync is an environment operation: scripts/self-sync.ps1)`,
            route: "intake",
            finding_category: "obsolete-structure",
            finding_level: "strict",
          },
    ...
    } else if (shimContent !== buildPluginLoaderShimContent(pkg)) {
      results.push(
        ng(
          "PluginProjection",
          "repo-local-plugin-projection-symmetry",
          `Repo-local Plugin loader shim content mismatch: plugins/${pkg}.ts`,
          resolveRelative(shimPath, root),
          undefined,
          {
            evidence: `shim-content-mismatch:plugins/${pkg}.ts`,
            expected: `fixed re-export template identical to scripts/self-sync.ps1 New-PluginLoaderShimContent (resync is an environment operation: scripts/self-sync.ps1)`,
            route: "intake",
            finding_category: "document-drift",
            finding_level: "strict",
          },
  ...
  // (b) 余剰投影: 正本にない agentdev-* 投影を検出する。
  for (const entry of projEntries) {
    const name = entry.name;
    // Windows junction は Dirent.isDirectory() が false になる（libuv reparse
    // point 挙動、listDirs と同一の対処）。junction / symlink も dir 候補とする。
    const isDirLike = entry.isDirectory() || entry.isSymbolicLink();
    const isTsFile = entry.isFile() && name.endsWith(".ts");
    if (!isDirLike && !isTsFile) continue;
    const baseName = isTsFile ? name.slice(0, -3) : name;
    if (!baseName.startsWith("agentdev-")) continue;
    if (sourcePackagesSet.has(baseName)) continue;

    if (isTsFile) {
      // loader shim は ADF 生成物であり、正本の対象消滅後も管理物と確定できる
      // （self-sync.ps1 の stale plugin loader shim 検出と同一判定）。
      results.push(
        ng(
          "PluginProjection",
          "repo-local-plugin-projection-symmetry",
          `Stale repo-local Plugin loader shim (absent from src/opencode/plugins): plugins/${name}`,
          resolveRelative(path.join(projectionPluginsDir, name), root),
          undefined,
          {
            evidence: `stale-shim:plugins/${name}`,
            expected: `remove the stale shim or restore src/opencode/plugins/${baseName}/ (resync is an environment operation: scripts/self-sync.ps1)`,
            route: "intake",
            finding_category: "obsolete-structure",
            finding_level: "strict",
          },
    ...
    // ディレクトリ（junction）: リンク先が正本パスに一致する場合のみ ADF 管理物
    // と確定できる（REQ-058-009 機械的基準(1)、REQ-058-008 非破壊境界）。
    const staleExpectedSource = path.resolve(
      root,
      "src",
      "opencode",
      "plugins",
    ...
    const state = inspectProjectionLinkState(
      path.join(projectionPluginsDir, name),
    );
    if (
      state.kind === "link" &&
      normalizeLinkTargetForCompare(state.target) ===
        normalizeLinkTargetForCompare(staleExpectedSource)
    ) {
      results.push(
        ng(
          "PluginProjection",
          "repo-local-plugin-projection-symmetry",
          `Stale repo-local Plugin projection junction (absent from src/opencode/plugins): plugins/${name}`,
          resolveRelative(path.join(projectionPluginsDir, name), root),
          undefined,
          {
            evidence: `stale-junction:plugins/${name}`,
            expected: `remove the stale junction (apply removes it) or restore src/opencode/plugins/${baseName}/ (resync is an environment operation: scripts/self-sync.ps1)`,
            route: "intake",
            finding_category: "obsolete-structure",
            finding_level: "strict",
          },
      ...
      // 管理物判定不能な junction は削除せず報告のみ（REQ-058-008 非破壊境界）。
      results.push(
        info(
          "PluginProjection",
          "repo-local-plugin-projection-symmetry",
          `Junction not managed by AgentDevFlow (left untouched): plugins/${name}`,
          resolveRelative(path.join(projectionPluginsDir, name), root),
          undefined,
          {
            evidence: `unmanaged-junction:plugins/${name}`,
            expected: "managed projection junctions must point to src/opencode/plugins/<name>",
          },
        ),
      );
    }
  ...
  if (
    ...
    results.push(
      ok(
        "PluginProjection",
        "repo-local-plugin-projection-symmetry",
        `Repo-local Plugin projection symmetric (${sourcePackages.length} package(s): junctions + loader shims in sync with src/opencode/plugins)`,
      ),
    );
  }
  return results;
...
async function main(): Promise<void> {
  ...
  const scanned: Record<string, number> = {
    ...
    Guides: fs.existsSync(path.join(root, "docs", "guides"))
      ...
      : 0,
    Specs: collectSpecMarkdownRecursively(designsDir).length,
  ...
  if (options.dryRun) {
    const targets = [
      ...
      `Specs: ${designsDir} (existence verified against ${designsDir}/README.md, v2:REQ-0154-001/003)`,
  ...
  const results: CheckResult[] = [
    ...
    ...checkDesignsExistence(designsDir, root),
    ...
    ...checkReqDesignBoundaryViolation(root), // IR-044 (v2:REQ-0108-259)
    ...checkGhDirectInvocation(root), // IR-053 (v2:REQ-0152-001/002)
    ...checkDraftSpecStaleness(designsDir, root), // IR-054 (v2:REQ-0154-002)
