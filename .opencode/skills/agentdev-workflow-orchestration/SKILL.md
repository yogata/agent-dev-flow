...
3. **Wave 順次実行**（Wave 番号昇順）:
   ...
   e. **Wave完了後ステータス一括更新**: 完了した Wave 内の全子 Issue について、Epic ステータス追跡テーブルを一括更新する（REQ-0106-018）。成功した子 Issue は `🔄 進行中` → `✅ 完了` に更新。失敗した子 Issue は `🔄 進行中` を維持。一括更新は親エージェントが `gh issue edit` で単一の API 呼び出しで実行し、サブエージェントによる競合更新を回避する
   f. **Wave間 rebase**: 次の Wave を開始する前に、`git rebase origin/main` を各子 Issue の worktree ブランチ（`{type}/issue-{N}`）に対して実行する（REQ-0106-019）。rebase 衝突が発生した場合はrebaseを中止し（`git rebase --abort`）、衝突内容をユーザーに報告して指示を待つ（自動的な衝突解決は禁止）
   g. **後続 Wave 制御**: 失敗影響を評価し後続 Wave の実行可否を判定（後述「Wave失敗時後続制御」）
4. **全 Wave 完了後**:
...
- **書き込み事後検証**（REQ-0106-020, MUST）: `gh issue edit`、`gh pr create` 等 `gh` CLI の書き込み操作を実行した後、Read tool で書き込み結果を読み戻し、意図した内容が正しく反映されたことを必ず検証すること。エンコーディング崩れ・テンプレート必須セクション欠落・制御文字混入がないことを確認し、差異がある場合は再実行またはエラー報告すること
