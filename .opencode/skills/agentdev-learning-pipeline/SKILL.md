...
learning pipeline（capture → refine → promote）の共通知識ベース。schema・分類基準・評価ディメンション・prune 方針・責任分界を定義し、learning-refine / learning-promote が参照する。
...
- **refactor**（learning-refine command）: 問題クラス分類・8軸評価・evaluation-report 生成・inbox→archive 移動
- **elevate**（learning-promote command）: 昇華判定・Requirement Source stub 生成・prune
...
| refactor | learning-refine（command） | 正規化・問題クラス分類・8軸評価・evaluation-report 生成・archive 移動 |
| elevate | learning-promote（command） | 廃棄判定・Requirement Source stub 生成・prune |
...
```
elevation-staging/ → req-define（明示入力ファイル）→ req-save → case-open → case-run
```

- staging stub は `req-define` の「明示入力ファイル」として扱われる
- staging stub の形式要件は @reference/staging-stub-schema.md を参照
