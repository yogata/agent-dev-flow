...
```
capture → inbox.md → refine → archive/active.md + evaluation-report.md → promote → promoted/
```
...
| archive/active.md | living pool（終端保管ではない）。refine で inbox から移動した entry を保持。promote の入力として参照され、promote 時の prune により動的に変化する。`archive` は終端保管を意味しない | 動的プール。promote のたびに内容が変化し、未処理 entry は次回 promote の対象として残る | refine が inbox から移動して書き込む。promote が読み取り・prune する。capture は参照しない |
