---
name: command-authoring-best-practices
...
`.opencode/skills/command-authoring-best-practices/`
...
例: Command本文に `issue-template-manager` のテンプレートを参照する記述がある場合、`load_skills` に `issue-template-manager` を含める。
...
- **skill-authoring-best-practices**: Skill品質基準（Command測定基準は本スキルが提供）
