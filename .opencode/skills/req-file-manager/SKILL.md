...
> **運用境界（current）**: 現時点の実行系（issue-save-req, req-file-manager）は per-file（REQ-{NNNN}.md）構造で動作する。`{area}.md` への直接保存および `INDEX.md` の生成・更新は行わない。area-based 構造への移行は別 Issue で実施する。
...
> **運用境界（future）**: 以下は将来の標準構造である。現時点では適用されない。実行系は per-file 構造で動作する。

docs/requirements/ の将来の標準構造を定義する。現在は per-file（REQ-{NNNN}.md）構造で運用しており、area-based 構造への移行は別 Issue で段階的に実施する。
...
├── README.md          # 運用説明（手動管理）
├── INDEX.md           # generated artifact（手動編集禁止）
├── core.md            # 初期・暫定area（area_status: provisional）
├── {area}.md          # 安定area（area_status: stable）
└── REQ-{NNNN}.md      # 移行対象（将来廃止予定）
```

- **README.md**: 運用説明・ディレクトリガイド
- **INDEX.md**: generated artifact。area一覧（status付き）とREQ-to-area mapping を含む。手動編集は禁止
- **core.md**: 初期・暫定area。全要件をまず core.md に配置し、凝集度が確認できた段階で {area}.md に分割する
- **{area}.md**: 安定area。natural cohesion が確認された領域単位

### 不採用の構造

以下は作成しない:
- `unclassified.md`: 未分類要件を正式要件として固定することを防ぐため
- `docs/requirements/sources/`: 外部ソース参照用ディレクトリは不要
- `docs/requirements/areas/`: area格納用サブディレクトリは不要（area file は requirements/ 直下に配置）

---

## area file frontmatter 規約

area file（`core.md`, `{area}.md`）の frontmatter は以下の最小構成とする:

```yaml
---
area_id: core
area_status: provisional  # provisional | stable | deprecated
status: active
last_reviewed: null
review_policy: early-stage  # early-stage | regular | stable
---
```

| フィールド | 型 | 説明 |
|-----------|-----|------|
| area_id | string | area の一意識別子（ファイル名の拡張子なし部分） |
| area_status | enum | `provisional`（暫定）/ `stable`（安定）/ `deprecated`（非推奨） |
| status | enum | `active`（有効）/ `archived`（アーカイブ済み） |
| last_reviewed | date/null | 最終レビュー日（未レビュー時は null） |
| review_policy | enum | `early-stage`（頻繁レビュー）/ `regular`（定期）/ `stable`（安定後） |

**owner_skill は含めない**: area file の管理責務は req-file-manager スキルにあるが、frontmatter には記載しない。スキルとファイルの対応はスキル定義（SKILL.md）で管理する。

**title は H1 で管理**: frontmatter の `title` フィールドは持たず、ファイル本文の H1 見出しで管理する。

**created_at / updated_at は Git 履歴で代替**: frontmatter に日付フィールドを持たず、Git 履歴で管理する。

---

## REQ-ID 安定ID 規約

REQ-ID（`REQ-{NNNN}`）は安定IDであり、area情報やファイル配置に依存しない。

- REQ-ID に area 情報を含めない（例: `REQ-core-0001` とはしない）
- area はファイル配置（`core.md`, `{area}.md`）と frontmatter / generated INDEX で管理する
- REQ-ID はファイル間の移動（area の分割・統合）が発生しても変更しない
...
area file 内の各REQセクションに、上記メタデータを構造化して記述する。frontmatter フィールドとしては管理しない。

---

## INDEX.md 生成方針

INDEX.md は generated artifact であり、req-file-manager スキルが管轄する。

### 生成内容

- area 一覧（area_id, area_status, review_policy）
- REQ-to-area mapping（REQ-ID → area_id の対応表）
- 生成日時

### 生成ルール

- 手動編集は禁止（変更は area file / REQ file の更新に伴う再生成で対応）
- 生成の実装は将来の Issue で対応（現在は未実装）
- 生成タイミング: REQ/area file の変更時（issue-save-req, issue-close 等）
...
- **次工程**: finding は issue-req の明示入力ファイルとして渡され、正式な要件変更（CREATE/APPEND/UPDATE）に変換される
- **SPLIT 検出時**: issue-save-req が SPLIT を検出した場合、保存可能範囲（CREATE/APPEND/UPDATE）を実行した後に finding を作成する
- **詳細**: `issue-lifecycle` reference の `requirements-review-finding-protocol.md` を参照
