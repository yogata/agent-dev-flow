...
> **運用境界（current）**: 現時点の実行系（issue-save-req, agentdev-req-file-manager）は per-file（REQ-{NNNN}.md）構造で動作する。`{area}.md` への直接保存および `INDEX.md` の生成・更新は行わない。area-based 構造への移行は別 Issue で実施する。
...
**owner_skill は含めない**: area file の管理責務は agentdev-req-file-manager スキルにあるが、frontmatter には記載しない。スキルとファイルの対応はスキル定義（SKILL.md）で管理する。
...
INDEX.md は generated artifact であり、agentdev-req-file-manager スキルが管轄する。
