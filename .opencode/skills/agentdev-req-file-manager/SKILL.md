...
REQ保存処理中にREQ体系上の歪みを検知した場合、REQ再構成intakeとして `.agentdev/intake/inbox/req-restructure/` に保存する（REQ-0109, REQ-0109）。
...
- REQ再構成候補の検知はSHOULD。検知時は通常のREQ保存処理を妨げず、非同期に保存する。検知した構造上の問題・根拠となる要件行・セクション・編集文脈・想定される再構成方向を記録する（REQ-0109）。
- REQ再構成intakeは保存時点でREQの分割・統合・移動・廃止を要求するものではなく、将来のREQ再構成レビューの入力材料として扱う（REQ-0109）。
...
| コマンド | タイミング | 要件ID |
|----------|-----------|--------|
| `req-define` | 要件展開（Step 4）の直後、ADR判定（Step 5）の前 | REQ-0109, REQ-0109 |
| `req-save` | CREATE対象REQの保存前（Step 3a） | REQ-0109, REQ-0109 |
...
1. 反映作業のみの候補は、独立した要件行として扱わない（REQ-0109）
2. 反映作業のみの候補は、後続工程への移送候補として以下のいずれかに振り分ける:
   - 対象REQ/ADR/SPEC等への UPDATE / APPEND
   - 関連ドキュメント更新候補（req-define の Step 4b で管理）
   - 後続Caseの変更対象
3. `req-save` での検査（REQ-0109, 016）: CREATE対象REQの保存前に反映作業のみの要件行が残っていないか検査し、検出時は保存を停止して該当行・判定理由・移送先を報告する
...
REQ CREATE / APPEND / UPDATE 時に `docs/DOC-MAP.md` への影響を確認するルール（REQ-0101 ~ 037）。
...
- DOC-MAPの更新は探索経路の更新であり、要件・判断・仕様の更新ではない（REQ-0101）
