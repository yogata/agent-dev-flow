...
> **運用境界（current）**: 現時点の実行系（req-save, agentdev-req-file-manager）は per-file（REQ-{NNNN}.md）構造で動作する。`{area}.md` への直接保存および `INDEX.md` の生成・更新は行わない。area-based 構造への移行は別 Issue で実施する。
...
- `req-save` の Step 7（docs変更整合性検証）で実行
- `case-close` の Step 3（docs検証）で実行
...
- 生成タイミング: REQ/area file の変更時（req-save, case-close 等）
