...
REQ保存処理中にREQ体系上の歪みを検知した場合、REQ再構成intakeとして `.agentdev/intake/inbox/req-restructure/` に保存する（REQ-0050-004, REQ-0050-007）。
...
- REQ再構成候補の検知はSHOULD。検知時は通常のREQ保存処理を妨げず、非同期に保存する。検知した構造上の問題・根拠となる要件行・セクション・編集文脈・想定される再構成方向を記録する（REQ-0050-007）。
- REQ再構成intakeは保存時点でREQの分割・統合・移動・廃止を要求するものではなく、将来のREQ再構成レビューの入力材料として扱う（REQ-0050-006）。
