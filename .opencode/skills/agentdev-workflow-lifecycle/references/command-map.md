...
**バグ修正・軽微変更（bugfix）**: `/agentdev/req-define` → `/agentdev/case-open` → `/agentdev/case-run` → `/agentdev/case-close`
**機能追加（feature）**: `/agentdev/req-define` → `/agentdev/req-save` → `/agentdev/case-open` → `/agentdev/case-run` → `/agentdev/case-close`
...
**基本フロー**: 学び発生 → `learning-capture`（スキル）→ `/agentdev/learning-refine` → `/agentdev/learning-promote` → `/agentdev/backlog-review` → `/agentdev/backlog-save` → `/agentdev/req-define`

| コマンド | 役割 |
|---|---|
| `/agentdev/learning-refine` | 問題クラス分類→8軸評価→archive移動 |
| `/agentdev/learning-promote` | 昇華判定→promoted artifact 生成（`promoted/*.md`） |
| `/agentdev/backlog-review` | promoted artifact を分析し、HITLで統合・分割判定、review draftを保存 |
| `/agentdev/backlog-save` | review draft から RU（Requirement Unit）を生成・git永続化 |

**反映ルート**: promoted artifact → `/agentdev/backlog-review`（HITL・review draft保存）→ `/agentdev/backlog-save`（RU 生成）→ `/agentdev/req-define`（RU を Requirement Source として読み込み）→ `/agentdev/req-save` → `/agentdev/case-open` → `/agentdev/case-run`

### intake ワークフロー（気づき・課題の収集）

**基本フロー**: 積み残し候補発生 → `/agentdev/intake-capture` / `/agentdev/intake-from-github` → `/agentdev/intake-review` → `/agentdev/intake-promote` → `/agentdev/backlog-review` → `/agentdev/backlog-save` → `/agentdev/req-define`
...
| `/agentdev/backlog-review` | promoted artifact（intake/learning）を分析し、HITLで統合・分割判定、review draftを保存 |
| `/agentdev/backlog-save` | review draft から RU（Requirement Unit）を生成・git永続化 |

### REQ再構成intakeワークフロー（REQ-0109）

REQ再構成intake: req-save / case-close で検知 → `inbox/req-restructure/` に保存 → intake-review で判定（通常intakeと独立基準）→ `accepted/req-restructure/` または `archive/rejected/req-restructure/` → backlog-review/backlog-save 除外（REQ-0109）
...
| `/agentdev/backlog-review` | promoted artifacts（intake/learning） | `.agentdev/backlog/drafts/review-draft-*.md` | バックログレビュー |
| `/agentdev/backlog-save` | `.agentdev/backlog/drafts/review-draft-*.md` | `.agentdev/backlog/req-units/RU-*.md` | RU 生成 |
...
| `/agentdev/backlog-review` | — | — | — | — | READ | — |
| `/agentdev/backlog-save` | — | — | — | — | — | WRITE（backlog） |
...
/agentdev/intake-capture / /agentdev/intake-from-github → .agentdev/intake/inbox/ → /agentdev/intake-review → /agentdev/intake-promote → .agentdev/intake/promoted/ → /agentdev/backlog-review → .agentdev/backlog/drafts/review-draft-*.md → /agentdev/backlog-save → .agentdev/backlog/req-units/RU-*.md → /agentdev/req-define
/agentdev/learning-refine(inbox → archive + evaluation-report) → /agentdev/learning-promote → .agentdev/learning/promoted/ → /agentdev/backlog-review → .agentdev/backlog/drafts/review-draft-*.md → /agentdev/backlog-save → .agentdev/backlog/req-units/RU-*.md → /agentdev/req-define
```
...
### work_type との区別（REQ-0103-016）

Implementation pattern は command の内部構造に基づく分類軸であり、REQ-0104 で定義される work_type（Issue 種別分類）とは直交する概念である。work_type は Issue 種別とワークフロー分岐を示し、implementation pattern は command の責務境界と許容副作用を示す。
