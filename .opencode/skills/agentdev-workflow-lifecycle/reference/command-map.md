...
**基本フロー**: 気づき発生 → `intake-capture` / `intake-from-github` → `intake-review` → `intake-promote` → `req-define` または `intake-open`
...
| `/agentdev/intake-promote` | review 済み item を後続コマンド用 artifact に整形（req-define または intake-open 用） |
