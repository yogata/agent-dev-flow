...
### REQ再構成intakeワークフロー（REQ-0050）

REQ再構成intake: req-save / case-close で検知 → `inbox/req-restructure/` に保存 → intake-review で判定（通常intakeと独立基準）→ `accepted/req-restructure/` または `archive/rejected/req-restructure/` → req-backlog 除外（REQ-0050）
