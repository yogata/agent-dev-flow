...
**バグ修正・軽微変更（Pattern A）**: `req-define` → `case-open` → `case-run` → `case-close`
**機能追加（Pattern B）**: `req-define` → `req-save` → `case-open` → `case-run` → `case-close`
...
**基本フロー**: 積み残し候補発生 → `intake-capture` / `intake-from-github` / `integrity-check` → `intake-review` → `req-define` または `intake-promote` → `intake-open`
...
| `/agentdev/intake-promote` | review 済み intake item を req-define / intake-open 用入力 artifact に整形 |
...
- **req-define**: 要件docを壁打ちで構築し、機能追加の場合はドラフトを保存する（draft WRITE）
