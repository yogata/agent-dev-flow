...
**基本フロー**: 積み残し候補発生 → `/agentdev/intake-capture` / `/agentdev/intake-from-github` / `/agentdev/integrity-check` → `/agentdev/intake-review` → `/agentdev/req-define` または `/agentdev/intake-promote` → `/agentdev/intake-open`
