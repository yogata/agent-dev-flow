...
**基本フロー**: 積み残し候補発生 → `/agentdev/intake-capture` / `/agentdev/intake-from-github` → `/agentdev/intake-review` → `/agentdev/req-define` または `/agentdev/intake-promote` → `/agentdev/intake-open`
...
| `/agentdev/intake-review` | `.agentdev/intake/inbox/` | review判定（item内） | レビュー |
