...
| `intake-open` | `issue_desc_backlog_epic.md` / `issue_desc_backlog_child.md` テンプレートでEpic + 子Issueを作成 |
...
BacklogはPattern A〜Dとは独立した専用分類である。`intake-from-github` → `intake-open` の経路で処理され、Pattern判定の対象外となる。
