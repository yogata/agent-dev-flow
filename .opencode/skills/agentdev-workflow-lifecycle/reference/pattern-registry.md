...
| `req-backlog` | promoted artifact から RU を生成。`issue_desc_backlog_epic.md` / `issue_desc_backlog_child.md` テンプレートでEpic + 子Issueを作成する場合もある |
...
BacklogはPattern A〜Dとは独立した専用分類である。`intake-from-github` → `req-backlog` の経路で処理され、Pattern判定の対象外となる。
