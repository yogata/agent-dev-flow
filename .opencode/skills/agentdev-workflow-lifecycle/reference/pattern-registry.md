...
Pattern A で ADR が必要な場合（`agentdev-adr-guidelines` の閾値判定で「必要」となった場合）、Pattern B に昇格する。昇格時は `enhancement`, `feature` ラベルを付与し、`req-save` を実行する。REQファイル・ADRファイルの作成が必要となるため。
...
Pattern C/Dはlightweight workflowである。Issue作成には`issue_desc_feature.md`テンプレートを使用し、コメント・close操作にはPattern Aのテンプレートを再利用する。

- **Issue テンプレート**: `issue_desc_feature.md`
- **コメント テンプレート**: `issue_comment_bug_analysis.md`
- **close テンプレート**: `issue_comment_bug_record.md`

> **注意**: Pattern C/Dはバグ修正ではないが、コメント・closeテンプレートには専用テンプレートが存在しないためPattern Aのテンプレートを再利用する。Issueテンプレートは要件分析の決定に基づき`issue_desc_feature.md`を使用する。
...
## Backlog 分類（Pattern A-D とは独立）

BacklogはPattern A〜Dとは独立した専用分類である。`intake-from-github` → `backlog-review` → `backlog-save` の経路で処理され、Pattern判定の対象外となる。
...
- Pattern B（機能追加）のEpicフローとはテンプレート・ラベルが異なる点に注意
