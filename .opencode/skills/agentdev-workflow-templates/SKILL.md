...
| `issue_desc_epic.md` | Epic Issue本文 | case-open, intake-open (Epic flow) | B (Epic) |
| `issue_desc_child.md` | 子Issue本文 | case-open, intake-open (Epic flow) | B (Epic) |
...
### Issue作成時のテンプレート選定（intake-open）

intake-open は intake-promote が生成した promoted artifact を読み込み、artifact の内容に応じてテンプレートを選択する。promoted artifact に frontmatter（`route`, `status` 等）は不要。route/status はディレクトリ配置（`.agentdev/intake/promoted/`）で表現される:

| 条件 | 本文テンプレート |
|------|-----------------|
| 単一 Issue（機能追加・改善） | `issue_desc_feature.md` |
| 単一 Issue（バグ修正） | `issue_desc_bug.md` |
| Epic Issue | `issue_desc_epic.md` |
| 子Issue（Epic 配下） | `issue_desc_child.md` |
