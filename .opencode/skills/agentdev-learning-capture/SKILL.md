...
**広く捉え、下流に委ねる。** エージェント SHALL は学び候補を広く抽出する。取捨選択・昇格判断は下流の `/agentdev/learning-refine`・`/agentdev/learning-promote`・prune で行うため、capture 段階では以下の方針をとる：

- **false positive を恐れない** — わずかでも学びの可能性があれば抽出する
- **品質評価を保留する** — 「これは本当に学びか？」の判断は refine に任せる
