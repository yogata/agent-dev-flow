...
describe("parseArgs", () => {
  ...
  it("handles a combination of all options", () => {
    const opts = parseArgs(["--help", "--json", "--dry-run", "path/a", "path/b"]);
    expect(opts).toEqual({
      help: true,
      json: true,
      dryRun: true,
...
describe("printHelp", () => {
  it("outputs USAGE, OPTIONS, and EXIT CODES sections", () => {
    ...
    try {
      printHelp("my-script", "validates stuff", "my-script [options] [paths...]");
    } finally {
...
describe("computeSummary", () => {
  ...
  it("counts each level correctly", () => {
    ...
    expect(computeSummary(results)).toEqual({ ok: 2, ng: 1, warning: 1, info: 2 });
  });

  it("handles single result", () => {
    expect(computeSummary([ng("x", "y", "z")])).toEqual({ ok: 0, ng: 1, warning: 0, info: 0 });
  });
});
...
describe("determineExitCode", () => {
  it("returns EXIT_OK when all ok", () => {
    expect(determineExitCode({ ok: 5, ng: 0, warning: 0, info: 3 })).toBe(EXIT_OK);
  });

  it("returns EXIT_NG when ng > 0", () => {
    expect(determineExitCode({ ok: 5, ng: 1, warning: 0, info: 0 })).toBe(EXIT_NG);
  });

  it("returns EXIT_NG when warning > 0", () => {
    expect(determineExitCode({ ok: 5, ng: 0, warning: 2, info: 0 })).toBe(EXIT_NG);
  });

  it("returns EXIT_NG when both ng and warning > 0", () => {
    expect(determineExitCode({ ok: 0, ng: 1, warning: 1, info: 0 })).toBe(EXIT_NG);
  });

  it("returns EXIT_OK with only info results", () => {
    expect(determineExitCode({ ok: 0, ng: 0, warning: 0, info: 10 })).toBe(EXIT_OK);
  });
});
...
describe("findRepoRoot", () => {
  ...
  it("finds .opencode when starting inside the worktree scripts dir", () => {
    const path = require("path");
    const scriptDir = path.join(process.cwd(), ".worktrees", "328-feature", ".opencode", "skills", "agentdev-integrity", "scripts");
