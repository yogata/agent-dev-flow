...
description: Provides quality criteria and best practices for authoring OpenCode command definitions (`.opencode/commands/`), including line count thresholds, step structure standards, load_skills rules, and orchestration skill judgment criteria. USE FOR: creating new commands, improving existing commands, reviewing command quality, deciding whether to extract orchestration skills, validating load_skills consistency, or checking DoD compliance for command/skill changes. DO NOT USE FOR: creating skills, writing templates, general coding tasks, or documentation fixes.
---
...
- **Steps数**: 5〜12個
- **load_skills 妥当性**: 即時必要 skill のみ。pattern別目安上限を参照
...
- **references/command-authoring-standards.md**: 行数目安・Steps数目安・load_skills budget・共通処理重複・canonical path・完了報告 variant・後方互換性の詳細
