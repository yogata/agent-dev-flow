...
- G19: 各フェーズのべき等チェックは必ず実行する
- G20: フェーズ境界でエラーが発生した場合、Step 0から再開することで安全に復旧できる
...
- G21: vibe-coding（実装先行）の場合も乖離報告を省略しない
- G23: PR本文の生成時に `pr_desc.md` テンプレートの【必須】セクションが全て含まれていることを確認すること
- G24: 乖離検出報告の生成時に `report_spec_compliance.md` テンプレートの【必須】セクションが全て含まれていることを確認すること
- G25: 完了判定はIssue本文の `完了条件` セクションを品質ゲートとして参照すること
...
- G22: gh CLI出力を読み取る際は `gh-cli-best-practices` の安全な読み取り手順に従うこと
