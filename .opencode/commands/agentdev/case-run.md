...
**Step 12**: 完了報告 → `agentdev-workflow-reporting` の完了報告variantに従って出力。実行モードに応じたvariantを選択:
    ...
    - Epic Orchestrator全Wave成功 → completion-reports/case-run/all-success.md
    - Epic Orchestrator一部失敗 → completion-reports/case-run/partial-fail.md
    - Epic Orchestrator全失敗 → completion-reports/case-run/all-fail.md
