...
**Step 4**: work_type 判定 → `agentdev-workflow-lifecycle` の workflow classification に従って bugfix/B/C/D を判定し、以降のStepの分岐を決定（バグ修正・軽微変更/機能追加/リファクタリング・保守作業/ドキュメント・雑務）
...
**Step 8**: 実装完了後、乖離検出 → `agentdev-spec-compliance` に従ってチェック。品質メトリクス収集も併せて実行。grep対象は `docs/` 全体（`docs/specs/` に限定せず、`docs/requirements/`、`docs/adr/`、`docs/guides/` 等を含む）とする（SHALL — REQ-0106-021）
...
**Step 10**: specs更新・関連ドキュメント整合性確認（2つの独立した要件）:
   - **work_type: feature（旧 feature）専用要件**: `docs/specs/system.md` または `docs/specs/patterns.md` を更新する（SHALL）
...
**Step 12**: 完了報告 → `agentdev-workflow-reporting` の完了報告variantに従って出力。実行モードに応じたvariantを選択:
    - 単一Issue → completion-reports/case-run/standard.md
    - Epic Orchestrator全Wave成功 → completion-reports/case-run-epic/all-success.md
    - Epic Orchestrator一部失敗 → completion-reports/case-run-epic/partial-fail.md
    - Epic Orchestrator全失敗 → completion-reports/case-run-epic/all-fail.md
...
以下の skill は frontmatter に宣言せず、必要時に manual reference として参照する（REQ-0103-032）:
...
- `agentdev-no-ai-slop-writing`: PR 本文生成時に参照（REQ-0103-032: 常時 load 禁止）
...
- G07: work_type: feature（旧 feature）の場合、実装結果を `docs/specs/system.md` または `docs/specs/patterns.md` に反映すること — Step 10で更新
...
- G10: Pattern分岐の判定基準と固有ルールは `agentdev-workflow-lifecycle` → workflow classification を参照
...
実装・検証フェーズで本筋外の不整合・規約違反・小改善候補を発見した場合の取り扱い。`agentdev-workflow-lifecycle` → `references/capture-boundaries.md` の Split Rule を SSoT とする。
