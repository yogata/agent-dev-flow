...
2. 前提確認: チェックボックス全完了確認、PR存在確認:
    ...
    - 未チェックのチェックボックス（`- [ ]`）が1件でも存在する場合、**即座にエラー停止**する（G08）。以下のエラーメッセージを表示し、Step 3以降に進まない:
      ```
      ## 完了ゲートエラー: 未完了チェックボックス検出

      **Issue**: #{N}
      **未完了項目数**: {count}件
      **未完了項目一覧**:
      {未チェック項目のリスト}
      **ユーザーアクション**: 未完了項目を完了させるか、不要な場合はチェックボックスを削除してから再実行してください
      ```
...
- G08: 未完了チェックボックス（`- [ ]`）がある場合はStep 2でエラー停止する。Step 5（テスト戦略チェックボックス更新）は完了ゲート通過後のみ実行される
