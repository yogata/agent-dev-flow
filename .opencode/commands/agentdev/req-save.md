    ...
    - **hash 一致検証**: `git rev-parse HEAD` で pull 後の commit hash を取得し、Step 2 で記録した読込時 hash と一致することを確認する
        ...
        - **一致しない場合**: リモートに新規変更が取り込まれたため、Step 2 の読込からやり直す（評価・承認を再実行）。構造化メッセージを表示:
          ...
          **アクション**: リモートに新規変更が取り込まれたため、読込からやり直します
          ```
9b. RU ファイル削除（SHALL — REQ-0105, 0039-017）:
...
12. 完了報告 → `agentdev-workflow-reporting` の完了報告フォーマット（completion-reports.md → req-save 完了時）に従って出力。SPLIT検出時・DOC-MAP更新時の variant も completion-reports.md に定義されている
...
### ファイル操作制約
- G02: ファイル編集スコープ: 以下のパスのみ作成・編集・削除を許可: `docs/requirements/**`（REQファイル）、`docs/adr/**`（ADR）、`docs/README.md`（ドキュメントハブ）、`.sisyphus/drafts/**`（ドラフトstatus更新用）、`.agentdev/backlog/req-units/**`（RU削除用 — REQ-0105）
