...
**注記**: `REQ-{NNNN}.md` を永続正本とする。`{area}.md` への保存は行わない（ADR-0007）。
...
8. views 影響確認: REQ操作が `docs/requirements/views/*.md` に影響するか確認する（REQ-0004-042）。影響がある場合は views を更新する。影響がない場合は「views更新なし」とする。更新が必要だが範囲を超える場合は follow-up とする（REQ-0004-044）。影響確認ルールの詳細は `agentdev-req-file-manager` スキルの「views 影響確認ルール」セクションを参照
9. 変更範囲検証: `git diff --name-only` で変更ファイル一覧を取得し、`docs/` 以外の変更が含まれていればエラー内容をユーザーに報告して指示を待つ（変更の自動破棄は行わない）
10. コミット・プッシュ → `agentdev-conventional-commits` に従ってコミットメッセージを生成し、mainブランチに push
11. ドラフトの `## draft-meta` セクションの `status` を `saved` に更新する
12. 完了報告 → `agentdev-workflow-reporting` の完了報告フォーマット（completion-reports.md → req-save 完了時）に従って出力
    ...
    Finding を作成しました: `.sisyphus/drafts/requirements-review-finding-{topic-slug}.md`
     ...
     views更新が不要だった場合:
     ```
     views更新: なし（関連する views/*.md が存在しないため）
     ```
...
### 委譲・参照制約
- G07: Pattern分岐の判定基準と固有ルールは `agentdev-workflow-lifecycle` → Pattern Registry を参照
