...
4. REQ ファイル操作 → `agentdev-req-file-manager` の判定ロジックと採番ルールに従って実行。ドラフトに複数の `req-operation` + `target-req` ペアが含まれる場合は、各ペアを順次処理する（バルクUPDATE対応）:
    - **CREATE**: テンプレート適用（目的/要件/適用範囲構造）、最大REQ番号+1で採番、`docs/requirements/REQ-{NNNN}.md` に保存
      ※ area-based構造の将来標準に対応するため、REQファイルは area file（{area}.md）内のセクションとしても管理される予定。現在は per-file（REQ-{NNNN}.md）構造で運用する
...
9. コミット・プッシュ → `agentdev-conventional-commits` に従ってコミットメッセージを生成し、mainブランチに push
10. ドラフトの `## draft-meta` セクションの `status` を `saved` に更新する
11. 完了報告 → `agentdev-workflow-reporting` の完了報告フォーマット（completion-reports.md → req-save 完了時）に従って出力
