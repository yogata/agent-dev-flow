...
12. 完了報告 → 完了報告templateに従って出力。実行結果に応じたvariantを選択:
     - SPLIT検出 → templates/req-save/split-detected.md（{docmap_status}変数あり）
     - DOC-MAP更新あり（SPLITなし）→ templates/req-save/docmap-updated.md
     - DOC-MAP確認・更新不要（SPLITなし）→ templates/req-save/docmap-not-needed.md
     - Epic規模 → templates/req-save/epic.md
     - 上記以外 → templates/req-save/standard.md
