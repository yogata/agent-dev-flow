...
完了報告 → `agentdev-workflow-reporting` の完了報告フォーマット（completion-reports.md → intake-open 一括処理完了時）に従って出力。git 永続化結果（変更有無・ファイル一覧・commit hash・push 成否）を含める
...
### 実行制約
- G07: GitHub Issue/PR の読み書きは `gh` CLI のみ使用（GitHub API 直接呼び出し不可）。ローカル artifact の読み取りはこの制約の対象外
