...
完了報告 → 完了報告templateに従って出力。template: .opencode/commands/agentdev/templates/learning-refine/standard.md。git 永続化結果（変更有無・ファイル一覧・commit hash・push 成否）を含める
...
各成果物（inbox.md, archive/active.md, evaluation-report.md）の役割・性格・lifecycle 詳細は `agentdev-learning-pipeline` skill の「Artifact Lifecycle」を参照。

**learning-refine の責務**: normalize → classify → evaluate → move。昇格判定は `/agentdev/learning-promote` の役割。
