...
1. **スクリプト実行**: `agentdev-integrity` の検査スクリプト（`scripts/check_integrity.ts`）を実行。検査カテゴリ・対象パス・検出結果の分類は `agentdev-integrity` SKILL.md の検査カテゴリ定義に準拠
   - 検査カテゴリ: REQ frontmatter ↔ ファイル名、ADR ↔ REQ 相互参照、Skill ↔ command 参照、frontmatter 許可フィールド（description + agent のみ）、旧 namespace 残存、完了報告フォーマット、DOC-MAP 存在性・参照整合性、Views 残存、README ↔ ファイル整合性
