...
- prune 対象の特定基準、削除禁止エントリ、実施フローは `tips-pipeline-orchestration` skill の「Prune 方針 → refactor 時 prune」を参照
...
- **evaluation-report.md は境界成果物**: tips-refactor と tips-elevate 間の受け渡し物
- **archive.md は生きている tips プール**: 永久保存先ではなく、prune・elevate 時 prune で動的に変化する
