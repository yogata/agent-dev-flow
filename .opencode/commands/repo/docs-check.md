...
- **知識文書構造検査（REQ-056-010/011）**: `check_knowledge_docs.ts`（`.opencode/skills/repo-agentdev-integrity/scripts/check_knowledge_docs.ts`）を `bun run` で併せて実行する。docs/knowledge/ 配下の正規配置（1知識1 Markdown ファイル、サブディレクトリ・非 Markdown ファイルの検出）、kebab-case slug 命名、必須内容5項目（知識内容、適用条件、適用対象、根拠、関連知識）の見出し存在を検査する。領域案内 README.md は検査対象外。知識文書の意味的妥当性は検査対象に含まない（REQ-056-011、検査範囲は構造面に限定）。check_knowledge_docs.ts の failure は docs-check 全体を fail とする
- **Design frontmatter 必須キー検査（IR-070、checker-execution-contracts Design「Design frontmatter 必須キー検証観点」）**: `check_design_frontmatter.ts`（`.opencode/skills/repo-agentdev-integrity/scripts/check_design_frontmatter.ts`）を `bun run` で併せて実行する。docs/designs/** の Design frontmatter について必須キー（`title` / `status` / `created` / `updated`）の欠落・空値、行頭空白付きキー行によるキー名破損（「updated値のキー名欠落」）、値形式不正（ISO 8601 日付形式、status の draft / accepted 値域）を検査する。README.md（Design インデックス）と references/ / audits/ / baselines/ 配下、baseline_for / audit_for 信号キー保持ファイルは検査対象外（IR-070 exemption）。check_design_frontmatter.ts の failure は docs-check 全体を fail とする
