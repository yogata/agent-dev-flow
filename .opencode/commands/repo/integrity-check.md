---
description: agent-dev-flow repo self-audit (repo-local, not distributed to consumers)
agent: sisyphus
---

# Repo Self-Audit: Integrity Check
...
1. **スクリプト実行**: `repo-agentdev-integrity` の検査スクリプト（`.opencode/skills/repo-agentdev-integrity/scripts/check_integrity.ts`）を実行。検査カテゴリ・対象パス・検出結果の分類は `repo-agentdev-integrity` SKILL.md の検査カテゴリ定義に準拠
    - 検査カテゴリ: REQ frontmatter ↔ ファイル名、ADR ↔ REQ 相互参照、Skill ↔ command 参照、frontmatter 禁止フィールド、旧 namespace 残存、完了報告フォーマット、DOC-MAP 存在性・参照整合性、Views 残存、README ↔ ファイル整合性、ADR status 正規化、RU-ID 根拠参照、workflow status 禁止、accepted ADR のみ現行根拠引用
    - Finding 分類: `document-drift` / `broken-reference` / `obsolete-structure` / `canonical-conflict` / `workflow-gap` / `integrity-rule-gap`
