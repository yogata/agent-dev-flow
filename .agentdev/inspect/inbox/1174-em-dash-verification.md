---
title: "em-dash 査読是正 検証レポート: AGENTS.md (0 件, Issue #1174)"
source_issue: 1174
parent_issue: 1169
operation_unit: OU-002
source_ru: RU-0006
generated_at: 2026-06-25T22:36:57+09:00
...
scope: AGENTS.md (リポジトリルート、単一ファイル, src/opencode-local/ 除く)
exclusions: テーブルセル `| — |` (PR #1122 対象外, CR-004)
---

# em-dash 査読是正 検証レポート: AGENTS.md (Issue #1174)

## 目的

OU-002 Epic #1169 Wave 1 child Issue #1174 の完了条件を満たすため、リポジトリルートの `AGENTS.md` の em-dash 本体（半角空白挟み ` — `）残存件数を機械判定アルゴリズムで確定する。PR #1163 カタログ（commit ca95b945, merge e3486488）では当該ファイルの em-dash 本体件数を 0 件と報告しており、本検証はその再確認である。

## 検証対象

- 対象ファイル: `AGENTS.md`（リポジトリルート、単一ファイル、1787 bytes）
...
`Select-String -Path "AGENTS.md" -Pattern " — " -SimpleMatch`（半角空白 + U+2014 + 半角空白）を適用した結果、一致件数 0 件。

- テーブルセル `| — |`（`| — |` 形式）の検出も 0 件（PR #1122 で対応済み、CR-004 対象外）。
- 対象ファイルは一切編集していない（0 件のため是正不要）。
...
2. **0 件の裏付け**: PR #1163 カタログ（`.agentdev/inspect/inbox/inspect-docs-finding-20260625-202639.md` L43）でも `AGENTS.md` の X-2a 本体件数を 0 件と報告。本検証の独立再実行結果も 0 件で一致。
...
4. **除外領域の遵守**: src/opencode-local/ は CR-002 により本バッチと排他、未変更。

## 完了条件対応表

| 完了条件（Issue #1174） | 対応 | 証拠 |
|---|---|---|
| AGENTS.md の本文中 ` — ` 検出件数が 0 件であること | 満たす | 本レポート「検証結果」参照（単一ファイル検索、0 件） |
...
| src/opencode-local/ ディレクトリを変更していないこと（CR-002） | 満たす | 本 PR は `.agentdev/inspect/inbox/1174-em-dash-verification.md` のみ追加。src/opencode-local/ は未変更 |
