---
title: "em-dash 査読是正 検証レポート: docs/guides (0 件, Issue #1173)"
source_issue: 1173
parent_issue: 1169
operation_unit: OU-002
source_ru: RU-0006
generated_at: 2026-06-25T22:35:57+09:00
...
scope: docs/guides (src/opencode-local/ 除く)
exclusions: テーブルセル `| — |` (PR #1122 対象外, CR-004)
---

# em-dash 査読是正 検証レポート: docs/guides (Issue #1173)

## 目的

OU-002 Epic #1169 Wave 1 child Issue #1173 の完了条件を満たすため、`docs/guides` 配下の em-dash 本体（半角空白挟み ` — `）残存件数を機械判定アルゴリズムで確定する。PR #1163 カタログ（commit ca95b945, merge e3486488）では当該ディレクトリの em-dash 本体件数を 0 件と報告しており、本検証はその再確認である。

## 検証対象

- 対象ディレクトリ: `docs/guides`（retired/ サブディレクトリは存在しない）
...
`Select-String -Pattern " — " -SimpleMatch`（半角空白 + U+2014 + 半角空白）を全 11 ファイルへ適用した結果、一致件数 0 件。
...
2. **0 件の裏付け**: PR #1163 カタログ（`.agentdev/inspect/inbox/inspect-docs-finding-20260625-202639.md` L42）でも `docs/guides` の X-2a 本体件数を 0 件と報告。本検証の独立再実行結果も 0 件で一致。
...
4. **除外領域の遵守**: src/opencode-local/ は CR-002 により本バッチと排他、未変更。docs/guides に retired/ サブディレクトリは存在しない。

## 完了条件対応表

| 完了条件（Issue #1173） | 対応 | 証拠 |
|---|---|---|
| docs/guides の本文中 ` — ` 検出件数が 0 件であること | 満たす | 本レポート「検証結果」参照（11 ファイル検索、0 件） |
...
| src/opencode-local/ ディレクトリを変更していないこと（CR-002） | 満たす | 本 PR は `.agentdev/inspect/inbox/1173-em-dash-verification.md` のみ追加。src/opencode-local/ は未変更 |
