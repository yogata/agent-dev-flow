# inspect-docs finding 20260916T003324Z

## サマリ

- スキャン対象: 現行 REQ 51件・retired 12件、Decision 28件、Design 171ファイル（status なし README 含む、accepted 170）、guides 12ファイル+README、ルート README・docs/README・requirements/README、配布物 246ファイル（`.opencode/commands/agentdev/`・`.opencode/skills/agentdev-*/`。シンボリック投影先を含む実ファイル走査）
- 探索手段: README 索引・正規成果物の直接読取・node による機械的走査（frontmatter、索引・AUTOGEN 突合、親REQ/DEC/行レベル参照実在、相対リンク実在、エンコーディング/構文/ID パターン、コマンド参照実在）。前回診断（20260914T214425Z）以降の docs 変更 58ファイルの差分精読を含む
- 検出事項: 2件（REQ 構造 DRIFT 1件、横断契約矛盾 1件）
- high severity: 0件（medium 2件）
- 既知 defer の継続・解消確認: 継続 8件・解消 2件（後述）
...
- **recommended_route**: UPDATE（旧行番号から現行所有行への付け替え一括是正。cleanup モデル処置候補: REFERENCE）。intake inbox の `2026-09-15-traceability-unknown-req-refs-existing-50.md`（ADF-COVERS 宣言 50箇所）と対象が重複・拡張関係（宣言側は #2879 で解消済み、本検出は本文引用側）のため、inspect-promote での統合判定を推奨。inspect-promote → backlog-review
...
- **recommended_route**: UPDATE（numbering-policy へ「分割・付け替え時の採番例外」の記録を追加、または 063〜081 を意図的予約欠番として policy・README へ明記）。inspect-promote → backlog-review
- **ng_classification**: 今回修正対象（ee306369、Case #2846 由来の波及。ただし採番自体はユーザー裁定によるため、修正対象は記録漏れ側）
- **notes**: req-define入力案「—」（Design 1行追記で解消可能な範囲）

## 推奨アクション

- F-01: 旧行番号引用の現行所有行への付け替え一括是正（対象約50ファイル・151箇所。機械列挙データは本診断の走査結果を再現可能。intake 既存 item との統合判定を inspect-promote で実施）
- F-02: numbering-policy への分割時採番例外の記録追加または 063〜081 欠番の明記
- 後段: `/agentdev/inspect-promote`（分類・採用）→ `/agentdev/backlog-review`
