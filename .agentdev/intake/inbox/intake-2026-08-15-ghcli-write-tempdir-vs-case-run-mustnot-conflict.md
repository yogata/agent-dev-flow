# intake: agentdev-gh-cli WRITE 標準手続きの一時ファイル配置先（.agentdev/tmp/）と case-run 委譲 MUST NOT（.agentdev/** 全域）の衝突
...
実装（case-run 実行担当サブエージェント委譲時の gh WRITE 操作）

## 検知方法

PR #2132 作成過程での手続き適用時の観測。
...
PR #2132 本文 `## Findings / Capture候補` intake セクションより:
...
#intake #agentdev-gh-cli #write-procedure #case-run #delegation-constraint
