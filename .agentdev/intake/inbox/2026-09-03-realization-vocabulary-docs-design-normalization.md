...
原因分類: 箇所の実在と語彙未反映は**確認済**（PR #2552 本文 Findings 記録 + case-close による main 上 rg 再確認）/ 概念面は現行契約と**矛盾しない**（req-define Design の実装面定義と req-analysis Design の列挙は、正規所有責務を持つ実体・成果物への変更の分析という点で現行契約と整合。語彙の旧式のみが残存）
...
- docs/designs/commands/req-define.md（164、175-178 行目）
- docs/designs/skills/agentdev-req-analysis.md（67 行目）
