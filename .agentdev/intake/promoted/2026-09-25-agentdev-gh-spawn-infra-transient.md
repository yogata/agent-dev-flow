...
- 元 intake item: `2026-09-25-agentdev-gh-spawn-infra-transient.md`（採用確定後に inbox から削除。原文は本ファイルに保持）
- 観測日: 2026-09-25
...
- learning 関連: learning-promote（2026-09-25・commit 1472a1aa）にて本 intake の先行処理と learning 固有知見の deferred 維持がユーザー HITL 承認済み。custom-tool-contracts.md への新規契約記録は当該承認の範囲外
...
- 回復単位の誤配置: gh spawn 環境の劣化はインフラ（harness）層の transient 障害で、serve 再起動で回復する。なのに現行では Case 側の blocker（ワークフロー停止）として分類され、supervisor が気づいて再起動するまで pipeline が前進しない
- 停止分類への混入: ツール基盤故障は ADF の停止条件（HITL・CI 失敗・構成不備等）のいずれの本質でもないが、正規書込経路が `agentdev_gh` に限定され代替が契約禁止のため、結果的に workflow 停止と同形になる
...
補足: 上記は観測時点の提案である。本採用は後続分析対象への振り分けを示すものであり、提案の実装承認を意味しない。
...
- 関連: case-auto の停止理由分類、case-ready STEP-7 の書込経路契約（正規書込は agentdev_gh に限定）
