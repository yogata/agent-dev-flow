# N17 command-capture-duty — case-close.md（配布物コマンド）への capture-boundaries 参照追加
...
- command-capture-duty NG が baseline 暫定管理のまま残り、QG-4 の機械 delta に恒常的な計上ノイズが発生する

## 課題

配布物是正の流れで case-close.md へ capture-boundaries 参照を追加し、baseline entry を除去する。併せて baseline entry のパス bucket key を検出パス表記（または checker 側のパス正規化）へ揃える検討を行う。

## 既存要件・成果物との関連

- 対象: src/opencode/commands/agentdev/case-close.md
- baseline: .opencode/skills/repo-agentdev-integrity/baselines/ng-baseline.json N17 entry

## 出典

- 発生日: 2026-08-16
- 発生源: PR #2151 (Issue #2136 / OU-002, Epic #2134 Wave 2) Findings / Capture候補 セクション intake 3、Level 2 rebase note Findings
- 元 item: intake-2026-08-16-ou002-ng17-case-close-capture-boundaries-ref.md
