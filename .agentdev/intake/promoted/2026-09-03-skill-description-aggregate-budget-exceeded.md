...
原因分類: 確認済（機械計測値）/ 超過に至った個別の追記経路は不明（直近の description 追加・拡張のうちどれが寄与したかは未特定）。
...
- AG-005（RU-0018 / Issue #2179）の description 集合予算 warn（層1機械検査）、lint_skills（repo-agentdev-integrity scripts）。
