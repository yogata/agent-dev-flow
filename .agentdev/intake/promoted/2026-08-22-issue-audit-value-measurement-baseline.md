...
計測基準の記録運用は backlog-review〜case-open〜case-run を横断する手続き変更であり、当該 case の是正対象（判定規則確定済み様式違反の機械是正）の範囲外として記録された。
...
- backlog-review → case-open → case-run の横断手続き（監査値の引き渡し様式）、Issue Execution Contract（REQ-017 系）の対象範囲記載。
