...
Epic #1301 は `docs/specs/` 配下の基盤 SPEC ファイル群が対象であり、ガイド層（`docs/guides/`）は Phase 1 のスコープ外。子Issue #1303 の対象範囲も `docs/DOC-MAP.md` に限定されていた。

## 影響
ドキュメント整合性の観点から信頼性低下要因。`docs/README.md`（基準文書の入口、Phase 1 対象 #1302）と `docs/guides/project-docs-and-specs.md`（ガイド層）は同じ SPEC 一覧表を持ち、基準文書との整合性の観点から修正が望ましい。探索利用者が両者で表現ブレを観測すると信頼性低下の要因になる。
...
- main commit 7f9e3472「docs(specs): docs-specs-phase1 Phase 1 整合修正 (6 SPEC updates)」
- REQ-0101-014（ガイド層の位置付け）
- REQ-0101-027（ガイド層の責務）
- ADR-0103（基準文書優先原則）
