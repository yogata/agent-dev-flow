...
- 元 intake item: `2026-09-24-case-close-3085-ir055-remaining-warnings.md`（採用確定後に inbox から削除。原文は本ファイルに保持）
...
- warning クラスは baseline ratchet の delta 管理対象外のため、絶対数の増加が機械的に検知されない。運用回次で蓄積する構造かは未確認
- `accepted-adr-only-citation` ×7 が引用形式の体裁問題か実体的な参照欠落か未分類であり、参照品質の劣化が見過ごされる恐れ
...
- 関連: IR-055（RuntimeReference 計測）の baseline 運用。warning クラスは delta 管理の対象外という計測機構上の区分が発端
