...
## headless 同期委譲の実効上限は環境の 5分 timeout で観測され「同期で完了できる単位」の実効判定基準になる

- **問題事象**: 全 corpus 1 単位の同期委譲が 5分 timeout で失敗（実失敗観察）。実装の選択基準どおり非同期委譲へ切替して完遂した
- **発生局面**: case-run TS-004 subset 測定（Case #3139・DEL-3139-1。この観察と委譲収束実績は TS-003 pass の根拠として有効のまま保持）
- **検知方法**: 同期委譲の timeout 失敗観察
- **根本原因**: 同期委譲の実効上限は harness 環境の timeout 設定（本環境で 5分）に依存し、corpus 全件規模の意味診断は超過する
- **自律対応内容**: 同期失敗 → 非同期切替が実装どおり機能することを含めて完遂（背景子 Abort 対策経路の実証）
- **ユーザー確認有無**: なし
- **Decision/REQ/spec影響**: なし
- **横展開観点**: 「同期委譲で完了できる単位」の判定は事前見積もりではなく環境 timeout の実観測を基準にすると安全。同期失敗 → 非同期切替の経路が機能することまで含めて検証する
- **再発条件**: 同期委譲で corpus 全件・深い読取を要する単位を委譲する場合に再発
- **予防策候補**: 同期委譲の目安時間を skill 側で注記（環境依存のため数値断定はしない）
- **想定反映先**: docs（inspect-docs workflow reference 等への注記追記。具体化の判断は backlog/intake 側）
- **関連**: agentdev-workflow-inspect-docs、Case #3139（PR #3141 本文 測定記録 前回測定節）
- **タグ**: `#delegation` `#headless` `#timeout`

## 並列診断の壁時計短縮は corpus 規模に依存し比較測定は作業内容の対称性を揃える測定設計が必須

- **問題事象**: subset 7 ファイルの非対称比較測定は、並列側が baseline 側に無い候補検証（RA-002 由来）を含むより深い作業を実行しており測定内容が非対称、かつ subset 小規模では委譲固定 overhead が支配するため、TS-004 契約（同一候補整理・同一読み取り・同一比較対象）を満たさず評価を取り下げた
- **発生局面**: case-run TS-004 再測定（Case #3139。DEL-3139-1 subset 測定の撤回 → DEL-3139-3 全件対称再測定で置換）
- **検知方法**: ユーザー判断（2026-09-25）による非対称測定の成功証拠流用不承認・同条件全件再検証の指示
- **根本原因**: 測定設計段階で作業内容の対称性（候補検証の有無・候補入力の同一性）が揃っていなかった
- **自律対応内容**: 全件対称再測定（同一 Git revision・docs corpus 同一〔diff 空〕・全件 303 ファイル/28,619 行・共通候補入力・同一委譲 prompt・同一判定基準・差は逐次か並列かのみ）で再実施し、並列 16分00秒 < 現行実行参照 21分29秒 < 全件逐次 baseline 55分47秒 の両比較対象に対する短縮を実証（単一実行の改善率断定なし REQ-036-033）
- **ユーザー確認有無**: あり（非対称測定の不承認・短縮要件の緩和不承認）
- **Decision/REQ/spec影響**: なし（REQ-036-033 の契約運用の実証のみ）
- **横展開観点**: 並列化の比較測定では作業内容の対称性を測定設計段階で揃える。非対称測定は短縮証拠として採用できない（本測定で実証）
- **再発条件**: 並列化の効果測定を subset で代用し測定内容の対称性を確認しない場合に再発
- **予防策候補**: 比較測定の受け入れ条件に「測定内容の対称性確認」を明示する
- **想定反映先**: docs（並列化評価を要件化する REQ/Design 群。REQ-036-033 は既存。具体化の判断は backlog/intake 側）
- **関連**: REQ-036-032/033、Case #3139（PR #3141 本文 測定記録）
- **タグ**: `#measurement` `#parallelization` `#asymmetric-measurement`

## LLM 意味診断は同一入力・同一 prompt でも検出集合が実行ごとに分散するため突合契約は差分の全件正当理由付けを正とする

- **問題事象**: 同一入力・同一 prompt・同一担当分割の baseline/並列両側測定で、完全再検出 25 + 部分再検出 6、判定差 4、新規検出 20、検出漏れ相当 14 の検出集合分散を観察した（意味的矛盾〔同一対象を一方が現行根拠とし他方が廃止とする等〕は 0 件）
- **発生局面**: case-run TS-001/TS-004 全件対称再測定の finding 突合（Case #3139・DEL-3139-3）
- **検知方法**: 両側検出候補の同一基準突合
- **根本原因**: LLM 意味診断の非決定的推論分散（探索経路差・深読み回数差・観点粒度の解釈差）。同一入力でも検出集合は実行ごとに変動する
- **自律対応内容**: 「検出集合の完全一致」ではなく「検出漏れ・過剰検出の有無確認と差分の全件正当理由付け」を突合基準とし、両側の和集合を本診断構成の発見能力の実測値として記録
- **ユーザー確認有無**: なし
- **Decision/REQ/spec影響**: なし（REQ-036-032 設計の妥当性再確認）
- **横展開観点**: LLM 診断の品質突合契約は決定的検査の期待値一致型にせず差分説明型（正当理由付け）を採用するのが妥当。REQ-036-032 の設計を再確認
- **再発条件**: LLM 意味診断の比較測定・回帰比較で期待値一致を要求する場合に毎回再発
- **予防策候補**: LLM 診断の比較契約テンプレートに差分説明型の明示
- **想定反映先**: docs（比較測定を要件化する REQ/Design 群。具体化の判断は backlog/intake 側）
- **関連**: REQ-036-032、Case #3139（PR #3141 本文 測定記録 finding 突合節）
- **タグ**: `#llm-diagnosis` `#nondeterminism` `#diff-justification`

## 並列測定の委譲回収は親が担当完了を個別記録する方式が失敗 0 件・全面再診断なしの立証に有効

- **問題事象**: 5 委譲の同時背景起動に対し、担当ごとの開始・完了時刻を個別記録する方式で委譲回収を実施した（08:05:57 UTC 同時起動 → 完了 dg-readme 08:13:28 / design-a 08:16:08 / design-b 08:16:15 / design-c 08:19:58 / req-structure 08:20:57）
- **発生局面**: case-run TS-004 全件対称再測定の委譲回収（Case #3139・DEL-3139-3）
- **検知方法**: 委譲記録の実施
- **根本原因**: なし（有効手段の観察記録）
- **自律対応内容**: 担当別個別記録により「失敗 0 件・再実行 0 件・親による全面再診断なし」を立証できた。TS-003 契約（失敗診断担当のみの識別と再実行・親の全面再診断なし）の検証証拠として有効
- **ユーザー確認有無**: なし
- **Decision/REQ/spec影響**: なし
- **横展開観点**: 並列委譲の fan-in 証跡は担当単位の個別記録（開始・完了・結果）で構成すると、失敗識別と全面再診断禁止の立証が機械的になる
- **再発条件**: 並列委譲の結果回収を記録なしで行う場合に立証不能のリスク
- **予防策候補**: 並列委譲 fan-in 手順に担当別個別記録の明示（実装側は STEP-2-1 fan-in 手順として既に存在）
- **想定反映先**: docs（並列委譲を導入する workflow 群。具体化の判断は backlog/intake 側）
- **関連**: agentdev-workflow-inspect-docs（STEP-2-1 fan-in 手順）、Case #3139（PR #3141 本文 測定記録 委譲記録節）
- **タグ**: `#delegation` `#fan-in` `#evidence`
