...
## 2026-08-22: 大規模監査レポートのエントリ完全性は手書きでは欠落しやすい（機械検収で 7 件の欠落を検出・補完）

- **問題事象**: REQ-045 網羅監査（PR 2374）で 714 ファイル・検出事項 28 件の監査レポートを作成した際、初回検収で 7 件のエントリに 4値判定ラベルの欠落があった。TS-002 の機械検収（ラベル存在の正規表現検査）で検出・補完し、再検収で欠落 0 とした。
- **発生局面**: 実装（case-run の監査レポート作成）、検証（TS-002 検収）
- **検知方法**: TS-002 の機械検収（4値判定・証拠7項目のラベル存在を正規表現で検査）
- **根本原因**: 大規模エントリ群の定形項目網羅は手書きでは保証できず、判定ラベル等の欠落が構造的に発生する
- **自律対応内容**: fix-and-reverify で 7 件を補完し再検収した（PR 2374 の Test strategy 結果 TS-002 に記録）
- **ユーザー確認有無**: なし
- **ADR/REQ/Design影響**: なし
- **横展開観点**: 定形項目を多数含む成果物（監査レポート等）の検収は項目存在の機械検査で保証する。回帰検査（Issue 2372）でも同様の機械検収を組むべき
- **再発条件**: 大規模エントリの成果物を機械検収なしで検収する場合
- **予防策候補**: 回帰検査（Wave 3）の検証手順へエントリ完全性の機械検収を明示する
- **想定反映先**: agentdev-workflow-case-run の検証手順、Wave 3（Issue 2372）の実行契約
- **関連**: PR 2374、Issue 2370、REQ-045-003/004
- **タグ**: `#監査` `#機械検収` `#網羅性`

---

## 2026-08-22: 機械検査（check_integrity）の delta NG 集計は観点V10（同一契約の複数箇所定義・検査コード陳腐化）の証拠源として有効

- **問題事象**: REQ-045 網羅監査（PR 2374）の観点V10（同一契約の複数箇所定義による矛盾、検査コードの陳腐化）に係る検出事項（F-019/F-020/F-024/F-025）は、字面走査のみの発見が困難な領域だった。
- **発生局面**: 実装（case-run の監査実行、10観点の機械スイープ）
- **検知方法**: check_integrity.ts の delta NG 集計（baseline 差分）を観点V10 の証拠源として使用
- **根本原因**: 観点V10 は「同一契約の複数箇所定義」「検査コードの陳腐化」という構造的背反の検出であり、単一文書の字面走査では捉えられない
- **自律対応内容**: delta NG 集計を証拠源として F-019/F-020/F-024/F-025 を発見し、レポートの問題クラス（PC-08〜PC-10 係る）へ集約した
- **ユーザー確認有無**: なし
- **ADR/REQ/Design影響**: なし
- **横展開観点**: 意味整合の監査・検査で「複数箇所定義」「陳腐化」を検出する場合は、機械検査の delta 集計を一次証拠源にする
- **再発条件**: delta 集計を使わず、観点V10 類の監査を字面走査のみで行う場合
- **予防策候補**: 監査・検査手順へ delta NG 集計の活用を明示する
