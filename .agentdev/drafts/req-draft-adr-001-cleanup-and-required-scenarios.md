...
summary: |
  ...
  10シナリオの中身は case-run 工程で確定するが、枠組み（セクション見出しと参照関係）を本ドラフトで定義する。
...
agreed_items:
  ...
  - id: AG-003
    content: |
      ...
      10シナリオの中身（個別シナリオ名、合格基準、関連 SPEC/REQ への参照）は case-run 工程で確定するが、
      本ドラフトでは枠組み（セクション見出しと ADR-001 L114 からの参照関係）を定義する。
      10シナリオは charter.md の基本フロー、ADR-001 決定6 のリリース条件に関連するシナリオから抽出する。
  - id: AG-004
    content: |
...
conflict_resolutions:
  ...
  - id: CR-002
    ...
    resolution: |
      ...
      10シナリオの中身は case-run 工程で確定（charter.md の基本フロー、ADR-001 決定6 のリリース条件関連から抽出）。
  - id: CR-003
...
test_strategy:
  ...
  - id: TS-003
    ...
    verification: |
      ...
      中身（シナリオ名、合格基準、関連 REQ/SPEC）は case-run 工程で確定するため、本検証時点では枠組みのみでよい。
    pass_criteria: |
...
- charter.md の基本フロー、ADR-001 決定6 のリリース条件に関連するシナリオから10件を抽出
- 各シナリオに「名称、合格基準、関連 REQ/SPEC」を定義
- docs/specs/quality/quality-specs.md「必須シナリオ（10シナリオ）」セクションへ記載
- ADR-001 L114 からの参照関係を確認
...
主要な変更対象は ADR-001 と docs/specs/quality/quality-specs.md。scale: large（10シナリオの中身確定に調査が必要なため）。

後続コマンドは spec-save（quality-specs.md 新規セクション追加）→ req-save（ADR-001 直接編集、REQ/ADR ファイルとして保存、新規 ADR なし）→ case-open → case-run（10シナリオ中身確定、明示承認記録）を想定。
