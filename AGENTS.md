...
- oh-my-openagent は OpenCode プラグインとしてエージェント型（Sisyphus-Junior 等）を提供する。case-run は `task(subagent_type="Sisyphus-Junior", load_skills=["ulw-loop"])` 経由でエージェント型を起動する（CLI subprocess は使用しない・ADR-0128 Decision #1）
