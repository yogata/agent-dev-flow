...
1. 現行 REQ: `docs/requirements/REQ-0101.md` ～ `docs/requirements/REQ-0142.md`（REQ-0111, REQ-0115, REQ-0116, REQ-0117, REQ-0118, REQ-0120, REQ-0121, REQ-0122 は廃止）。
...
- 新ガイドライン（要件行記述基準・HOW 除去等）は新規 REQ から適用し、既存 REQ は別途整備 Issue がない限り現状維持すること（REQ-0147-002）。
...
本プロジェクトは case-run コマンドの実行ハーネスとして [oh-my-openagent](https://github.com/code-yeongyu/oh-my-openagent) を選定し、導入済み（ADR-0114・ADR-0128）。

- oh-my-openagent は OpenCode プラグインとしてエージェント型（Sisyphus-Junior 等）を提供する。case-run は `task(subagent_type="Sisyphus-Junior", load_skills=["agentdev-case-run-execution-adapter"])` 経由でエージェント型を起動する（委譲 prompt 内で `/ulw-loop` command を指定・CLI subprocess は使用しない・ADR-0128 Decision #1）
- 委譲プロンプト・task() 起動方式の詳細は `.opencode/skills/agentdev-case-run-execution-adapter/references/oh-my-openagent.md` 参照
